...
layout( binding = 19 ) uniform sampler2D g_tFow;
...
layout(location=3) out vec4 PS_INPUT_gl_vNormalWs;
...
void main()
{
	...
	vec3 misc3a4 = mix(vFullFowColor, H_remlte1, flFowResult.x);
	vec3 H_1u6qqe = g_vRefractControl.zzz * misc3a4;
	vec3 H_b1funu1 = H_1u6qqe * g_vecBlendControl.zzz;
	vec4 H_7iwv2m1 = vec4(H_b1funu1.x, H_b1funu1.y, H_b1funu1.z, H_771o8l);
	float flDist = distance(vecWorldPos, g_vCameraPositionWs);
	float H_j6vk5z = flDist * g_flInvFogRange;
	float parama = H_j6vk5z + g_flNegFogStartOverFogRange;
	float misc3a5 = clamp(parama, C_0d0, C_1d0);
	float flFog = pow(misc3a5, g_flFogExponent);
	float misc2a3 = min(g_flFogMaxDensity, flFog);
	vec4 H_yinz2w1 = vec4(vNormalWs.x, vNormalWs.y, vNormalWs.z, misc2a3);
	PS_INPUT_gl_m_vTexCoords = H_c3n6se1;
	PS_INPUT_gl_m_vTintColor = H_7iwv2m1;
	PS_INPUT_gl_vPositionWs_flLinearDepth = H_f4kfbg1;
	PS_INPUT_gl_vNormalWs = H_yinz2w1;
