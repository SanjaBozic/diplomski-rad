...
layout( binding = 0 ) uniform sampler2D g_tBaseTextureNoSRGB;
...
void main()
{
	vec4 vecSourceTexel = texture(g_tBaseTextureNoSRGB, PS_INPUT_gl_m_vTexCoords.xy);
	vec3 H_mc1p161 = vecSourceTexel.xyz + C_u1zmzx1;
	vec3 H_3w9id71 = H_mc1p161 * PS_INPUT_gl_m_vTintColor.xyz;
	float flLuminance = dot(H_3w9id71, C_kfumqy1);
	vec3 H_t1lelu1 = vec3(flLuminance);
	vec3 vecSourceTexel1 = mix(H_3w9id71, H_t1lelu1, g_flDesatAmount);
	...
	float misc3a = clamp(H_6jtzu9, C_0d0, C_1d0);
	float H_9v8ms6 = PS_INPUT_gl_m_vTintColor.w * misc3a;
	vec3 H_1s3obw = vec3(g_flMod2xIdentity);
	vec3 vModulationColor = mix(H_1s3obw, vecSourceTexel2.xyz, PS_INPUT_gl_m_vTintColor.xyz);
	vec3 vFinalOutput = mix(H_1s3obw, vModulationColor, H_9v8ms6);
	float H_4rx1xx1 = H_9v8ms6 * g_vecBlendControl.w;
	vec4 H_ntaxt71 = vec4(vFinalOutput.x, vFinalOutput.y, vFinalOutput.z, H_4rx1xx1);
	PS_OUTPUT_gl_m_vColor = H_ntaxt71;
}
