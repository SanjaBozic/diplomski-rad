...
layout(location=5) in vec4 PS_INPUT_gl_m_vTexCoordsSeq1;
...
void main()
{
	...
	vec4 vecSourceTexel2 = texture(g_tBaseTexture, PS_INPUT_gl_m_vTexCoordsSeq1.xy);
	vec4 vecSourceTexel3 = texture(g_tBaseTexture, PS_INPUT_gl_m_vTexCoordsSeq1.zw);
	vec4 vRet1 = mix(vecSourceTexel2, vecSourceTexel3, PS_INPUT_gl_m_vAnimationBlendValues.y);
	vec3 H_cmmu3a1 = PS_INPUT_gl_m_vTintColor.xyz * vRet1.xyz;
	float maxVal = max(H_cmmu3a1.y, H_cmmu3a1.z);
	float maxVal1 = max(H_cmmu3a1.x, maxVal);
	float maxVal2 = max(maxVal1, C_1d0);
	vec3 H_yiibq21 = vec3(maxVal2);
	vec3 param = H_cmmu3a1 / H_yiibq21;
	...
	vec3 vecSourceTexel4 = mix(H_4iwnh7, H_t1lelu1, g_flDesatAmount);
	vec4 H_498olf1 = vRet;
	H_498olf1.xyz = vecSourceTexel4.xyz;
	vec4 vecSourceTexel5 = H_498olf1 * g_vColorTransformScale;
	float H_f22fwf = g_flColorTransformAlphaBias + vecSourceTexel5.w;
	float misc3a2 = clamp(H_f22fwf, C_0d0, C_1d0);
	float H_hpqrbr1 = PS_INPUT_gl_m_vTintColor.w * misc3a2;
	vec3 H_imt73b = vec3(H_hpqrbr1);
	vec3 misc3a3 = clamp(vecSourceTexel5.xyz, C_vec3p0d0p, C_vec3p1d0p);
