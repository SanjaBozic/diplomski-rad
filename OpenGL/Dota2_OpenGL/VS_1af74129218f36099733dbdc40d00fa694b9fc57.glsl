...
layout(location=3) in vec2 VS_INPUT_gl_vTexCoords;
layout(location=4) in vec2 VS_INPUT_gl_vTransformTextureUV;
out gl_PerVertex {
	vec4 gl_Position;
	float gl_ClipDistance[1];
} ;
layout(location=0) out vec2 PS_INPUT_gl_vTexCoords;
layout(location=1) out vec4 PS_INPUT_gl_vPositionWs_flLinearDepth;
layout(location=2) out vec3 PS_INPUT_gl_vNormalWs;
layout(location=3) out vec4 PS_INPUT_gl_vVertexColor;
layout(location=4) out vec3 PS_INPUT_gl_vTangentUWs;
layout(location=5) out vec3 PS_INPUT_gl_vTangentVWs;
...
void main()
{
	...
	vec3 vTangentVWs = cross(vNormalWs, vTangentUWs1);
	vec3 vTangentVWs1 = VS_INPUT_gl_vTangentUOs_flTangentVSign.www * vTangentVWs;
