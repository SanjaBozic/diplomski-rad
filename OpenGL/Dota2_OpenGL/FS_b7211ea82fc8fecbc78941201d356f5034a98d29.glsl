...
layout( binding = 8 ) uniform sampler2D g_tClouds;
layout( binding = 7 ) uniform sampler2D g_tDiffuseWarp;
...
void main()
{
	...
	float flDiffuseWarpMask = max(vMasks.y, g_flDiffuseWarpBlendToFull);
	...
	vec3 H_mzytuo1 = vAlbedo1 * vDiffuse5;
	vec3 H_1nnjud = H_mzytuo1 + vSpecular4;
	vec3 H_0w5cj1 = mix(H_1nnjud, vSpecular4, flMetalnessMask);
	...
	vec3 H_lrs6111 = H_0w5cj1 + vRimLight4;
	float misc3a6 = clamp(flSelfIllumMask1, C_0d0, C_1d0);
	vec3 H_wgz99j1 = mix(H_lrs6111, vAlbedo, misc3a6);
	vec4 H_rag4dq1 = vec4(H_wgz99j1.x, H_wgz99j1.y, H_wgz99j1.z, H_1f2e8d1);
	PS_OUTPUT_gl_vDiffuse = H_rag4dq1;
