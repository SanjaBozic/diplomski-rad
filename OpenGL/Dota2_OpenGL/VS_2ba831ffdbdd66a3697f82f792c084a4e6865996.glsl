//ATTRIBMAP-00-30-60-50-20-10-5D-5E-xx
#version 330 core
// LunarGOO output
...
layout( binding = 19 ) uniform sampler2D g_tCompositeMorphTextureAtlas;
...
layout(location=7) in float VS_INPUT_gl_nVertexIndex;
out gl_PerVertex {
	vec4 gl_Position;
...
const ivec2 C_ivec2p4ca0p = ivec2(4, 0);
const ivec2 C_ivec2p5ca0p = ivec2(5, 0);
const int C_0 = 0;
const int C_1 = 1;
...
void main()
{
	...
	int H_934vuc1 = ivec4(VS_INPUT_gl_vBlendIndices).w << C_2;
	int H_64guqg = C_8 + H_934vuc1;
	float H_vzn43k1 = float(H_64guqg);
	float H_z83kl01 = C_0d5 + H_vzn43k1;
	float H_ec2tso1 = H_z83kl01 * g_vInvGBufferSize.z;
	vec2 H_41k5bl1 = vec2(H_ec2tso1, C_0d0);
	vec2 vBlendTransform0UV3 = H_41k5bl1 + VS_INPUT_gl_vTransformTextureUV;
	...
	vec4 H_jv5o29 = vec4(VS_INPUT_gl_vBlendWeight.w);
	vec4 vMatObjectToWorldRowe = textureLod(g_tTransformTexture, H_6iy5tc, C_0d0);
	vec4 H_u4zydn1 = H_jv5o29 * vMatObjectToWorldRowe;
	vec4 vMatObjectToWorldRowf = H_u4zydn1 + vMatObjectToWorldRowb;
	...
	vec4 H_i2x44v1 = H_jv5o29 * vTmp4;
	vec4 vMatObjectToWorldRowg = H_i2x44v1 + vMatObjectToWorldRowc;
	vec4 H_hri4ev1 = H_jv5o29 * vTmp5;
	vec4 vMatObjectToWorldRowh = H_hri4ev1 + vMatObjectToWorldRowd;
	...
	vec3 H_w9zzx51 = vec3(H_g1gr9q.x, H_g1gr9q.y, H_csfx2e);
	...
	vec4 vTmp6 = textureLodOffset(g_tTransformTexture, VS_INPUT_gl_vTransformTextureUV, C_0d0, C_ivec2p4ca0p);
	vec4 vTmp7 = textureLodOffset(g_tTransformTexture, VS_INPUT_gl_vTransformTextureUV, C_0d0, C_ivec2p5ca0p);
	int nVertexId = int(uint(VS_INPUT_gl_nVertexIndex));
	int nURange = int(uint(vTmp6.w));
	int H_7ioyqm = int(uint(nVertexId) % uint(nURange));
	int H_rvfptx1 = int(uint(vTmp6.z));
	int H_xk1phm1 = H_7ioyqm + H_rvfptx1;
	int H_tjaejh = int(uint(nVertexId) / uint(nURange));
	int H_nkt9kp = int(uint(vTmp7.w));
	int H_tu367v = H_nkt9kp + H_tjaejh;
	ivec2 H_arsojx1 = ivec2(H_xk1phm1, H_tu367v);
	vec4 vPositionDeltaOs = texelFetch(g_tCompositeMorphTextureAtlas, H_arsojx1, C_0);
	int H_bqcr0a = int(uint(g_vMorphTextureAtlasSize.x));
	int H_szkiww = int(uint(H_bqcr0a) >> uint(C_1));
	int H_vp21ng = H_szkiww + H_xk1phm1;
	ivec2 H_ynni7q = ivec2(H_vp21ng, H_tu367v);
	vec4 vMorphNormalDeltaOs_flWrinkle = texelFetch(g_tCompositeMorphTextureAtlas, H_ynni7q, C_0);
	vec3 H_ag176n = VS_INPUT_gl_vPositionOs + vPositionDeltaOs.xyz;
	vec3 H_erzgcl = H_w9zzx51 + vMorphNormalDeltaOs_flWrinkle.xyz;
	vec3 vNormalOs = normalize(H_erzgcl);
	vec4 H_6hzazs1 = vec4(vNormalOs.x, vNormalOs.y, vNormalOs.z, C_0d0);
	float dotres = dot(H_6hzazs1, vMatObjectToWorldRowf);
	float dotres1 = dot(H_6hzazs1, vMatObjectToWorldRowg);
	float dotres2 = dot(H_6hzazs1, vMatObjectToWorldRowh);
	...
	float dotres3 = dot(H_cs27ko1, vMatObjectToWorldRowf);
	float dotres4 = dot(H_cs27ko1, vMatObjectToWorldRowg);
	float dotres5 = dot(H_cs27ko1, vMatObjectToWorldRowh);
	...
	vec3 H_3ce8vv = H_ag176n * vAnimationControlWord.zzz;
	vec4 H_wpo4x6 = vec4(H_3ce8vv.x, H_3ce8vv.y, H_3ce8vv.z, C_1d0);
	float dotres6 = dot(H_wpo4x6, vMatObjectToWorldRowf);
	float dotres7 = dot(H_wpo4x6, vMatObjectToWorldRowg);
	float dotres8 = dot(H_wpo4x6, vMatObjectToWorldRowh);
	...
	PS_INPUT_gl_vBaseTexCoord_vDetailTexCoord = H_m6u7rh;
	PS_INPUT_gl_vNormalWs = vNormalWs;
	PS_INPUT_gl_vTangentUWs_flTangentVSign = H_5as0dk1;
	PS_INPUT_gl_vVertexColor = H_4n00ue1;
	PS_INPUT_gl_vPositionWs_flLinearDepth = H_s4cwvz;
	PS_INPUT_gl_vRimColor_flFog = H_jk7un5;
	PS_INPUT_gl_vLightAtten = C_vec4p0d0p;
