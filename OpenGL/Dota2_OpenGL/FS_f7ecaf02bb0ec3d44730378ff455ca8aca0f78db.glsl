...
void main()
{
	...
	vec4 H_z3s84s1 = vec4(H_4iwnh7.x, H_4iwnh7.y, H_4iwnh7.z, vecSourceTexel1);
	vec4 select8 = H_ekgkwb1 ? H_z3s84s1 : H_s911ye1;
	...
	float select9 = H_zwh6gm1 ? oFactor : oFactor1;
	vec4 vecSourceTexel2 = mix(select8, g_vecOutlineColor, select9);
	vec4 selecta = H_2diyjf ? vecSourceTexel2 : select8;
	float flLuminance = dot(selecta.xyz, C_kfumqy1);
	vec3 H_t1lelu1 = vec3(flLuminance);
	vec3 vecSourceTexel3 = mix(selecta.xyz, H_t1lelu1, g_flDesatAmount);
	vec4 H_8rhj1i = selecta;
	H_8rhj1i.xyz = vecSourceTexel3.xyz;
	vec4 vecSourceTexel4 = H_8rhj1i * g_vColorTransformScale;
	float H_c137u51 = g_flColorTransformAlphaBias + vecSourceTexel4.w;
	float misc3a2 = clamp(H_c137u51, C_0d0, C_1d0);
	float H_hpqrbr1 = PS_INPUT_gl_m_vTintColor.w * misc3a2;
	vec3 H_imt73b = vec3(H_hpqrbr1);
	vec3 misc3a3 = clamp(vecSourceTexel4.xyz, C_vec3p0d0p, C_vec3p1d0p);
