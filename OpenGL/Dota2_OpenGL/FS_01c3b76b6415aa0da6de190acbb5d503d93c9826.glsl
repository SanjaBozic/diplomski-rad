...
void main()
{
	...
	vec4 vecSourceTexel1 = texture(g_tBaseTexture, PS_INPUT_gl_m_vTexCoordsSeq1.xy);
	vec4 vecSourceTexel2 = texture(g_tBaseTexture, PS_INPUT_gl_m_vTexCoordsSeq1.zw);
	vec4 vRet = mix(vecSourceTexel1, vecSourceTexel2, PS_INPUT_gl_m_vAnimationBlendValues.y);
	...
	vec3 vecSourceTexel3 = mix(H_flafr61, H_t1lelu1, g_flDesatAmount);
	vec4 H_s911ye1 = vecSourceTexel;
	H_s911ye1.xyz = vecSourceTexel3.xyz;
	vec4 vecSourceTexel4 = H_s911ye1 * g_vColorTransformScale;
	float H_c137u51 = g_flColorTransformAlphaBias + vecSourceTexel4.w;
	float H_nwnqss1 = H_c137u51 * PS_INPUT_gl_m_vTintColor.w;
	vec3 H_1xiw7v1 = vec3(H_nwnqss1);
	vec3 misc3a = clamp(vecSourceTexel4.xyz, C_vec3p0d0p, C_vec3p1d0p);
	vec3 H_fotevb1 = H_1xiw7v1 * misc3a;
	float H_6hjnec = H_nwnqss1 * g_vecBlendControl.w;
	vec4 H_whvays = vec4(H_fotevb1.x, H_fotevb1.y, H_fotevb1.z, H_6hjnec);
	PS_OUTPUT_gl_m_vColor = H_whvays;
}
