...
layout( binding = 7 ) uniform samplerCube g_tCubeMap;
...
void main()
{
	...
	float misc2a = dot(H_qw6t8a1, H_t3ywhi1);
	float H_qdt8jw1 = misc2a * C_2d0;
	vec3 H_f33zyj = vec3(H_qdt8jw1);
	vec3 H_0acp1y1 = H_f33zyj * H_qw6t8a1;
	float misc2a1 = dot(H_qw6t8a1, H_qw6t8a1);
	vec3 H_p4zmch = vec3(misc2a1);
	vec3 H_6ji6ta1 = H_p4zmch * H_t3ywhi1;
	vec3 H_ybvulf = H_0acp1y1 - H_6ji6ta1;
	vec4 vCubeMap = texture(g_tCubeMap, H_ybvulf);
	vec3 H_u54pb21 = vec3(g_flCubeMapScalar);
	vec3 vCubeMap1 = H_u54pb21 * vCubeMap.xyz;
	vec3 H_dszgw31 = vec3(g_bMaskCubeMapByMetalness);
	float H_diz9vu = C_1d0 - g_bMaskCubeMapByMetalness;
	vec3 H_fitx9b1 = vec3(H_diz9vu);
	vec3 H_15remp = H_fitx9b1 * vCubeMap1;
	vec3 vSpecular2 = H_15remp + vSpecular1;
	vec3 vSpecular3 = H_0ooyak * vSpecular2;
	vec3 vSpecularTint = mix(vColorTexel.xyz, g_vSpecularColor, flTintByBaseMask);
	vec3 vSpecular4 = vSpecular3 * vSpecularTint;
	vec3 vSpecular5 = H_txpgr4 * vSpecular4;
	vec3 H_mcvnai1 = vAlbedo * vDiffuse4;
	vec3 H_uhlpk91 = H_mcvnai1 + vSpecular5;
	vec3 H_rn9sed1 = H_dszgw31 * vCubeMap1;
	vec3 vMetalness = H_rn9sed1 + vSpecular5;
	vec3 H_tnx4q6 = mix(H_uhlpk91, vMetalness, flMetalnessMask);
	...
	vec3 H_y88cgl = H_tnx4q6 + vRimLight4;
	float misc3a6 = clamp(flSelfIllumMask, C_0d0, C_1d0);
	vec3 H_o7oys71 = mix(H_y88cgl, vColorTexel.xyz, misc3a6);
	vec4 H_ja13ni1 = vec4(H_o7oys71.x, H_o7oys71.y, H_o7oys71.z, H_1f2e8d1);
	PS_OUTPUT_gl_vDiffuse = H_ja13ni1;
