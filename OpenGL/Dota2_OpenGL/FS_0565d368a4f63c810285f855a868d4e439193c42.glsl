...
layout( binding = 5 ) uniform sampler2D g_tDiffuseWarp;
...
layout( binding = 2 ) uniform sampler2D g_tMasks2;
layout( binding = 3 ) uniform sampler2D g_tColor;
...
void main()
{
	...
	float flDiffuseWarpMask = max(C_0d0, g_flDiffuseWarpBlendToFull);
	float flMetalnessMask = max(C_0d0, g_flMetalnessBlendToFull);
	float flSelfIllumMask = max(C_0d0, g_flSelfIllumBlendToFull);
	float flFresnelSpecular = max(vFresnelTerms.z, flMetalnessMask);
	vec3 H_txpgr4 = vec3(flFresnelSpecular);
	vec4 vMasks = texture(g_tMasks2, PS_INPUT_gl_vBaseTexCoord_vDetailTexCoord.xy);
	float flSpecularMask = max(vMasks.x, g_flSpecularBlendToFull);
	vec3 H_0ooyak = vec3(flSpecularMask);
	float flRimMask = max(vMasks.y, g_flEnvMapBlendToFull);
	vec3 H_znzd9q = vec3(flRimMask);
	float flTintByBaseMask = max(vMasks.z, g_flReflectionsTintByBaseBlendToNone);
	float flSpecularExponent = max(vMasks.w, g_flSpecularExponentBlendToFull);
	...
	vec2 H_x4l7oh1 = vec2(flHalfLambert1, flDiffuseWarpMask);
	vec4 vDiffuse = texture(g_tDiffuseWarp, H_x4l7oh1);
	vec3 vDiffuse1 = g_vGlobalLightColor * vDiffuse.xyz;
	...
	float vDiffuse2 = dot(g_vGlobalLightAmbientDir, H_qw6t8a1);
	float misc3a5 = clamp(vDiffuse2, C_0d0, C_1d0);
	...
	vec3 vDiffuse3 = H_xovjmu + vDiffuse1;
	...
	float vDiffuse4 = max(H_ckpxio1, H_lf8czm1);
	vec3 H_d8mijr = vec3(vDiffuse4);
	vec3 H_07xnys1 = H_d8mijr * vShadowAmbientColor1;
	vec3 H_4ruzat = H_07xnys1 * H_j21u991;
	vec3 vDiffuse5 = H_4ruzat + vDiffuse3;
	...
	vec3 H_l1hnki1 = vAlbedo * vDiffuse5;
	vec3 H_qoc5t2 = H_l1hnki1 + vSpecular4;
	vec3 H_owma1o = mix(H_qoc5t2, vSpecular4, flMetalnessMask);
	...
	vec3 H_5izeg01 = H_owma1o + vRimLight4;
	float misc3a6 = clamp(flSelfIllumMask, C_0d0, C_1d0);
	vec3 H_472037 = mix(H_5izeg01, vColorTexel.xyz, misc3a6);
	vec4 H_vaumf01 = vec4(H_472037.x, H_472037.y, H_472037.z, H_1f2e8d1);
	PS_OUTPUT_gl_vDiffuse = H_vaumf01;
