...
void main()
{
	...
	float maxVal = max(H_3w9id71.y, H_3w9id71.z);
	float maxVal1 = max(H_3w9id71.x, maxVal);
	float maxVal2 = max(maxVal1, C_1d0);
	vec3 H_yiibq21 = vec3(maxVal2);
	vec3 param = H_3w9id71 / H_yiibq21;
	...
	vec3 vFinalOutput = mix(H_1s3obw, vModulationColor, H_hpqrbr1);
	float H_zr08kx = H_hpqrbr1 * g_vecBlendControl.w;
	vec4 H_ynq65b = vec4(vFinalOutput.x, vFinalOutput.y, vFinalOutput.z, H_zr08kx);
	PS_OUTPUT_gl_m_vColor = H_ynq65b;
}
