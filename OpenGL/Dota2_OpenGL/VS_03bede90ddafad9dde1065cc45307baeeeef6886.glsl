...
void main()
{
	...
	vec3 vRight = cross(C_imhxe9, g_vCameraDirWs);
	...
	vec3 H_b4eox7 = C_imhxe9 * H_yaca1b;
	vec3 H_zizxhg = H_3j552u + H_b4eox7;
	vec3 H_h1him61 = C_imhxe9 * H_zxczdp;
	vec3 H_l64tfc = H_yaca1b * vRight;
	vec3 H_v799ek = H_h1him61 - H_l64tfc;
	float flSin = sin(VS_INPUT_gl_m_vRotationAndRadiusData.y);
	float flCos = cos(VS_INPUT_gl_m_vRotationAndRadiusData.y);
	float flAxisXSquared = H_v799ek.x * H_v799ek.x;
	float flAxisYSquared = H_v799ek.y * H_v799ek.y;
	float flAxisZSquared = H_v799ek.z * H_v799ek.z;
	float H_1zt6w01 = C_1d0 - flAxisXSquared;
	float H_itict51 = H_1zt6w01 * flCos;
	float H_m0w0j91 = H_itict51 + flAxisXSquared;
	float H_aut1m21 = H_v799ek.x * H_v799ek.y;
	float H_k83isr = C_1d0 - flCos;
	float H_e3ri7i = H_aut1m21 * H_k83isr;
	float H_fc0lv5 = H_v799ek.z * flSin;
	float H_r0owho = H_e3ri7i + H_fc0lv5;
	float H_b582c21 = H_v799ek.x * H_v799ek.z;
	float H_qp97141 = H_b582c21 * H_k83isr;
	float H_ud3k4m1 = H_v799ek.y * flSin;
	float H_0mwpdq = H_qp97141 - H_ud3k4m1;
	float H_5ralyq = H_e3ri7i - H_fc0lv5;
	float H_0hkqtj1 = C_1d0 - flAxisYSquared;
	float H_aobm0p = H_0hkqtj1 * flCos;
	float H_73knml1 = H_aobm0p + flAxisYSquared;
	float H_6sjamr = H_v799ek.y * H_v799ek.z;
	float H_mhz5pl1 = H_6sjamr * H_k83isr;
	float H_5tx5wq = H_v799ek.x * flSin;
	float H_neu9el1 = H_5tx5wq + H_mhz5pl1;
	float H_iz36gz = H_qp97141 + H_ud3k4m1;
	float H_fa4fgy1 = H_mhz5pl1 - H_5tx5wq;
	float H_7i1isn1 = C_1d0 - flAxisZSquared;
	float H_lr8md3 = H_7i1isn1 * flCos;
	float H_rv88f5 = H_lr8md3 + flAxisZSquared;
	vec3 H_7dzfxk1 = vec3(H_m0w0j91, H_5ralyq, H_iz36gz);
	vec3 H_ctmqpq1 = vec3(H_r0owho, H_73knml1, H_fa4fgy1);
	vec3 H_l1nle21 = vec3(H_0mwpdq, H_neu9el1, H_rv88f5);
	float dotres = dot(H_zizxhg, H_7dzfxk1);
	float dotres1 = dot(H_zizxhg, H_ctmqpq1);
	float dotres2 = dot(H_zizxhg, H_l1nle21);
	...
	vec3 H_fc43tf = H_j0cr26 * H_v799ek;
	vec3 H_x4w717 = H_fc43tf + H_j2ki6s;
	...
	vec3 H_o9q0ni1 = H_098nor1 * H_x4w717;
	vec3 vecWorldPos = H_o9q0ni1 + VS_INPUT_gl_m_vPos;
