...
layout( binding = 3 ) uniform sampler2D g_tNormal;
...
void main()
{
	...
	vec4 flParallaxMask = texture(g_tParallaxMask, PS_INPUT_gl_vBaseTexCoord);
	vec2 H_5kzfth = vec2(vVectorTs, vVectorTs1);
	vec2 H_9oj7h3 = H_5kzfth * flParallaxMask.yy;
	vec2 H_0gl6pt = vec2(g_flParallaxStrength);
	vec2 vParallaxVector = H_0gl6pt * H_9oj7h3;
	vec2 vBaseTexCoord = PS_INPUT_gl_vBaseTexCoord + vParallaxVector;
	vec4 vDiffuse = texture(g_tColor, vBaseTexCoord);
	...
	vec4 vSpecular = texture(g_tSpecular, vBaseTexCoord);
	float H_i0xhec = g_flSpecularIntensity * vSpecular.x;
	float H_2q4yo51 = g_flSpecularBloom * vSpecular.z;
	vec4 vNormalTexel = texture(g_tNormal, vBaseTexCoord);
