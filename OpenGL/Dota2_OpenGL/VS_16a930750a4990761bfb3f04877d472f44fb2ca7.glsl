...
layout(location=5) out vec4 PS_INPUT_gl_m_vTexCoordsSeq1;
...
void main()
{
	...
	float param6 = VS_INPUT_gl_m_vSequenceAndTimeData.y - g_vSheetDataIndex.w;
	bool H_mdx7i3 = param6 < C_0d0;
	float phitmp1 = C_3d0 * VS_INPUT_gl_m_vSequenceAndTimeData.y;
	float select2 = H_mdx7i3 ? phitmp1 : C_0d0;
	float H_mq8auc = g_vSheetDataIndex.z * select2;
	float H_fue99i = H_mq8auc + g_vSheetDataIndex.x;
	vec2 H_23b4ob1r = C_a1z03r;
	H_23b4ob1r.y = H_fue99i;
	vec4 vecSequenceControlData1 = textureLod(g_tParticleSheetTexture, H_23b4ob1r, C_0d0);
	float flSequenceIndexingTime1 = H_31hyrm1 * g_vecSequenceAddressingControls.y;
	...
	float misc2a1 = min(param3, flTexFrameIndex4);
	float H_i42rku1 = mod(flTexFrameIndex4, g_vSheetDataIndex.y);
	float select3 = H_kfyb5s1 ? misc2a1 : H_i42rku1;
	float flTexFrameIndex5 = H_fz8gko1 * select3;
	vec2 H_nv43ll1 = vec2(flTexFrameIndex5, H_fue99i);
	vec4 vFrameControlData1 = textureLod(g_tParticleSheetTexture, H_nv43ll1, C_0d0);
	float H_5r83t6 = mix(vFrameControlData1.x, vFrameControlData1.w, H_d9smdi1);
	float H_90mcpv = H_fue99i + g_vSheetDataIndex.z;
	vec2 H_pjr7u5 = vec2(flTexFrameIndex5, H_90mcpv);
	vec4 H_q2y12v = textureLod(g_tParticleSheetTexture, H_pjr7u5, C_0d0);
	float H_nb8l9l = H_fue99i + H_k46du;
	vec2 H_7wky8v1 = vec2(flTexFrameIndex5, H_nb8l9l);
	vec4 H_qmsvjf = textureLod(g_tParticleSheetTexture, H_7wky8v1, C_0d0);
	float H_atbluh1 = H_fz8gko1 + flTexFrameIndex5;
	vec2 H_kngosu = vec2(H_atbluh1, H_90mcpv);
	vec4 H_mo9wp3 = textureLod(g_tParticleSheetTexture, H_kngosu, C_0d0);
	vec2 vCroppedSize1 = H_mo9wp3.zw - H_mo9wp3.xy;
	vec2 H_cq53471 = H_q2y12v.zw - H_q2y12v.xy;
	vec2 vUncroppedSize2 = H_cq53471 / vCroppedSize1;
	vec2 H_x8961l1 = H_mo9wp3.xy * vUncroppedSize2;
	vec2 H_r6racf1 = H_q2y12v.xy - H_x8961l1;
	vec2 H_rk4rv81 = H_r6racf1 + vUncroppedSize2;
	vec2 H_4al5e2 = H_qmsvjf.zw - H_qmsvjf.xy;
	vec2 vUncroppedSize3 = H_4al5e2 / vCroppedSize1;
	vec2 H_yjn6rk1 = H_mo9wp3.xy * vUncroppedSize3;
	vec2 H_7c5exk1 = H_qmsvjf.xy - H_yjn6rk1;
	vec2 H_n5rzhy = H_7c5exk1 + vUncroppedSize3;
	float param7 = C_1d0 / g_vecSequenceAddressingControls.w;
	float misc3a2 = clamp(param7, C_0d0, C_1d0);
	vec4 H_s51pdr = vec4(misc3a2);
	vec4 H_8wnfzo = H_646op7 * H_s51pdr;
	vec4 vZoomedUVs1 = C_vec4p0d5p + H_8wnfzo;
	float H_5q0pyb1 = C_0d5 * H_5r83t6;
	float H_4d24fa1 = C_0d5 + H_5q0pyb1;
	vec4 H_0w7dxj1 = vec4(H_4d24fa1, H_4d24fa1, H_5q0pyb1, H_5q0pyb1);
	vec4 vLerpedValue1 = mix(H_4p7ye11, vZoomedUVs1, H_0w7dxj1);
	float H_e0zfew = g_vecFinalUVOffsetAndScale.z * vLerpedValue1.x;
	float H_v60g8m = H_e0zfew + g_vecFinalUVOffsetAndScale.x;
	float H_u2nfqz = g_vecFinalUVOffsetAndScale.w * vLerpedValue1.y;
	float H_7wvio1 = H_u2nfqz + g_vecFinalUVOffsetAndScale.y;
	float H_gmrguv = g_vecFinalUVOffsetAndScale.z * vLerpedValue1.z;
	float H_04o7ye = H_gmrguv + g_vecFinalUVOffsetAndScale.x;
	float H_k0obi21 = g_vecFinalUVOffsetAndScale.w * vLerpedValue1.w;
	float H_zxfty81 = H_k0obi21 + g_vecFinalUVOffsetAndScale.y;
	vec4 H_ro2se71 = vec4(H_v60g8m, H_7wvio1, H_04o7ye, H_zxfty81);
	vec4 vDeltaUV3 = g_vecFinalUVOffsetAndScale.xyxy - H_ro2se71;
	vec4 vDeltaUV4 = C_vec4p2d0p * vDeltaUV3;
	vec4 vDeltaUV5 = g_vecFinalUVOffsetAndScale.zwzw + vDeltaUV4;
	bool H_tx3n3k = vDeltaUV5.x < C_0d0;
	float H_ieioux1 = H_tx3n3k ? H_mo9wp3.z : H_mo9wp3.x;
	bool H_68bszv = vDeltaUV5.y < C_0d0;
	float vTestCorner1 = H_68bszv ? H_mo9wp3.w : H_mo9wp3.y;
	vec2 H_9whdk1 = vec2(H_ieioux1, vTestCorner1);
	vec2 H_tnuman = vec2(H_v60g8m, H_7wvio1);
	vec2 H_7p3xlp = H_9whdk1 - H_tnuman;
	vec2 param8 = H_7p3xlp / vDeltaUV5.xy;
	vec2 misc3a3 = clamp(param8, C_vec2p0d0p, C_vec2p1d0p);
	vec2 vCroppedCorner = min(misc3a1, misc3a3);
	vec4 H_t5fax11 = vCroppedCorner.xyxy * vDeltaUV5;
	vec4 vSeq1LocalCornerUVs = H_ro2se71 + H_t5fax11;
	vec2 H_tn7d13 = H_rk4rv81 - H_r6racf1;
	vec2 H_uqarg11 = H_tn7d13 * vSeq1LocalCornerUVs.xy;
	vec2 H_tpu1mh = H_r6racf1 + H_uqarg11;
	vec2 H_qefy3a1 = H_n5rzhy - H_7c5exk1;
	vec2 H_7oev5z = H_qefy3a1 * vSeq1LocalCornerUVs.zw;
	vec2 H_okpv0x1 = H_7c5exk1 + H_7oev5z;
	vec4 H_kvqmst1 = vec4(H_tpu1mh.x, H_tpu1mh.y, H_okpv0x1.x, H_okpv0x1.y);
	vec4 H_su0a721 = vCroppedCorner.xyxy * vDeltaUV2;
	vec4 vSeq0LocalCornerUVs = H_2wsz46 + H_su0a721;
	vec2 H_3x9uv2 = vCroppedCorner * C_vec2p2d0p;
	vec2 H_hu77kt = C_vec2p1d0p - H_3x9uv2;
	vec2 H_zmh5yj1 = H_hu77kt * VS_INPUT_gl_m_vCornerID;
	...
	vec4 H_pack81 = vec4(H_bhjmxk, H_5r83t6, C_1d0, C_0d0);
	...
	float select4 = H_woa5qi1 ? vExpSegment.x : vLinearSegment.x;
	bool H_zk280s = VS_INPUT_gl_m_vTint.y > C_0d04045;
	float select5 = H_zk280s ? vExpSegment.y : vLinearSegment.y;
	bool H_2iabxr = VS_INPUT_gl_m_vTint.z > C_0d04045;
	float select6 = H_2iabxr ? vExpSegment.z : vLinearSegment.z;
	vec4 H_gogsay1 = vec4(select4, select5, select6, VS_INPUT_gl_m_vTint.w);
	vec4 vecTint = mix(VS_INPUT_gl_m_vTint, H_gogsay1, g_vecExtraData2.x);
	...
	float H_4k96le1 = H_zmh5yj1.x + g_vecDepthControlAndRotationCenterData.y;
	vec3 H_ofv5py1 = vec3(H_4k96le1);
	vec3 H_9qb4x81 = H_3cij4j1 * H_ofv5py1;
	float H_0vipoy = H_zmh5yj1.y + g_vecDepthControlAndRotationCenterData.z;
	vec3 H_rlzayn1 = vec3(H_0vipoy);
	vec3 H_q8kzlg = H_f7awl61 * H_rlzayn1;
	vec3 H_i38vv4 = H_9qb4x81 + H_q8kzlg;
	...
	if (H_ab3nes1) {
		...
		vec4 select7 = H_0mhdov1 ? C_vec4p0d0p : H_l9aejv1;
		vec4 select8 = H_0fy6si ? select7 : vecTint;
		float param9 = flDistanceFromEyeToParticle * g_vecRadiusControls.x;
		float parama = flDistanceFromEyeToParticle * g_vecRadiusControls.w;
		float H_ufnzi31 = clamp(VS_INPUT_gl_m_vRotationAndRadiusData.w, param9, parama);
		param = select8;
		param1 = H_ufnzi31;
	}
	bool H_gud1601 = param.w < C_0d00392157;
	float paramb = H_gud1601 ? C_0d0 : param1;
	vec3 H_ymty5t = vec3(paramb);
	float flAlphaAdjustment = VS_INPUT_gl_m_vRotationAndRadiusData.w / paramb;
	float vecTint1 = mix(C_1d0, flAlphaAdjustment, g_vecOrientationFadeControlData.w);
	float H_771o8l = param.w * vecTint1;
	vec3 H_ru4krx1 = H_i38vv4 * H_ymty5t;
	vec3 vecWorldPos = H_ru4krx1 + VS_INPUT_gl_m_vPos;
	...
	vec3 paramc = H_b734ov1 + vecWorldPos;
	vec4 H_44eiz91 = vec4(paramc.x, paramc.y, paramc.z, C_1d0);
	vec4 H_u4dt8e1 = H_44eiz91 + g_vWorldToCameraOffsetMultiview[0];
	float dotres7 = dot(H_u4dt8e1, g_matWorldToProjectionMultiview[0][2]);
	float dotres8 = dot(H_u4dt8e1, g_matWorldToProjectionMultiview[0][3]);
	float H_nbz4rl1 = dotres7 / dotres8;
	float flDesiredZ = H_nbz4rl1 * dotres6;
	float misc2a2 = min(C_0d001, dotres5);
	float misc2a3 = max(flDesiredZ, misc2a2);
	...
	PS_INPUT_gl_m_vAnimationBlendValues = H_pack81;
	PS_INPUT_gl_m_vTexCoordsSeq1 = H_kvqmst1;
	float H_rdf2pt1 = C_0d0 - dotres4;
	float H_bm580d1 = C_2d0 * misc2a3;
	float H_bdslpo1 = H_bm580d1 - dotres6;
	vec4 H_38xb1j1 = vec4(dotres3, H_rdf2pt1, H_bdslpo1, dotres6);
	gl_Position = H_38xb1j1;
