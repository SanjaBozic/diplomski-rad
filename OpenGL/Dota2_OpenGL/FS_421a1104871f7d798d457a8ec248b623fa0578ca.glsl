...
void main()
{
	...
	float vVectorTs = dot(H_i1nbjv1, vTangentUWs);
	float vVectorTs1 = dot(H_i1nbjv1, vTangentVWs2);
	vec4 txt = texture(g_tParallaxMask, PS_INPUT_gl_vBaseTexCoord0_vBaseTexCoord1.zw);
	vec2 H_nfhxzf = vec2(g_flParallaxStrength1);
	vec2 H_5kzfth = vec2(vVectorTs, vVectorTs1);
	vec2 H_ip7zxn = H_5kzfth * txt.yy;
	vec2 vParallaxVector = H_ip7zxn * H_nfhxzf;
	...
	vec2 H_tkymyh1 = H_di5dis + PS_INPUT_gl_vBaseTexCoord0_vBaseTexCoord1.zw;
	vec4 txt1 = texture(g_tParallaxMask, PS_INPUT_gl_vBaseTexCoord2_vBaseTexCoord3.zw);
	vec2 H_pj5kui1 = vec2(g_flParallaxStrength3);
	vec2 H_z14lr1 = H_5kzfth * txt1.ww;
	vec2 vParallaxVector3 = H_pj5kui1 * H_z14lr1;
	float vParallaxVector4 = dot(g_vTexCoordTransform3U.xy, vParallaxVector3);
	float vParallaxVector5 = dot(g_vTexCoordTransform3V.xy, vParallaxVector3);
	vec2 H_jlvna1 = vec2(vParallaxVector4, vParallaxVector5);
	vec2 H_zdw8il = H_jlvna1 + PS_INPUT_gl_vBaseTexCoord2_vBaseTexCoord3.zw;
	vec4 vDiffuse = texture(g_tColor0, PS_INPUT_gl_vBaseTexCoord0_vBaseTexCoord1.xy);
	vec4 vDiffuse1 = texture(g_tColor1, H_tkymyh1);
	vec4 vDiffuse2 = texture(g_tColor2, PS_INPUT_gl_vBaseTexCoord2_vBaseTexCoord3.xy);
	vec4 vDiffuse3 = texture(g_tColor3, H_zdw8il);
	vec4 vTintMasks = texture(g_tTintMasks, PS_INPUT_gl_vBaseTexCoord0_vBaseTexCoord1.xy);
	vec4 vTintMasks1 = texture(g_tTintMasks, H_tkymyh1);
	vec4 vTintMasks2 = texture(g_tTintMasks, PS_INPUT_gl_vBaseTexCoord2_vBaseTexCoord3.xy);
	vec4 vTintMasks3 = texture(g_tTintMasks, H_zdw8il);
	...
	vec4 vSpecular1 = texture(g_tSpecular1, H_tkymyh1);
	vec4 vSpecular2 = texture(g_tSpecular2, PS_INPUT_gl_vBaseTexCoord2_vBaseTexCoord3.xy);
	vec4 vSpecular3 = texture(g_tSpecular3, H_zdw8il);
	...
	vec4 vNormalTexel1 = texture(g_tNormal1, H_tkymyh1);
	vec4 vNormalTexel2 = texture(g_tNormal2, PS_INPUT_gl_vBaseTexCoord2_vBaseTexCoord3.xy);
	vec4 vNormalTexel3 = texture(g_tNormal3, H_zdw8il);
