#version 460 core
// ps_5_0
// Checksum: 9e7a41ec_c02b7cb7_9d88ae9a_cff4ba91
// Name: charactereyegloss_indoor_fade
...
void main()
{
	...
	r1.y = fract(r1.x);
	r0.w = fma(-r0.w, idx_uniforms1_ps.cb1[1].w, float(1.00000000f));
	r0.w = -r0.w + r1.y;
	...
	if (floatBitsToUint(idx_uniforms4_ps.cb4[3].x) != uint(0))
	{
		...
		r2.x = intBitsToFloat(float(4.94999981f) >= r0.w ? int(0xffffffff) : int(0x00000000));
		if (floatBitsToUint(r2.x) != uint(0))
		{
			...
			r2.x = saturate(r2.x);
			r0.w = dot(v0.xy, vec2(0.834930003f, -0.629559994f));
			r4.xw = fract(r1.xx);
			r4.yz = fract(r0.ww);
	...
	r1.x = ((texture(resourceSamplerPair_2_ps, r2.xy)).wxyz).x;
	r1.x = saturate(r1.x + idx_uniforms2_ps.cb2[0].w);
	r0.w = r0.w * r1.x;
	...
	r1.x = r1.x + r1.x;
	r4.xyz = fma(r1.yzw, -r1.xxx, -r2.xyz);
	...
	r1.x = saturate(dot(r4.xyz, vec3(0.212500006f, 0.715399981f, 0.0720999986f)));
	r1.x = log2(r1.x);
	r1.x = r1.x * idx_uniforms2_ps.cb2[1].w;
	r1.x = exp2(r1.x);
	r1.x = r1.x * idx_uniforms2_ps.cb2[1].z;
