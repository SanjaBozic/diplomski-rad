			...
			{
				{
					...
					float slot0ColorMask = inputs.colorMask.x;
					float slot1ColorMask = inputs.colorMask.z;
					float bloomColorMask = inputs.colorMask.y;
					customAlbedo = mix( customAlbedo, _fa_freqHigh[8 ].xyz, slot0ColorMask * _fa_freqHigh[8 ].w );
					customAlbedo = mix( customAlbedo, _fa_freqHigh[9 ].xyz, slot1ColorMask * _fa_freqHigh[9 ].w );
					inputs.albedo = customAlbedo;
					inputs.emissive = vec3( bloomColorMask * _fa_freqHigh[10 ].w ) * _fa_freqHigh[10 ].xyz;
				};
			};
			{
