...
layout( binding = 0, r11f_g11f_b10f ) uniform image2D ib_uavdeferredprobes ;
		...
		{
			{
				...
				resultColor = vec4( 0, 0, 0, 0 ); if( dot( spec.xyz, vec3( 1 ) ) > 0 && depth < 1.0 ) {
					...
					vec3 frustumVecX0 = mix( _ca_freqLow[0 ].xyz, _ca_freqLow[1 ].xyz, tc.x * _ca_freqLow[2 ].z);
					vec3 frustumVecX1 = mix( _ca_freqLow[3 ].xyz, _ca_freqLow[4 ].xyz, tc.x * _ca_freqLow[2 ].z);
					vec3 frustumVec = mix( frustumVecX1.xyz, frustumVecX0.xyz, tc.y* _ca_freqLow[2 ].w);
					inputs.albedo = vec3( 1 );
					inputs.fragCoord.xy = vec2( pixelLocation.xy.xy );
					vec4 wPosM = vec4( tc.xy, depth, 1.0 );
					float zLinear = GetLinearDepth( depth, _ca_freqLow[5 ], 1.0, false );
					vec3 world_pos = _ca_freqLow[6 ].xyz + frustumVec.xyz * zLinear;
					inputs.fragCoord.z = depth;
					inputs.position = world_pos.xyz;
					inputs.view = normalize( _ca_freqLow[6 ].xyz - world_pos.xyz );
					...
					{
						uint skipStaticModel = _ca_freqHigh[1 ].x > 0 ? ( 1 << 3 ) : 0;
						uint clusterOffset;
						{
							{
								...
								clusterCoordinate.xy = screenPosToTexcoord( inputs.fragCoord.xy, _ca_freqHigh[2 ] ) * _ca_freqLow[2 ].zw;
								clusterCoordinate.xy = floor( clusterCoordinate.xy * vec2( NUM_CLUSTERS_X, NUM_CLUSTERS_Y ) );
								float curr_z = GetLinearDepth( inputs.fragCoord.z, _ca_freqLow[5 ], 1.0, false );
								float slice = log2( max( 1.0, curr_z / _ca_freqLow[7 ].z ) ) * _ca_freqLow[7 ].w;
						...
						{
							{
								...
								ambient = mix( GetLuma( ambient.xyz ).xxx, ambient.xyz, _ca_freqLow[8 ].www );
								...
								for ( uint probeIdx = probesMin; probeIdx < probesMax; ) {
									...
									{ light_t lightParms;
										...
										light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _ca_freqLow[8 ].www );
										{
											...
											diffEnvProbe = mix( GetLuma( diffEnvProbe.xyz ).xxx, diffEnvProbe.xyz, _ca_freqLow[8 ].www );
											specEnvProbe = mix( GetLuma( specEnvProbe.xyz ).xxx, specEnvProbe.xyz, _ca_freqLow[8 ].www );
					...
					resultColor.rgb = inputs.specular_lighting.xyz;
				}
			}
		};
		imageStore( ib_uavdeferredprobes, pixelLocation.xy, resultColor );
	}
}