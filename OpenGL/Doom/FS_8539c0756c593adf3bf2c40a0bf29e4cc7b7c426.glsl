...
uniform sampler2D samp_distortionlastframemap;
...
void main() {
	{
		...
		float scale = 1.0 - saturate( ( srcDepth - dstDepth ) / _fa_freqHigh[1 ].x );
		vec4 final = tex2D( samp_transmap, vofi_TexCoord0.xy * _fa_freqHigh[2 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[2 ].zw );
		vec4 distortion = tex2D( samp_distortionlastframemap, screenTC ) - 0.5;
		vec2 perturb = distortion.xy * 0.1;
		vec4 lastFrame = tex2Dlod( samp_viewcolorlastframemap, vec4( screenTC + perturb, 0.0, 1.0 ) );
		final.xyz = lastFrame.xyz;
		final *= vofi_Color;
		final.w *= scale;
