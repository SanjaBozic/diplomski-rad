	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				{
					float woundMask = saturate( ( ( 1.0 - saturate( _fa_freqHigh[12 ].x > 0.0 ? vofi_TexCoord6.x : woundBloodBlend.x * 0.5 ) ) - ( 1.0 - woundMap.w ) ) * 3.0 );
					...
					inputs.normal.xy += mix( woundMap.xy * vec2( woundMask ), vec2( 0.0 ), vec2( _fa_freqHigh[14 ].w ) );
				};
			};
			{
			...
			} ;
			{
				...
				{ if ( _fa_freqHigh[24 ].w > 0.0 ) {
						...
						noise = DeGamma( noise );
						float fade = 1.0 - _fa_freqHigh[24 ].w * _fa_freqHigh[24 ].w;
						inputs.diffuse_lighting.xyz.xyz.xyz += mix( inputs.diffuse_lighting.xyz.xyz.xyz, _fa_freqHigh[24 ].xyz * noise.z * noise.z * 20.0, _fa_freqHigh[24 ].w );
					}
				};
				inputs.diffuse_lighting.xyz.xyz += vec3( _fa_freqHigh[8 ].x * inputs.lightmap.x * inputs.lightmap.x ) * _fa_freqHigh[11 ].xyz;
				{ if ( _fa_freqHigh[25 ].x < 1.0 ) {
						vec2 texcoord = gl_FragCoord.xy * ( 0.00390625 );
						float dither = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * _fa_freqHigh[26 ].xy, 0.0, 0.0 ) ).x;
						dither = saturate( abs( clamp( _fa_freqHigh[25 ].x - dither * 0.5 - 0.1, -1.0, 0.0 ) ) );
						dither *= 600.0;
