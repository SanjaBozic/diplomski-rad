...
uniform sampler2D samp_ribbonmask;
...
void main() {
	{
		...
		vec4 final = tex2DDeGamma( samp_transmap, vofi_TexCoord0.xy * _fa_freqHigh[0 ].xy + vec2( myTime ) * _fa_freqHigh[0 ].zw );
		final *= vofi_Color;
		final.xyz *= DeGamma( _fa_freqHigh[1 ].xyz )* vec3( _fa_freqHigh[2 ].x );
		final *= tex2DDeGamma( samp_ribbonmask, vofi_TexCoord0.xy );
		final.w = saturate( final.w );
		float fog = saturate( exp2( vofi_TexCoord0.w ) );
