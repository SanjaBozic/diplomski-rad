...
void main() {
	{
		...
		vofi_TexCoord1.xy = in_Normal.xy;
		vec3 viewDir = gl_Position.xyz - _va_freqLow[0 ].xyz;
		float viewDistance = length( viewDir );
		vofi_TexCoord0 = vec4( in_TexCoord.x, in_TexCoord.y, viewDistance, in_Position.y );
		vofi_Color = ( _va_freqHigh[4 ].x > 0.0 ) ? DeGamma( in_Color ) : in_Color;
	}
}