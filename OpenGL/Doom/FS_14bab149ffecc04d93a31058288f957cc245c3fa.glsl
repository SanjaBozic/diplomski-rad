	...
	{
		{
			...
			} ;
			{
				...
				{ if ( _fa_freqHigh[23 ].w > 0 ) {
						...
						vec3 finalPulseFresnel = saturate( digitalNoise.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[23 ].xyz;
						vec3 finalPulseFlat = saturate( digitalNoise.x ) * _fa_freqHigh[23 ].xyz;
						vec3 finalPulse = mix( finalPulseFresnel, finalPulseFlat, vec3( 0.25 ) );
						vec3 constantGlow = saturate( 0.5 * ( fresnel ) + edgeGlow ) * _fa_freqHigh[23 ].xyz;
						finalPulse /= _fa_freqHigh[22 ].xyz;
						vec3 finalPulseColor = finalPulse * _fa_freqHigh[23 ].w;
				...
				}; if ( _fa_freqHigh[24 ].x > 0.0 ) {
					float teamfxID = _fa_freqHigh[25 ].x == 1.0 ? 0.0 : 1.0;
					...
					vec3 viewDir = normalize( -vofi_TexCoord4.xyz );
					{
						...
						float fadeStart = _fa_freqHigh[26 ].x;
						float fadeEnd = _fa_freqHigh[27 ].x;
						float fadeRange = max( 1.0, fadeEnd - fadeStart );
						float value = max( 0.0, distance - fadeStart );
						float alpha = saturate( value / fadeRange );
						fresnel = saturate( pow( fresnel, _fa_freqHigh[28 ].x ) * _fa_freqHigh[29 ].y );
						vec3 rimLightColor = ( 1.0 * _fa_freqHigh[29 ].x ) * _fa_freqHigh[30 ].xyz;
						vec3 finalRimlight = fresnel * rimLightColor;
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz + finalRimlight, alpha * teamfxID * _fa_freqHigh[31 ].x );
					};
				}
				{ if ( _fa_freqHigh[17 ].x > 0.0 ) {
						vec2 resolution = abs( vec2( 1.0 ) / _fa_freqHigh[18 ].zw );
						...
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						float pulseTime = mix( 10.0, 30.0, _fa_freqHigh[17 ].y );
						...
						finalPulse = saturate( dither.x * ( fresnel + pulse ) + edgeGlow ) * _fa_freqHigh[21 ].xyz * 2.0;
						inputs.diffuse_lighting.xyz.xyz += finalPulse;
					}
				};
				{ if ( _fa_freqHigh[32 ].x > 0.0 ) {
						vec4 colorWave = tex2DlodDeGamma( samp_transmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[33 ].xy + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[33 ].zw, 0.0, 0.0) );
						...
						vec3 close = crevices * crevices * vec3( 180.0 ) * _fa_freqHigh[34 ].xyz + finalOutline * _fa_freqHigh[34 ].xyz;
						vec3 far = _fa_freqHigh[34 ].xyz * vec3( 0.1 ) * colorWave.xyz * vec3( fresnel2 ) + vec3( fresnel2 ) * _fa_freqHigh[34 ].xyz * vec3( 20.0 );
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz, ( inputs.diffuse_lighting.xyz + mix( close, far, vec3( farFade ) ) * colorWave.xyz ) + _fa_freqHigh[34 ].xyz * vec3( 0.01 ) * vec3( fresnel2 ), vec3( _fa_freqHigh[32 ].x ) );
					}
				};
				{ if ( _fa_freqHigh[35 ].x == 1.0 ) { if ( _fa_freqHigh[36 ].x == 1.0 ) {
							vec4 shieldPattern = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[33 ].xy ) * 3.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[29 ].zw, 0.0, 0.0 ) );
							vec4 shieldPattern2 = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[33 ].xy ) * 2.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[33 ].zw, 0.0, 0.0 ) );
							...
							inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 1.0, 0.25 ) * vec3( 50.0 ), _fa_freqHigh[36 ].w );
						}
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.5, 1.0, 0.5 ) * vec3( 50.0 ), _fa_freqHigh[36 ].y );
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 0.0, 0.0 ) * vec3( 50.0 ), _fa_freqHigh[36 ].z );
					}
				};
				{ if ( _fa_freqHigh[37 ].x > 0.0 ) {
						...
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, holoColor.xyz, _fa_freqHigh[37 ].x );
						inputs.diffuse_lighting.xyz.xyz = mix (inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz * noise.xyz * 2.3, _fa_freqHigh[37 ].x);
					}
				};
			};
			{
