...
void main() {
	{
		...
		vec2 tcWorld = vofi_TexCoord5.xz;
		vec2 tcPulse = vofi_TexCoord5.xz;
		vec2 tcMainSurface = vofi_TexCoord5.xz;
		...
		vec2 mainSurfaceTC = tcMainSurface * _fa_freqHigh[5 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[5 ].z;
		...
		vec3 pulse = vec3( tex2D( samp_transmap, edgePulseTC ) * _fa_freqHigh[5 ].w );
		vec3 mainSurface = vec3( tex2D( samp_transmap2, mainSurfaceTC ) * _fa_freqHigh[5 ].w );
		vec4 fresnelControls = _fa_freqHigh[6 ];
		vec4 colBlue = vec4(0.1, 0.2, 1.0, 0.5);
		vec4 colRed = vec4(1.0, 0.2, 0.1, 0.5);
		vec4 colDefault = _fa_freqHigh[7 ];
		vec4 boundaryColor = mix(mix( colDefault, colBlue, clamp( _fa_freqHigh[8 ].x, 0.0, 1.0) ) , colRed, clamp(( _fa_freqHigh[8 ].x - 1), 0.0, 1.0) ) ;
		...
		float fresnel = pow( mix(saturate( abs( ndotv ) ),1- saturate( abs( ndotv ) ),fresnelControls.x) , _fa_freqHigh[9 ].x * fresnelControls.y);
