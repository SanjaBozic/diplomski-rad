...
void main() {
	{
		...
		vec4 posWorldSpace;
		posWorldSpace.x = dot4( position, _va_freqHigh[4 ] );
		posWorldSpace.y = dot4( position, _va_freqHigh[5 ] );
		posWorldSpace.z = dot4( position, _va_freqHigh[6 ] );
		posWorldSpace.w = dot4( position, _va_freqHigh[7 ] );
		vofi_TexCoord0 = posWorldSpace - _va_freqLow[2 ];
	}
}