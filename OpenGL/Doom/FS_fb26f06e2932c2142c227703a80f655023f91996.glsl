...
void main() {
	{
		vec2 tc = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[4 ] );
		{
			...
			vec3 cMax = vec3( 0 );
			vec3 cMin = vec3( 0 );
			vec3 M = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ), 0 ).xyz );
			float transparency_k = saturate( ( length( M.xyz - toneMap( tex2DFetch( samp_tex3, ivec2( gl_FragCoord.xy ), 0 ).xyz ) ) ) * _fa_freqHigh[5 ].z );
			{
				vec3 ML = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( -1, 0 ), 0 ).xyz );
				vec3 MR = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( 1, 0 ), 0 ).xyz );
				vec3 T = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( 0, -1 ), 0 ).xyz );
				vec3 B = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( 0, 1 ), 0 ).xyz );
				...
				curr_frame = ( ( ( ( T * _fa_freqHigh[6 ].y ) + ML * _fa_freqHigh[6 ].w ) + M * _fa_freqHigh[7 ].x ) + MR * _fa_freqHigh[7 ].y ) + B * _fa_freqHigh[7 ].w;
				curr_frame_lowpass = ( ( ( ( T * _fa_freqHigh[8 ].y ) + ML * _fa_freqHigh[8 ].w ) + M * _fa_freqHigh[9 ].x ) + MR * _fa_freqHigh[9 ].y ) + B * _fa_freqHigh[9 ].w;
			}
			{
				vec3 TL = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( -1, -1 ), 0 ).xyz );
				vec3 TR = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( 1, -1 ), 0 ).xyz );
				vec3 BL = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( -1, 1 ), 0 ).xyz );
				vec3 BR = toneMap( tex2DFetch( samp_tex0, ivec2( gl_FragCoord.xy ) + ivec2( 1, 1 ), 0 ).xyz );
				...
				curr_frame += ( ( ( TL * _fa_freqHigh[6 ].x ) + TR * _fa_freqHigh[6 ].z ) + BL * _fa_freqHigh[7 ].z ) + BR * _fa_freqHigh[10 ].x;
				curr_frame_lowpass += ( ( ( TL * _fa_freqHigh[8 ].x ) + TR * _fa_freqHigh[8 ].z ) + BL * _fa_freqHigh[9 ].z ) + BR * _fa_freqHigh[11 ].x;
			}
			...
			{
				vec2 tc_prev = tc * _fa_freqHigh[12 ].xy;
				vec2 uv = ( tc_prev.xy - velocity.xy) * _fa_freqHigh[0 ].xy;
				...
				vec2 wm = ( w1 + w2 );
				vec2 fuvM = w2 / ( w1 + w2 );
				vec2 t0 = ( iuv - 1 ) * _fa_freqHigh[0 ].zw;
				vec2 t1 = ( iuv + fuvM ) * _fa_freqHigh[0 ].zw;
				vec2 t2 = ( iuv + 2 ) * _fa_freqHigh[0 ].zw;
			...
			float velocity_k = saturate( length( velocity.xy * _fa_freqHigh[0 ].xy ) ) ;
			float k = prev_frame_edges * ( 1.0 + 5.0 * velocity_k ) / 8.0;
			float w = saturate( transparency_k + k / ( k + curr_frame_edges ) );
			out_FragColor0.xyz = max( vec3( 0 ), toneMapInverse( mix( prev_frame.xyz, curr_frame.xyz, vec3( saturate( w ) ) ) ) );
			out_FragColor1.xy = OutputVelocityTarget( _fa_freqHigh[13 ], velocity.xy , velocity.z ).zw;
