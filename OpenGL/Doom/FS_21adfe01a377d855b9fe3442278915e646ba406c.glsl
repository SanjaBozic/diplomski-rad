...
void main() {
	{
		...
		float worldNoiseMap = tex2D( samp_transmap2, vofi_TexCoord1.xy * 0.002 + ( _fa_freqLow[0 ].x * 0.025 ) + ( vofi_TexCoord1.z * 0.002 ) ).x;
		vec2 rampCoords = tex2D( samp_transmap, vofi_TexCoord0.xy ).xy;
		vec3 finalColor = tex2D( samp_transmap1, rampCoords * saturate( ( worldNoiseMap + 0.1 ) ) ).xyz;
		out_FragColor0.xyz = finalColor.y * vofi_Color.xyz;
