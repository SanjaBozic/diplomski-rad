...
void main() {
	{
		...
		vec3 dir = vec3( 0.0 );
		dir.x = sin( _va_freqLow[4 ].x * 5.7 + in_Position.y * 0.5 ) * _va_freqHigh[8 ].x;
		dir.y = sin( _va_freqLow[4 ].x * 7.1 + in_Position.z * 0.63 ) * _va_freqHigh[8 ].x;
		dir.z = sin( _va_freqLow[4 ].x * 9.3 + in_Position.x * 0.77 ) * _va_freqHigh[8 ].x;
		dir = normalize( dir );
