...
void main() {
	{
		...
		final.w *= mix(1.0, scale, _fa_freqHigh[2 ].x);
		final.xyz *= vofi_Color.xyz;
		...
		final.xyz = mix(final.xyz, mix( _fa_freqLow[0 ].xyz * vec3( _fa_freqLow[1 ].x ), final.xyz, fogVal), _fa_freqHigh[3 ].x );
		final.xyz = mix( final.xyz * 5.0, final.xyz * _fa_freqLow[2 ].x, ( 1.0 - _fa_freqHigh[4 ].x ) );
