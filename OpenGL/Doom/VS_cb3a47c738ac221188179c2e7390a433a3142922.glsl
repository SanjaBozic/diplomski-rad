...
void main() {
	{
		...
		vec4 localPosition;
		localPosition.x = dot4( position, vec4( _va_freqHigh[0 ].xyz, 0.0 ) );
		localPosition.y = dot4( position, vec4( _va_freqHigh[1 ].xyz, 0.0 ) );
		localPosition.z = dot4( position, vec4( _va_freqHigh[2 ].xyz, 0.0 ) );
		localPosition.w = dot4( position, vec4( _va_freqHigh[3 ].xyz, 1.0 ) );
		vec4 worldPos;
		worldPos.xyz = _va_freqLow[2 ].xyz + localPosition.xyz;
		worldPos.w = 1.0;
		...
		vofi_TexCoord0.z = saturate( exp2( -( max( gl_Position.w - _va_freqLow[6 ].x, 0.0 ) * _va_freqLow[6 ].z * _va_freqHigh[4 ].x ) ) );
