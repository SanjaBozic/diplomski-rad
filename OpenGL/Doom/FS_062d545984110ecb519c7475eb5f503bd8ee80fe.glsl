	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				inputs.alpha = saturate( _fa_freqHigh[1 ].x + sampleAlbedo.w );
			};
			{
				...
				{
					{
						...
						inputs.normal = GetWorldSpaceNormal( inputs.normal, inputs.invTS, isFrontFacing );
						{
							...
							decalsMin = ( skipStaticModel == 0 && _fa_freqHigh[2 ].y == 0.0 ) ? decalsMax : decalsMin;
			...
			} ;
			{
				...
				inputs.output_lighting.xyz.xyz = SRGBlinearApprox( _fa_freqHigh[4 ].xyz ) * lumMask * inputs.albedo.xyz;
				...
				float fogAmount;
				{
					...
					vec3 heightFogColor = mix( fogLightInscatterScaledColor, _fa_freqLow[10 ].xyz, vofi_TexCoord6.z );
					...
					fogColor = mix( _fa_freqLow[11 ].xyz, heightFogColor, fogSelection );
					fogAmount = 1.0 - fogAmount;
				};
				inputs.output_lighting.xyz.xyz = mix( fogColor * vec3( _fa_freqLow[12 ].x ), inputs.output_lighting.xyz.xyz, fogAmount );
			};
			out_FragColor0 = vec4( inputs.output_lighting.xyz, inputs.alpha );
			out_FragColor0.xyz += _fa_freqLow[13 ].xyz;
		};
		out_FragColor0.rgb = min( out_FragColor0.rgb, vec3( 500 ) );
		{
