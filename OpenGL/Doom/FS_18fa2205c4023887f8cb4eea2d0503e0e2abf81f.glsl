	...
	{
		{
				...
				{
					{
						...
						vec3 ambientSSS = vec3( 0 );
						{
							...
							ambientSSS.xyz += max ( indirectDiffuse.xyz * vec3( 1 ) * saturate( _fa_freqLow[10 ].x ), vec3( 0 ) );
						};
						...
						for ( uint lightIdx = lightsMin; lightIdx < lightsMax; ) {
							...
							{ light_t lightParms;
								...
								light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _fa_freqLow[11 ].www );
								{
									...
									vec3 normalR = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[13 ].xxx );
									vec3 normalG = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[13 ].yyy );
									vec3 normalB = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[13 ].zzz );
									vec3 bssrdf = saturate( vec3( dot( normalR, light_vector ), dot( normalG, light_vector ), dot( normalB, light_vector ) ) );
									bssrdf += saturate( _fa_freqHigh[14 ].xyz * ( pow( saturate( dot( inputs.view.xyz, -light_vector.xyz ) ), _fa_freqHigh[14 ].w ) ) * inputs.thickness );
			...
			{
				{ if ( _fa_freqHigh[15 ].w > 0 ) {
						vec4 tiling = vec4( 0.003, 0.01, 0.05, 0.3 );
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[16 ].zw );
						...
						vec3 finalPulseFresnel = saturate( digitalNoise.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[15 ].xyz;
						vec3 finalPulseFlat = saturate( digitalNoise.x ) * _fa_freqHigh[15 ].xyz;
						vec3 finalPulse = mix( finalPulseFresnel, finalPulseFlat, vec3( 0.25 ) );
						vec3 constantGlow = saturate( 0.5 * ( fresnel ) + edgeGlow ) * _fa_freqHigh[15 ].xyz;
						finalPulse /= _fa_freqHigh[17 ].xyz;
						vec3 finalPulseColor = finalPulse * _fa_freqHigh[15 ].w;
				...
				{ if ( _fa_freqHigh[18 ].w > 0 ) {
						...
						inputs.diffuse_lighting.xyz += dither.xxx * vec3( nDotSomething + 0.01 ) * _fa_freqHigh[18 ].xyz * vec3( _fa_freqHigh[18 ].w * 2.0 ) * 2.0;
					}
				};
				{ if ( _fa_freqHigh[19 ].x > 0 ) {
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[16 ].zw );
						vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy*1.5, _fa_freqHigh[3 ] ) * ( resolution / 32.0 );
						vec4 dither = tex2Dlod( samp_echodithermap, vec4( screenTexCoord * vec2( 0.7 ), 0.0, 0.0 ) );
						float fresnel = 1.0 - dot( inputs.normal, normalize( -vofi_TexCoord4.xyz ) );
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[20 ].x; if ( _fa_freqHigh[19 ].y >= _fa_freqHigh[20 ].z ) {
							pulsefrequency = _fa_freqHigh[20 ].y;
						}
						float width = mix( _fa_freqHigh[21 ].x, _fa_freqHigh[21 ].y, _fa_freqHigh[19 ].y );
						float zValue = ( vofi_TexCoord4.xyz.z ) * width;
						float pulse = sin( zValue + _fa_freqLow[14 ].x * pulsefrequency ) * magnitude + magnitude;
						float power = _fa_freqHigh[21 ].z;
						pulse = pow( pulse, power );
						pulse *= fresnel;
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[22 ].xyz * 2.0;
						finalPulse /= _fa_freqHigh[17 ].xyz;
				...
				{ if ( _fa_freqHigh[23 ].w > 0.0 ) {
						...
						inputs.diffuse_lighting.xyz += ( crevices + finalOutline * vec3( 15.0 ) ) * _fa_freqHigh[23 ].xyz;
					}
				};
				{ if ( _fa_freqHigh[24 ].w > 0.0 ) {
						vec2 resolution = abs( vec2( 1.0 ) / _fa_freqHigh[16 ].zw );
						vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[3 ] ) * ( resolution / vec2( 32.0 ) );
						vec4 colorWave = tex2DlodDeGamma( samp_transmap, vec4( screenTexCoord * _fa_freqHigh[25 ].xy + vec2( _fa_freqLow[14 ].x ) * _fa_freqHigh[25 ].zw, 0.0, 0.0) );
						...
						vec3 finalOutline = vec3( fresnel );
						vec3 close = crevices * crevices * vec3( 180.0 ) * _fa_freqHigh[24 ].xyz + finalOutline * _fa_freqHigh[24 ].xyz;
						vec3 far = _fa_freqHigh[24 ].xyz * vec3( 0.1 ) * colorWave.xyz * vec3( fresnel2 ) + vec3( fresnel2 ) * _fa_freqHigh[24 ].xyz * vec3( 20.0 );
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz, ( inputs.diffuse_lighting.xyz + mix( close, far, vec3( farFade ) ) * colorWave.xyz ) + _fa_freqHigh[24 ].xyz * vec3( 0.1 ) * vec3( fresnel2 ), vec3( _fa_freqHigh[24 ].w ) );
					}
				};
				{ if ( _fa_freqHigh[26 ].x > 0 ) {
						float clampedGunGlow = saturate( _fa_freqHigh[26 ].x * _fa_freqHigh[26 ].x );
						vec3 glowColor = mix( _fa_freqHigh[10 ].xyz, _fa_freqHigh[27 ].xyz, clampedGunGlow );
						float glowScale = mix( 1.0, _fa_freqHigh[8 ].x, clampedGunGlow );
						inputs.diffuse_lighting.xyz.xyz += clampedGunGlow * sampleLightmap.x * glowScale * glowColor;
					}
				};
				inputs.diffuse_lighting.xyz.xyz += vec3( _fa_freqHigh[8 ].x * inputs.lightmap.x * inputs.lightmap.x ) * _fa_freqHigh[11 ].xyz;
				{ if ( _fa_freqHigh[28 ].x < 1.0 ) {
						vec2 texcoord = gl_FragCoord.xy * ( 0.00390625 );
						float dither = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * _fa_freqHigh[29 ].xy, 0.0, 0.0 ) ).x;
						dither = saturate( abs( clamp( _fa_freqHigh[28 ].x - dither * 0.5 - 0.1, -1.0, 0.0 ) ) );
						dither *= 600.0;
