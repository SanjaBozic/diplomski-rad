...
void main() {
	{
		...
		float wobble = in_Tangent.x * 0.5 + 1.0;
		...
		float myTime = _va_freqLow[3 ].x * ( in_Tangent.z * 0.3 + 1.0 + wobble * 5.0 ) + in_Tangent.z * 73.0;
		...
		float fade = size / _va_freqHigh[0 ].x;
		vofi_Color = vertexColor;
		vofi_Color.xyz *= _va_freqHigh[7 ].xyz;
		vofi_Color.w *= _va_freqHigh[7 ].w;
		vofi_Color.w *= fade;
	}
}