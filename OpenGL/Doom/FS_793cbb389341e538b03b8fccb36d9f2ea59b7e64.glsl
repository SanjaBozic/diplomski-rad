	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				vec2 tc_scroll = vofi_TexCoord0.xy * _fa_freqHigh[8 ].xy + ( _fa_freqHigh[8 ].zw * _fa_freqLow[4 ].x );
				vec2 tcN = (vofi_TexCoord0.xy * _fa_freqHigh[9 ].xy) + ( _fa_freqHigh[10 ].xy * _fa_freqLow[4 ].x );
				vec2 tcN2 = (vofi_TexCoord0.xy * _fa_freqHigh[9 ].zw) + ( _fa_freqHigh[10 ].zw * _fa_freqLow[4 ].x );
				vec4 normal1 = tex2D( samp_sparebumpmap, tcN);
				vec4 normal2 = tex2D( samp_sparebumpmap, tcN2);
				float texNoise = (normal1.x + normal2.x) ;
				vec2 tcS = (tc * _fa_freqHigh[8 ].xy + ( _fa_freqHigh[10 ].zw * _fa_freqLow[4 ].x ))+ ( texNoise.xx * ( _fa_freqHigh[8 ].zw + 0.02) );
				vec2 tcSB = (tc * _fa_freqHigh[8 ].xy + ( _fa_freqHigh[10 ].xy * _fa_freqLow[4 ].x ))+ ( texNoise.xx * _fa_freqHigh[8 ].zw );
				vec2 tcBaseAnim = (vofi_TexCoord0.xy * _fa_freqHigh[11 ].xy )+ ( texNoise.xx * _fa_freqHigh[8 ].zw );
				vec2 tcBase = (vofi_TexCoord0.xy * _fa_freqHigh[11 ].xy );
				vec4 baseCombo = tex2D( samp_sparediffusemap2, tcBaseAnim);
				vec4 baseComboStill = tex2D( samp_sparediffusemap2, tcBase);
				vec4 normalBase = tex2D( samp_sparebumpmap2, tcBase);
				vec2 animUVBlend = mix(tcBaseAnim, tcBase, baseCombo.y);
				baseCombo = mix(baseCombo, baseComboStill,baseComboStill.y );
				vec4 baseDiffuse = tex2D( samp_sparediffusemap, mix(animUVBlend, tcBase,baseCombo.y )) ;
				vec4 sampleSpecular1 = tex2D( samp_sparespecularmap, tcS );
				vec4 normal = saturate( normal1 + normal2 - 0.5 ) ;
				vec4 mixedNormal = saturate(mix( normal , normalBase , baseCombo.y * texNoise.x));
				vec4 sampleSpecular = mix ( vec4( 1.22 ) * 0.15 ,vec4( 2.39 ), sampleSpecular1.x * (1 - baseCombo.y)) ;
				float samplePower = mix ( 0.8, .85, ( 1 - baseCombo.y));
				vec4 bloodDiffuse = mix( _fa_freqHigh[12 ] * (sampleSpecular1.y + 0.1), _fa_freqHigh[7 ] * (sampleSpecular1.x + 0.1), sampleSpecular1.y);
				vec4 mixDiffuse = mix (bloodDiffuse , mix(baseDiffuse * _fa_freqHigh[13 ], baseDiffuse * _fa_freqHigh[14 ], texNoise.x * baseCombo.x), texNoise.x * baseCombo.x );
				vec3 finalAlbedo = mixDiffuse.xyz;
				vec3 finalSpecular = sampleSpecular.xyz * mixDiffuse.x ;
				float finalSmoothness = samplePower ;
				vec3 finalNormal = mixedNormal.xyz;
			...
			{
				uint skipStaticModel = _fa_freqHigh[15 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
