...
layout( binding = 0, r32f ) uniform image2D ib_computetargetdepth ;
...
void main() {
	{
		...
		vec2 tc = screenPosToTexcoord( pixelLocation.xy + vec2( 0.5 ), _ca_freqHigh[0 ] );
		float depth = tex2Dlod( samp_viewdepthmap, vec4( tc.xy * _ca_freqLow[0 ].xy, 0.0, 0.0 ) ).x;
		imageStore( ib_computetargetdepth, pixelLocation.xy, vec4( depth ) );
	}
}