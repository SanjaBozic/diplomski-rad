...
void main() {
	{
		...
		vec2 Dtc = ( vofi_TexCoord0.xy - _fa_freqHigh[2 ].zw ) / _fa_freqHigh[2 ].xy;
		vec2 DtcControl = Dtc * _fa_freqHigh[3 ].xy + vec2( _fa_freqLow[0 ].xy * _fa_freqHigh[3 ].zw );
		vec4 distortionMap = tex2D( samp_transmap2, DtcControl);
		distortionMap.xyzw = vec4( vofi_TexCoord0.xy, vofi_TexCoord1.xy ) + ( distortionMap.xy * ( _fa_freqHigh[4 ].x * 0.001) ).xyxy;
		distortionMap.xyzw = ( distortionMap.xyzw - _fa_freqHigh[2 ].zwzw ) / _fa_freqHigh[2 ].xyxy;
		vec2 borderWidth = (2.0 + _fa_freqHigh[4 ].y) / ( _fa_freqHigh[2 ].xy * _fa_freqLow[1 ].xy);
		distortionMap = clamp(distortionMap, borderWidth.xyxy, 1.0 - borderWidth.xyxy);
		distortionMap.xyzw = distortionMap.xyzw * _fa_freqHigh[2 ].xyxy + _fa_freqHigh[2 ].zwzw;
		vec4 final;
		{
			vec4 result0 = tex2DDeGammaApprox( samp_transmap, distortionMap.xy ) * ( 1.0 - vofi_TexCoord1.z );
			vec4 result1 = tex2DDeGammaApprox( samp_transmap, distortionMap.zw ) * ( vofi_TexCoord1.z );
			final = ( result0 + result1 );
		};
		final.w = final.y;
		final.xyz = final.xxx;
		clip( final.w - 1e-6 );
		vec3 particleColor = _fa_freqHigh[5 ].xyz * _fa_freqHigh[6 ].x;
		final.xyz = vec3( 1.0 ) - ( vec3( 1.0 ) - final.xyz ) * vec3( vofi_TexCoord2.z );
		final.xyz *= vec3( vofi_TexCoord2.x );
		final.xyz *= vofi_Color.xyz;
		final.w *= mix(1.0, scale, _fa_freqHigh[7 ].x);
		final.xyz *= vofi_Color.xyz;
		...
		final.xyz = mix(final.xyz, mix( SRGBlinearApprox( _fa_freqLow[2 ].xyz ), final.xyz, fogVal * fogVal * fogVal ), _fa_freqHigh[4 ].w);
		final.xyz = mix( final.xyz * 5.0, final.xyz * _fa_freqLow[3 ].x, ( 1.0 - _fa_freqHigh[6 ].x ) );
		out_FragColor0.xyz = final.xyz;
		out_FragColor0.w = saturate(final.w);
		{
