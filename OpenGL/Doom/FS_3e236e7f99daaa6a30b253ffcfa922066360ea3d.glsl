...
void main() {
	{
		...
		vec3 color = vofi_Color.xyz; if ( _fa_freqHigh[0 ].y > 0.0 ) {
			vec2 glowTC = vofi_TexCoord0.xy + ( _fa_freqHigh[1 ].xy / _fa_freqHigh[0 ].zw );
			...
			vec4 fxGlowColor = ( _fa_freqHigh[2 ].x > 0.0 ) ? DeGamma( _fa_freqHigh[3 ] ) : _fa_freqHigh[3 ];
			color = mix( fxGlowColor.xyz, color, alpha );
			alpha = max( alpha, glowAlpha * fxGlowColor.a );
		}
		vec3 color2 = vec3( 1.0, 1.0, 0.0 );
		vec3 color1 = vec3( 1.0, 0.1, 0.0 );
		float step = vofi_TexCoord1.y;
		vec3 gradient = mix( color2, color1 , step * 1.5 - 0.85 );
		out_FragColor0.xyz = gradient.xyz;
