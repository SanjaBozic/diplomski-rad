	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				vec4 sampleAlbedo = tex2DDeGammaApprox( samp_sparediffusemap, tc );
				...
				inputs.alpha = saturate( _fa_freqHigh[0 ].x + sampleAlbedo.w );
			};
			{
				...
				{
					{
						...
						clusterCoordinate.xy = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[2 ] ) * _fa_freqLow[1 ].zw;
			...
			{
				float deathFade = 1.0;
				{ if ( _fa_freqHigh[3 ].x < 1.0 ) {
						vec2 texcoord = gl_FragCoord.xy * ( 0.00390625 );
						float dither = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * _fa_freqHigh[4 ].xy, 0.0, 0.0 ) ).x;
						deathFade = _fa_freqHigh[3 ].x - dither * 0.5 - 0.1;
						if ( false ) {
							clip( deathFade );
						}
					}
				...
				inputs.output_lighting.xyz.xyz = SRGBlinearApprox( _fa_freqHigh[5 ].xyz ) * lumMask * inputs.albedo.xyz;
				inputs.output_lighting.xyz.xyz = inputs.output_lighting.xyz.xyz + inputs.specular_lighting.xyz;
				inputs.alpha = max( inputs.alpha, lumMask );
				inputs.output_lighting.xyz.xyz = mix( _fa_freqLow[8 ].xyz * vec3( _fa_freqLow[9 ].x ), inputs.output_lighting.xyz.xyz, fogVal );
				{ if ( _fa_freqHigh[3 ].x < 1.0 ) {
						vec2 texcoord = gl_FragCoord.xy * ( 0.00390625 );
						float dither = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * _fa_freqHigh[4 ].xy, 0.0, 0.0 ) ).x;
						dither = saturate( abs( clamp( _fa_freqHigh[3 ].x - dither * 0.5 - 0.1, -1.0, 0.0 ) ) );
						dither *= 600.0;
						inputs.output_lighting.xyz.xyz += vec3( dither ) * vec3( 1.0, 0.01, 0.0 );
					}
				};
			};
			out_FragColor0 = vec4( inputs.output_lighting.xyz, inputs.alpha );
			out_FragColor0.xyz += _fa_freqLow[10 ].xyz;
		};
		{
