...
void main() {
	{
		...
		vofi_Color = _va_freqHigh[7 ] * _va_freqHigh[8 ].x * _va_freqHigh[9 ] * SRGBlinearApprox( in_Color ) * vec4( vec3( _va_freqHigh[10 ].x ), 1.0 );
		float flicker = max( sin( ( _va_freqLow[3 ].x + _va_freqHigh[11 ].x ) * 40.0 ) * 0.25 + 0.75, 0.0 );
		float strike = min( max( sin( ( _va_freqLow[3 ].x + _va_freqHigh[11 ].x ) * 3.0 ) * 20.0, 0.0 ), 1.0 ) * ( sin( ( _va_freqLow[3 ].x + _va_freqHigh[11 ].x ) * 2.8 ) * 0.5 + 0.5 );
		float intensity = sin( _va_freqLow[3 ].x + _va_freqHigh[11 ].x ) * 0.4 + 0.6;
		vofi_TexCoord5 = vec4( flicker, strike, intensity, 0.0 );
		vec3 worldPos;
		worldPos.x = dot4( vec4(in_Position.xyz, 1.0 ), _va_freqHigh[12 ] );
		worldPos.y = dot4( vec4(in_Position.xyz, 1.0 ), _va_freqHigh[13 ] );
		worldPos.z = dot4( vec4(in_Position.xyz, 1.0 ), _va_freqHigh[14 ] );
		{
			...
			if ( _va_freqLow[5 ].z < 1.0 ) {
				vec3 cameraToPixel = worldPos - _va_freqLow[6 ].xyz;
				float distanceToPixel = length( cameraToPixel );
				float be = _va_freqLow[5 ].x * exp2( - _va_freqLow[7 ].w * ( _va_freqLow[6 ].z - _va_freqLow[7 ].x ) );
				...
				lineIntegral *= ( 1.0 - exp2( - _va_freqLow[7 ].w * effectiveZ ) ) / ( _va_freqLow[7 ].w * effectiveZ );
				heightFogAmount = 1.0 - max( saturate( exp2( -lineIntegral ) ), _va_freqLow[5 ].z );
				float cosL = dot( _va_freqLow[8 ].xyz, normalize( cameraToPixel ) );
				float blendValue = saturate( ( cosL - _va_freqLow[7 ].y ) * _va_freqLow[7 ].z );
