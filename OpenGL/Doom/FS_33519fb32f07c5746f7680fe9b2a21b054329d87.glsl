	...
	{
		{
			...
			inputs.emissive = vec3( _fa_freqHigh[3 ].x * sampleLightmap.x * sampleLightmap.x * _fa_freqHigh[4 ].x ) * _fa_freqHigh[5 ].xyz * _fa_freqHigh[6 ].xyz;
			...
			{
				vec2 tc = vofi_TexCoord0.xy * _fa_freqHigh[7 ].xy;
				vec4 sampleNormal = tex2D( samp_sparebumpmap, tc );
				vec4 sampleAlbedo = tex2DDeGamma( samp_sparediffusemap, tc );
				vec4 sampleSpecular = tex2D( samp_sparespecularmap, tc );
				float sampleSmoothness = tex2D( samp_sparepowermap, tc ).x;
				inputs.albedo = sampleAlbedo.xyz;
				inputs.specular = sampleSpecular.xyz;
				inputs.smoothness = sampleSmoothness;
				inputs.normal = sampleNormal.xyz;
				inputs.alpha = _fa_freqHigh[8 ].x;
			};
			{
				...
				{
					{
						...
						clusterCoordinate.xy = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[10 ] ) * _fa_freqLow[4 ].zw;
			...
			{
				float fogVal = saturate( vofi_TexCoord0.z );
				vec4 icon = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy );
				float lumMask = saturate( dot( inputs.specular_lighting.xyz, vec3( 1 ) ) );
				inputs.output_lighting.xyz.xyz = SRGBlinearApprox( _fa_freqHigh[11 ].xyz ) * vec3( lumMask ) * inputs.albedo.xyz;
				inputs.output_lighting.xyz.xyz += _fa_freqHigh[11 ].xyz * vec3( lumMask * _fa_freqHigh[12 ].x );
				inputs.output_lighting.xyz.xyz = saturate( inputs.output_lighting.xyz.xyz + inputs.specular_lighting.xyz ) + icon.xyz * _fa_freqHigh[11 ].xyz;
				inputs.alpha = max( inputs.alpha, lumMask );
				vec3 viewDir = normalize( -vofi_TexCoord4.xyz );
				float teamfxID = _fa_freqHigh[13 ].x == 1.0 ? 0.0 : 1.0;
				float distance = length( vofi_TexCoord4.xyz );
				{
					float fresnel = saturate( 1.0 - dot( inputs.normal.xyz.xyz, viewDir.xyz ) );
					float fadeStart = _fa_freqHigh[14 ].x;
					float fadeEnd = _fa_freqHigh[15 ].x;
					float fadeRange = max( 1.0, fadeEnd - fadeStart );
					float value = max( 0.0, distance - fadeStart );
					float alpha = saturate( value / fadeRange );
					fresnel = saturate( pow( fresnel, _fa_freqHigh[16 ].x ) * _fa_freqHigh[7 ].y );
					vec3 rimLightColor = ( 1.0 * _fa_freqHigh[7 ].x ) * _fa_freqHigh[17 ].xyz;
					vec3 finalRimlight = fresnel * rimLightColor;
					inputs.output_lighting.xyz.xyz = mix( inputs.output_lighting.xyz.xyz, inputs.output_lighting.xyz.xyz + finalRimlight, alpha * teamfxID * 1.0 );
				};
				vec3 fogColor;
				float fogAmount;
				{
					...
					vec3 heightFogColor = mix( fogLightInscatterScaledColor, _fa_freqLow[13 ].xyz, vofi_TexCoord6.z );
				...
				inputs.output_lighting.xyz.xyz = mix( fogColor * vec3( _fa_freqLow[15 ].x ), inputs.output_lighting.xyz.xyz, fogAmount );
			};
