...
void main() {
	{
		...
		float clipTransitionSeconds = _fa_freqHigh[0 ].x;
		float clipProgress = _fa_freqHigh[0 ].y * clipTransitionSeconds;
		float thresholdThickness = 0.075;
		vec3 extraEmissive = vec3( step( clipMask.r - clipProgress / clipTransitionSeconds - thresholdThickness, 0.0 ) );
		lightingInput_t lightInput = lightingInput_t( vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, vec3(0,0,0), vec3(0,0,1), vec3(0,0,1), vec3(0,0,0), vec3(0,0,0), 1, 1, vec3(0,0,0), vec3(0,0,0), vec4(0,0,0,0), vec4(0,0,0,0), mat3x3( vec3(1,0,0), vec3(0,1,0), vec3(0,0,1) ), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, 0, 0, uvec4(0,0,0,0), uvec4(0,0,0,0), uvec4(0,0,0,0) );
		{
			...
			vec3 globalNormal;
			{
				...
				vec2 feedbackTexCoord = texCoords.xy;
				{
					...
					float feedbackBias = _fa_freqHigh[3 ].y;
					uvec2 feedbackPageUV = uvec2( feedbackTexCoord * widthInPages );
					uint feedbackLOD = uint( min( _fa_freqHigh[3 ].w, max( 0.0, anisoLOD ) + feedbackBias ) );
			...
			};
			{
				...
				vec2 FRAMEBUFFER_PIXEL_SIZE = TEXTURE_RESOLUTION * _fa_freqHigh[4 ].zw;
				vec2 tc = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[4 ] ) * TEXTURE_RESOLUTION;
			...
			{
				uint skipStaticModel = _fa_freqHigh[5 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
						...
						clusterCoordinate.xy = screenPosToTexcoord( lightInput.fragCoord.xy, _fa_freqHigh[4 ] ) * _fa_freqLow[5 ].zw;
			...
			float friendlyEnemyControl = _fa_freqHigh[7 ].z;
			vec3 controlledPulseColor = _fa_freqHigh[7 ].x * mix( _fa_freqHigh[8 ].xyz, _fa_freqHigh[9 ].xyz, friendlyEnemyControl );
			float emissiveStrength = 6.0;
			vec3 controlledEmissiveColor = mix( _fa_freqHigh[10 ].xyz, _fa_freqHigh[11 ].xyz, friendlyEnemyControl ) * emissiveStrength;
			...
			lightInput.diffuse_lighting.xyz += emissiveContrib + extraEmissive * controlledEmissiveColor;
		};
		{
