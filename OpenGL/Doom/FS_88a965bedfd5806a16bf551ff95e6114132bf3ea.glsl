	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				vec4 specular = vec4( 0.08 );
				vec4 normal = tex2D( samp_sparebumpmap, tc.xy );
				float smoothness = tex2DDeGamma( samp_sparepowermap, tc ).x;
				inputs.albedo = _fa_freqHigh[1 ].xyz * albedo.xyz;
				inputs.alpha = _fa_freqHigh[1 ].w * albedo.w;
						...
						for ( uint probeIdx = probesMin; probeIdx < probesMax; ) {
							...
							{ light_t lightParms;
								...
								light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _fa_freqLow[5 ].www );
								{
									...
									diffEnvProbe = mix( GetLuma( diffEnvProbe.xyz ).xxx, diffEnvProbe.xyz, _fa_freqLow[5 ].www );
									specEnvProbe = mix( GetLuma( specEnvProbe.xyz ).xxx, specEnvProbe.xyz, _fa_freqLow[5 ].www );
			...
			} ;
			{
				...
				vec4 refr_tc = MatrixMul( inputs.position.xyz + refraction_vec * refr_len, mat4x4( _fa_freqLow[8 ], _fa_freqLow[9 ], _fa_freqLow[10 ], _fa_freqLow[11 ] ) );
				...
				float fogAmount;
				{
					...
					vec3 fogLightInscatterScaledColor = _fa_freqLow[12 ].xyz * _fa_freqLow[13 ].x;
					vec3 heightFogColor = mix( fogLightInscatterScaledColor, _fa_freqLow[14 ].xyz, vofi_TexCoord5.z );
					...
					fogColor = mix( _fa_freqLow[15 ].xyz, heightFogColor, fogSelection );
					fogAmount = 1.0 - fogAmount;
				};
				inputs.output_lighting.xyz.xyz = mix( fogColor * vec3( _fa_freqLow[16 ].x ) * fogScale, inputs.output_lighting.xyz.xyz, fogAmount );
			};
			out_FragColor0 = vec4( inputs.output_lighting.xyz, inputs.alpha );
			out_FragColor0.xyz += _fa_freqLow[17 ].xyz;
		};
