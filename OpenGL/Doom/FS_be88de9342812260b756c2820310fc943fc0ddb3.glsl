...
void main() {
	{
		...
		globalReflection.xyz = reflect( vec3( 0.0, 0.0, 1.0 ), normalize( vec3( 1.0, 1.0, 0.0 ) ) );
		...
		environmentColor = texCUBElod( samp_dynamicdiffuseenvmap, globalReflection );
		environmentColor = SRGBlinearApprox(environmentColor);
		final.xyz *= vofi_Color.xyz * vofi_TexCoord2.x;
		final.xyz = saturate( final.xyz + saturate( spec.xyz * vofi_TexCoord2.z * environmentColor.xyz ) );
		final.w *= scale;
		final.w = saturate( final.w );
		final.xyz = mix( _fa_freqLow[0 ].xyz, final.xyz, vofi_TexCoord0.z );
