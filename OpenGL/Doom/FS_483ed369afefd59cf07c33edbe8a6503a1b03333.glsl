...
void main() {
	{
		vec3 planeToPos = vofi_TexCoord7.xyz - _fa_freqHigh[0 ].xyz;
		clip( dot3( planeToPos, _fa_freqHigh[1 ].xyz ) );
		{
			...
			{
				vec4 texAlbedo = tex2D( samp_sparediffusemap, vofi_TexCoord0.xy );
				inputs.albedo = texAlbedo.xyz * _fa_freqHigh[2 ].xyz;
				clip( texAlbedo.w - 0.3 );
				inputs.specular = tex2D( samp_sparespecularmap, vofi_TexCoord0.xy ).xyz;
				inputs.smoothness = tex2D( samp_sparepowermap, vofi_TexCoord0.xy ).x;
				inputs.normal = tex2D( samp_sparebumpmap, vofi_TexCoord0.xy ).wyz;
				{
					inputs.smoothness = -inputs.smoothness;
				};
			};
			{
				...
				{
					{
						...
						clusterCoordinate.xy = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[4 ] ) * _fa_freqLow[1 ].zw;
