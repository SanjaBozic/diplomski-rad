...
void main() {
	{
		float edgeGlowIntensity = 30.0;
		float edgeWidth = 16.0;
		...
		vec2 fromCenter = vofi_TexCoord0.xy - center;
		vec2 rotator = fromCenter;
		vec2 sineCosine = vec2( 0.0, 0.0 );
		sineCosine.x = sin( _fa_freqLow[0 ].x * 0.125 ); sineCosine.y = cos( _fa_freqLow[0 ].x * 0.125 );;
		rotator.x = fromCenter.x * sineCosine.y - fromCenter.y * sineCosine.x;
		rotator.y = fromCenter.x * sineCosine.x + fromCenter.y * sineCosine.y;
		vec2 rotatedUVs = center + rotator;
		...
		vec4 final = tex2D( samp_doublehelixdiffusemap, rotatedUVs + distort.xy * 0.025 );
		vec4 luminanceValues = vec4( 0.299, 0.587, 0.114, 0.0 );
		float luminance = dot( final, luminanceValues );
		luminance *= luminance;
		final.xyz *= mix( _fa_freqHigh[4 ].xyz, vec3( 1.0, 1.0, 1.0 ), luminance ) * 0.35;
		final.xyz = mix( final.xyz, final.xyz * edgeGlowIntensity, intersection );
