...
void main() {
	{
		...
		vofi_TexCoord0.zw = tc;
		vofi_Color = _va_freqHigh[7 ] * _va_freqHigh[8 ].x * _va_freqHigh[9 ] * SRGBlinearApprox( in_Color ) * vec4( vec3( _va_freqHigh[10 ].x ), 1.0 );
		vec3 worldPos;
		worldPos.x = dot4( vec4(in_Position.xyz, 1 ), _va_freqHigh[11 ] );
		worldPos.y = dot4( vec4(in_Position.xyz, 1 ), _va_freqHigh[12 ] );
		worldPos.z = dot4( vec4(in_Position.xyz, 1 ), _va_freqHigh[13 ] );
		{
