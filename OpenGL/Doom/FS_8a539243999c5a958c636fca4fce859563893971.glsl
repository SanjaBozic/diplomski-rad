...
void main() {
	{
		vec4 noise = tex2DDeGamma( samp_magnificationclouds, vofi_TexCoord0.xy * vec2( 10.0 ) );
		float fade = 1.0 - _fa_freqHigh[0 ].w;
		clip( fade - noise.y );
		float deathFade = 1.0;
		{ if ( _fa_freqHigh[1 ].x < 1.0 ) {
				vec2 texcoord = gl_FragCoord.xy * ( 0.00390625 );
				float dither = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * _fa_freqHigh[2 ].xy, 0.0, 0.0 ) ).x;
				deathFade = _fa_freqHigh[1 ].x - dither * 0.5 - 0.1;
				if ( true ) {
					clip( deathFade );
				}
			}
