...
void main() {
	{
		vec3 globalNormal = vofi_TexCoord1.xyz;
		vec3 viewOffset = vofi_TexCoord4.xyz;
		float fresnel = abs( dot( globalNormal, normalize( -viewOffset ) ) );
		float fresnel2 = fresnel * fresnel * fresnel;
		fresnel2 = 1.0 - fresnel2;
		fresnel = 1.0 - fresnel;
		vec3 degammacolor = SRGBlinearApprox( _fa_freqHigh[0 ].xyz );
		vec3 degammacolor2 = SRGBlinearApprox( _fa_freqHigh[1 ].xyz );
		vec3 colorLerp = mix( degammacolor * _fa_freqHigh[2 ].x, degammacolor2, fresnel2 );
		out_FragColor0.xyz = colorLerp;
		vec3 fogColor;
		float fogAmount;
		{
			float FLT_EPSILON = 0.0001;
			vec3 fogLightInscatterScaledColor = _fa_freqLow[0 ].xyz * _fa_freqLow[1 ].x;
			vec3 heightFogColor = mix( fogLightInscatterScaledColor, _fa_freqLow[2 ].xyz, vofi_TexCoord5.z );
		...
		out_FragColor0.xyz = mix( fogColor * vec3( _fa_freqLow[4 ].x ), out_FragColor0.xyz, fogAmount ) * _fa_freqHigh[3 ].x;
		out_FragColor0.w = mix( _fa_freqHigh[0 ].w, 1.0, fresnel ) * _fa_freqHigh[3 ].x;
		{
