...
void main() {
	{
		...
		float A = 1.0; if ( _fa_freqHigh[0 ].x > 0.0 ) {
			A = tex2D( samp_binka, vofi_TexCoord0.xy).x;
		}
		vec3 p = vec3( Y * 1.164123535 );
		p += ( crc * Cr ) + ( crb * Cb ) + adj;
		out_FragColor0.xyz = p * vofi_Color.xyz * _fa_freqHigh[1 ].x;
