...
void main() {
	{
		...
		gl_Position.w = dot4( in_Position, _va_freqHigh[3 ] );
		{
			...
			};
			{
				...
				vofi_TexCoord1.x = dot3( localTangent, _va_freqHigh[0 ] );
				vofi_TexCoord1.y = dot3( localBitangent, _va_freqHigh[0 ] );
				vofi_TexCoord1.z = dot3( localNormal, _va_freqHigh[0 ] );
				vofi_TexCoord1.w = morphScale;
				vofi_TexCoord2.x = dot3( localTangent, _va_freqHigh[1 ] );
				vofi_TexCoord2.y = dot3( localBitangent, _va_freqHigh[1 ] );
				vofi_TexCoord2.z = dot3( localNormal, _va_freqHigh[1 ] );
				vofi_TexCoord2.w = 0.0;
				vofi_TexCoord3.x = dot3( localTangent, _va_freqHigh[2 ] );
				vofi_TexCoord3.y = dot3( localBitangent, _va_freqHigh[2 ] );
				vofi_TexCoord3.z = dot3( localNormal, _va_freqHigh[2 ] );
				vofi_TexCoord3.w = 0.0;
			};
			{
				globalPosition.x = dot4( localPosition, _va_freqHigh[0 ] );
				globalPosition.y = dot4( localPosition, _va_freqHigh[1 ] );
				globalPosition.z = dot4( localPosition, _va_freqHigh[2 ] );
		...
		};
		{
			...
			if ( _va_freqLow[5 ].z < 1.0 ) {
				vec3 cameraToPixel = gl_Position.xyz - _va_freqLow[2 ].xyz;
