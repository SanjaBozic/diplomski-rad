...
void main() {
	{
		...
		float opacitySoftZScale = 1.0;
		{
			...
			opacitySoftZScale = saturate( mix( opacitySoftZScale, opacitySoftZScale * 500.0, _fa_freqHigh[1 ].x ) );
		};
		out_FragColor0.a = opacity * opacitySoftZScale;
		float emissive = vofi_TexCoord2.w;
		vec3 diffuse_lighting = tex2DBicubic( samp_particleslightatlasmap, vofi_TexCoord4.xy, _fa_freqLow[0 ].xy ).xyz;
