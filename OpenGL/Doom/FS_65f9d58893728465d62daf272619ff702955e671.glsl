	...
	{
		{
			...
			{
				vec2 worldTc = vofi_TexCoord6.xy * vec2( _fa_freqHigh[8 ].zw );
				vec2 slowTc = vofi_TexCoord0.xy * vec2( 0.8 ) + vec2( 0.0, _fa_freqLow[4 ].x ) * vec2( _fa_freqHigh[8 ].xy * 2.0 );
				vec4 flowMap = tex2D( samp_flowvariation, worldTc * vec2( _fa_freqHigh[9 ].xy ) + vec2( _fa_freqLow[4 ].x ) * vec2( _fa_freqHigh[9 ].zw ) );
				flowMap.xy = flowMap.xy * 2.0 - 1.0;
				flowMap.xy = vec2( flowMap.x * ( 0.02 ), flowMap.y * ( 0.02 ) );
				vec2 tc1 = vofi_TexCoord0.xy + vec2( 0.0, _fa_freqLow[4 ].x ) * vec2( _fa_freqHigh[8 ].xy ) + flowMap.xy;
				vec2 tc2 = vofi_TexCoord0.xy + flowMap.xy;
				vec4 panRock = tex2DDeGammaApprox( samp_sparediffusemap, tc1 );
				vec4 panRockNormal = tex2D( samp_sparebumpmap, tc1 );
				vec4 staticRock = tex2DDeGammaApprox( samp_sparediffusemap, tc2 );
				vec4 staticRockNormal = tex2D( samp_sparebumpmap, tc2 );
				vec4 blendRock = tex2DDeGammaApprox( samp_sparediffusemap, slowTc );
				vec4 blendRockNormal = tex2D( samp_sparebumpmap, slowTc );
				vec4 glow = mix( staticRockNormal, mix( panRockNormal, blendRockNormal, vec4( vofi_Color.y ) ), vec4( vofi_Color.z ) );
				glow = SRGBlinearApprox( glow );
				vec4 finalColor = mix( staticRock, mix( panRock, blendRock, vec4( vofi_Color.y ) ), vec4( vofi_Color.z ) );
				vec4 finalNormal = mix( staticRockNormal, mix( panRockNormal, blendRockNormal, vec4( vofi_Color.y ) ), vec4( vofi_Color.z ) );
				inputs.albedo = finalColor.xyz;
				inputs.smoothness = finalColor.w;
				inputs.specular = _fa_freqHigh[10 ].xyz;
				inputs.normal = finalNormal.xyz;
				inputs.alpha = glow.w;
			};
			{
				uint skipStaticModel = _fa_freqHigh[11 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
				...
				{
					{
						...
						inputs.normal = GetWorldSpaceNormal( inputs.normal, inputs.invTS, isFrontFacing );
						{
							...
							decalsMin = ( skipStaticModel == 0 && _fa_freqHigh[11 ].y == 0.0 ) ? decalsMax : decalsMin;
			...
			{
				float clumps = 1.0 - inputs.alpha;
				clumps *= clumps * clumps * clumps;
				clumps = 1.0 - clumps;
				clumps = mix( clumps, 1.0, vofi_Color.x * vofi_Color.x * vofi_Color.x );
				vec3 redHotMask = clumps * vec3( vofi_Color.x ) * vec3( clumps );
				inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, mix( _fa_freqHigh[12 ].xyz, _fa_freqHigh[13 ].xyz * 10.0, redHotMask ), redHotMask );
			};
			{
