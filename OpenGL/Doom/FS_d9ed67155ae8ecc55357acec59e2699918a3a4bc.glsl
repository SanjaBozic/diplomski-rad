...
void main() {
	{
		...
		vec4 cloud = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy * _fa_freqHigh[2 ].xy + vec2( _fa_freqLow[0 ].x * _fa_freqHigh[2 ].z, _fa_freqLow[0 ].y * _fa_freqHigh[2 ].w ) );
		vec3 lightningCloudMask = abs( vec3( 1.0 ) - dot( cloud.xyz, vec3( 1.0 ) ) );
		lightningCloudMask = cloud.xyz;
		lightningCloudMask *= lightningCloudMask * lightningCloudMask * lightningCloudMask;
		cloud *= vofi_Color;
		vec4 mask = tex2DDeGammaApprox( samp_transmap1, vofi_TexCoord0.xy );
		vec4 lightningMask = tex2DDeGammaApprox( samp_transmap2, vofi_TexCoord0.xy * _fa_freqHigh[3 ].xy + vec2( _fa_freqLow[0 ].x * _fa_freqHigh[3 ].z, _fa_freqLow[0 ].y * _fa_freqHigh[3 ].w ) );
		cloud *= _fa_freqHigh[4 ];
		cloud.w *= falloff;
		cloud.w *= mask.w;
		out_FragColor0.w = cloud.w;
		vec3 fogColor;
		float fogAmount;
		{
		...
		fogAmount = 1.0 - ( ( 1.0 - fogAmount ) * _fa_freqHigh[5 ].x );
		cloud.xyz = mix( fogColor * vec3( _fa_freqLow[5 ].x ), cloud.xyz, fogAmount );
		float maskVariation = mix( lightningMask.z, mix( lightningMask.x, lightningMask.y, vofi_TexCoord5.x * vofi_TexCoord5.z ), vofi_TexCoord5.y );
		float finalLightning = vofi_TexCoord5.x * vofi_TexCoord5.y * vofi_TexCoord5.z;
		out_FragColor0.xyz = cloud.xyz + ( lightningCloudMask * maskVariation * _fa_freqHigh[6 ].xyz * _fa_freqHigh[7 ].x * finalLightning );
		{
