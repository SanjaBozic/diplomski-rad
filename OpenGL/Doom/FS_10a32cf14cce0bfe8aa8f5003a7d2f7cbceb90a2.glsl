	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				finalRipple = mix( flatNormal.xyz, mix( normalRipples.xyz, normalRipples2.xyz, vec3( 0.5 ) ), vec3( vofi_Color.x ) );
				vec4 texAlbedo = tex2DDeGammaApprox( samp_sparediffusemap, panningUV );
				inputs.albedo = texAlbedo.xyz * _fa_freqHigh[10 ].xyz;
				inputs.specular = tex2DDeGammaApprox( samp_sparespecularmap, panningUV ).xyz;
				inputs.smoothness = tex2DDeGammaApprox( samp_sparepowermap, panningUV ).x;
