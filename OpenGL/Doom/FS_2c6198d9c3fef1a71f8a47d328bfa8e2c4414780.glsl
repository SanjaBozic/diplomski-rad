...
void main() {
	{
		...
		float fresnelSmooth = 1.0 - dot( globalNormal, normalize( -viewOffset ) );
		float fresnelSmooth2 = pow( fresnelSmooth, 2 );
		float interpolatedDepth = GetFragmentDepth( gl_FragCoord );
		vec2 flatTc = vec2( 0.5 );
		...
		float softScale = 1.0;
		{
			...
			softScale = saturate( ( r1 - r2 ) * 1.0 );
			softScale = saturate( mix( softScale, softScale * 500.0, 0.0 ) );
		};
		float scale = (1.0 - _fa_freqHigh[1 ].x) * softScale;
		float fresnelScaleStatic = ( fresnelSmooth2 * 0.5 + 0.25) * scale;
		vec2 staticDistort = ( vofi_TexCoord1.xy ) * fresnelScaleStatic * _fa_freqHigh[2 ].x;
		float fresnelScaleDynamic = ( fresnelSmooth2 * 0.8 + 0.05 ) * scale;
		vec2 dynamicDistort = ( vofi_TexCoord2.xy ) * fresnelScaleDynamic * _fa_freqHigh[2 ].x;
		vec2 distort = staticDistort + dynamicDistort;
		distort *= vec2( 0.3 * saturate( vofi_TexCoord0.z ) );
		float fog = vofi_TexCoord0.z;
		distort *= vec2( fog * vofi_TexCoord0.w );
		out_FragColor0 = ComputeDistortionMV( distort, 0.0 );
	}
}