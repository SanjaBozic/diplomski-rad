	...
	{
		{
			...
			} ;
			{
				...
				{ if ( _fa_freqHigh[18 ].x > 0.0 ) {
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						inputs.diffuse_lighting.xyz.xyz += ( ( _fa_freqHigh[19 ].xyz * 0.001 ) * _fa_freqHigh[19 ].w * _fa_freqHigh[20 ].x ) * fresnel * fresnel * 0.8;
					}
				};
			};
			{
