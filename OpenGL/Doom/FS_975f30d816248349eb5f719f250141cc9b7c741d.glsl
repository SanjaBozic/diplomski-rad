...
void main() {
	{
		...
		vec4 distort = tex2D( samp_transmap1, vofi_TexCoord0.xy * _fa_freqHigh[1 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[1 ].zw + vec2( vofi_TexCoord2.z * 3.0 ) );
		distort.xy = ( distort.xy - vec2( 0.5 ) ) * vec2( _fa_freqHigh[2 ].x );
		vec4 electric = tex2D( samp_transmap2, vofi_TexCoord0.xy * _fa_freqHigh[3 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[3 ].zw + distort.xy + vec2( vofi_TexCoord2.w * 4.0 ) );
		electric *= diffuse.w;
		electric *= electric;
		vec4 final = diffuse;
		final.w = 1.0;
		final += electric.x;
		final.w *= scale;
		final.xyz *= vofi_TexCoord2.x;
		final.w *= vofi_TexCoord2.y;
		final *= vofi_Color;
		final.xyz -= vec3( _fa_freqHigh[4 ].x );
