...
void main() {
	{
		...
		vec3 headNormal = normalize( localPosition.xyz - _va_freqHigh[9 ].xyz );
		vofi_TexCoord5.x = dot3( headNormal, _va_freqHigh[5 ] );
		vofi_TexCoord5.y = dot3( headNormal, _va_freqHigh[6 ] );
		vofi_TexCoord5.z = dot3( headNormal, _va_freqHigh[7 ] );
		...
		vofi_TexCoord7.xy = vofi_TexCoord0.xy * _va_freqHigh[30 ].xy;
