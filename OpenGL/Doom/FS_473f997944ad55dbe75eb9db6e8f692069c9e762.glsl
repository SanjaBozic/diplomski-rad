...
void main() {
	{
		vec4 mask = vec4( 1.0 ); if( _fa_freqHigh[0 ].w > 0 ) {
			mask = tex2D( samp_transmap1, vofi_TexCoord1.zw );
		}
		vec4 clouds = tex2D( samp_transmap, vofi_TexCoord1.xy );
		float scale = 1.0; if ( _fa_freqHigh[1 ].x > 0 ) {
			...
			srcDepth += _fa_freqHigh[1 ].y * ( 1.0 - clouds.w );
			scale = saturate( ( dstDepth - srcDepth ) / _fa_freqHigh[1 ].x );
		}
		vec4 final = clouds;
		final.xyz = vec3( 1.0 ) - ( ( vec3( 1.0 ) - final.xyz ) * vec3( 1.0 - _fa_freqHigh[0 ].y ) );
		final.xyz *= vec3( _fa_freqHigh[0 ].x );
		final.w = saturate( clouds.w + _fa_freqHigh[1 ].z );
		final.w *= scale * vofi_TexCoord0.w;
		final *= vofi_Color * mask;
		final.xyz -= _fa_freqHigh[3 ].x;
		float fog = vofi_TexCoord0.z;
		final.xyz = mix( _fa_freqLow[0 ].xyz, final.xyz, fog );
