...
void main() {
	{
		...
		vec3 wpos = vofi_TexCoord4.xyz + _va_freqLow[2 ].xyz;
		{
			if ( _va_freqHigh[9 ].x > 0.0 && _va_freqHigh[9 ].y > 0.0 ) {
				...
				vec3 normSamplePoint = saturate( ( wpos - vec3( AMBIENT_MIN_WORLD ) ) * leafWidth );
		...
		};
		{
			...
			if ( _va_freqLow[5 ].z < 1.0 ) {
				vec3 cameraToPixel = wpos - _va_freqLow[2 ].xyz;
