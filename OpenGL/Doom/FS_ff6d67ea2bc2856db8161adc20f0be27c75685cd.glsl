...
void main() {
	{
		vec3 pos;
		pos.z = 1.0 / gl_FragCoord.w;
		pos.xy = pos.z * ( 2.0 * screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[0 ] ) - 1.0 );
		vec3 normal = -normalize( cross( dFdx( pos ), dFdy( pos ) ) );
		vec3 L = normalize( vec3( 1.0, 1.0, 0.0 ) - pos );
		out_FragColor0 = vec4( saturate( dot3( normal, L ) ) + 0.3 );
		vec4 texcolor = tex2D( samp_texturemap, vofi_TexCoord0.xy );
		if ( texcolor.w < 0.1 ) {
			out_FragColor0 *= _fa_freqHigh[1 ];
		} else {
