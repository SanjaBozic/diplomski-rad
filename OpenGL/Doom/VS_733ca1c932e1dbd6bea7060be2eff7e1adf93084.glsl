...
void main() {
	{
		...
		vec3 vertexPos = in_Position.xyz;
		vec4 size = vec4( _va_freqHigh[30 ].x, _va_freqHigh[30 ].y, _va_freqHigh[30 ].z, _va_freqHigh[30 ].w );
		vec4 smmBrainFrontLeft = vec4( 130.8352, 70.5662, 326.917, 150.0 );
		vec3 smmWoundA = ( smmBrainFrontLeft.xyz - vertexPos.xyz );
		float smmWoundA_DOT = dot3( smmWoundA, smmWoundA );
		smmWoundA_DOT = sqrt( smmWoundA_DOT );
		smmWoundA_DOT = clamp( 1.0 - smmWoundA_DOT / ( smmBrainFrontLeft.w * size.x ), 0.0, 1.0 );
		vec4 smmBrainFrontRight = vec4( 130.8352, -70.5662, 326.917, 150.0 );
		vec3 smmWoundB = ( smmBrainFrontRight.xyz - vertexPos.xyz );
		float smmWoundB_DOT = dot3( smmWoundB, smmWoundB );
		smmWoundB_DOT = sqrt( smmWoundB_DOT );
		smmWoundB_DOT = clamp( 1.0 - smmWoundB_DOT / ( smmBrainFrontRight.w * size.y ), 0.0, 1.0 );
		vec4 smmBrainMidLeft = vec4( -8.22215, 112.359, 361.132, 210.0 );
		vec3 smmWoundC = ( smmBrainMidLeft.xyz - vertexPos.xyz );
		float smmWoundC_DOT = dot3( smmWoundC, smmWoundC );
		smmWoundC_DOT = sqrt( smmWoundC_DOT );
		smmWoundC_DOT = clamp( 1.0 - smmWoundC_DOT / ( smmBrainMidLeft.w * size.z ), 0.0, 1.0 );
		vec4 smmBrainMidRight = vec4( -8.22215, -112.359, 361.132, 210.0 );
		vec3 smmWoundD = ( smmBrainMidRight.xyz - vertexPos.xyz );
		float smmWoundD_DOT = dot3( smmWoundD, smmWoundD );
		smmWoundD_DOT = sqrt( smmWoundD_DOT );
		smmWoundD_DOT = clamp( 1.0 - smmWoundD_DOT / ( smmBrainMidRight.w * size.w ), 0.0, 1.0 );
		float revealMask;
		revealMask = saturate( smmWoundA_DOT + smmWoundB_DOT + smmWoundC_DOT + smmWoundD_DOT );
		vofi_TexCoord6.y = revealMask;
		vofi_TexCoord7 = vec4( vofi_TexCoord0.xy * _va_freqHigh[31 ].xy, 0, 0 );
	}
}