...
void main() {
	{
		vec2 texcoord = gl_FragCoord.xy * _fa_freqHigh[0 ].zw;
		{
			vec4 scalingInfos = vec4( _fa_freqLow[0 ].xy, vec2( 1. / _fa_freqHigh[0 ].xy ) );
			vec3 linearColor;
			if ( _fa_freqLow[1 ].x < 3 ) {
				vec3 linearColorLowPass = tex2DBicubicClamped( samp_viewcolormap, texcoord.xy, _fa_freqHigh[0 ].xy, scalingInfos ).rgb;
				linearColor = SRGBlinear( saturate( mix( linearColorLowPass, tex2DlodClamped( samp_viewcolormap, vec4( texcoord, 0, 0 ), scalingInfos ).rgb, _fa_freqLow[0 ].www ) ) ); if ( _fa_freqLow[1 ].x == 2 ) {
					linearColor *= ( texcoord.y < 0.05 * _fa_freqLow[0 ].y || texcoord.y > 0.95 * _fa_freqLow[0 ].y ) ? 0.0 : 1.0;
				}
			}
			else {
			...
			vec4 guiColor = tex2Dlod( samp_tex0, vec4( texcoord / _fa_freqLow[0 ].xy, 0, 0 ) );
			guiColor.xyz = SRGBlinear( guiColor.xyz );
			float guiSaturation = ( _fa_freqLow[1 ].x == 0 ) ? 1.0 : 0.9;
			float guiBlendScale = ( _fa_freqLow[1 ].x == 0 ) ? 1.0 : 0.9;
			...
			out_FragColor0.xyz = linearSRGB( finalColor ); if ( _fa_freqHigh[1 ].x != 1.0 ) {
				out_FragColor0.xyz = pow( out_FragColor0.xyz, _fa_freqHigh[1 ].xxx );
			}
			{ if ( _fa_freqHigh[2 ].w > 0.0 ) {
					vec2 grain_tc = texcoord.xy + ( ivec2( _fa_freqLow[2 ].xy ) & 1023 ) / 1023.0;
					float grain = fract( sin( grain_tc.x + grain_tc.y * 521.0 ) * 493013.0 ) * 2.0 - 1.0;
					out_FragColor0.xyz = saturate( out_FragColor0.xyz + saturate( 1 - GetLuma( out_FragColor0.xyz ) ) * grain.xxx * _fa_freqHigh[2 ].w );
				}
			};
			{
				...
				-0.5044, 0.0159, 2.0517 ); if ( _fa_freqHigh[2 ].x == 1.0 ) {
					vec3 blindVisionR = vec3( 0.20, 0.99, -0.19 );
					vec3 blindVisionG = vec3( 0.16, 0.79, 0.04 );
					vec3 blindVisionB = vec3( 0.01, -0.01, 1.00 );
					vec3 opponentColor = MatrixMul( RGBtoOpponentMat , out_FragColor0.xyz );
					opponentColor.x -= opponentColor.y * 1.5;
					out_FragColor0.xyz = MatrixMul( OpponentToRGBMat , opponentColor );
					out_FragColor0.xyz = vec3( dot( out_FragColor0.xyz, blindVisionR ), dot( out_FragColor0.xyz, blindVisionG ), dot( out_FragColor0.xyz, blindVisionB ) );
				}
				else if ( _fa_freqHigh[2 ].x == 2.0 ) {
