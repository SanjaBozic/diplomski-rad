	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				woundMap.xyz -= vec3( 0.5 );
				{
					...
					vec3 diffuseBloodLerp = mix( inputs.albedo.xyz.xyz, _fa_freqHigh[8 ].xyz, vec3( woundMask ) );
					inputs.albedo.xyz.xyz *= mix( vec3( 1.0 ), _fa_freqHigh[9 ].xyz, vec3( woundMask ) );
					...
					inputs.normal.xy += mix( woundMap.xy * vec2( woundMask ), vec2( 0.0 ), vec2( _fa_freqHigh[9 ].w ) );
				};
			};
			{
				uint skipStaticModel = _fa_freqHigh[10 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
			...
			{
				{ if ( _fa_freqHigh[11 ].x > 0 ) {
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[12 ].zw );
						...
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[13 ].x; if ( _fa_freqHigh[11 ].y >= _fa_freqHigh[13 ].z ) {
							pulsefrequency = _fa_freqHigh[13 ].y;
						}
						float width = mix( _fa_freqHigh[14 ].x, _fa_freqHigh[14 ].y, _fa_freqHigh[11 ].y );
						float zValue = ( vofi_TexCoord4.xyz.z ) * width;
						float pulse = sin( zValue + _fa_freqLow[11 ].x * pulsefrequency ) * magnitude + magnitude;
						float power = _fa_freqHigh[14 ].z;
						pulse = pow( pulse, power );
						pulse *= fresnel;
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[15 ].xyz * 2.0;
						finalPulse /= _fa_freqHigh[16 ].xyz;
				...
				{ if ( _fa_freqHigh[17 ].w > 0 ) {
						...
						vec3 finalPulseFresnel = saturate( digitalNoise.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[17 ].xyz;
						vec3 finalPulseFlat = saturate( digitalNoise.x ) * _fa_freqHigh[17 ].xyz;
						vec3 finalPulse = mix( finalPulseFresnel, finalPulseFlat, vec3( 0.25 ) );
						vec3 constantGlow = saturate( 0.5 * ( fresnel ) + edgeGlow ) * _fa_freqHigh[17 ].xyz;
						finalPulse /= _fa_freqHigh[16 ].xyz;
						vec3 finalPulseColor = finalPulse * _fa_freqHigh[17 ].w;
				...
				}; if ( _fa_freqHigh[18 ].x * ( 1.0 - _fa_freqHigh[19 ].x ) > 0.0 ) {
					float teamfxID = _fa_freqHigh[20 ].x == 1.0 ? 0.0 : 1.0;
					...
					float distance = length( vofi_TexCoord4.xyz );
					{
						...
						float fadeStart = _fa_freqHigh[21 ].x;
						float fadeEnd = _fa_freqHigh[22 ].x;
						float fadeRange = max( 1.0, fadeEnd - fadeStart );
						float value = max( 0.0, distance - fadeStart );
						float alpha = saturate( value / fadeRange );
						fresnel = saturate( pow( fresnel, _fa_freqHigh[23 ].x ) * _fa_freqHigh[24 ].y );
						vec3 rimLightColor = ( 1.0 * _fa_freqHigh[24 ].x ) * _fa_freqHigh[25 ].xyz;
						vec3 finalRimlight = fresnel * rimLightColor;
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz + finalRimlight, alpha * teamfxID * _fa_freqHigh[26 ].x );
					};
				}
				{ if ( _fa_freqHigh[11 ].x > 0.0 ) {
						vec2 resolution = abs( vec2( 1.0 ) / _fa_freqHigh[12 ].zw );
						...
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						float pulseTime = mix( 10.0, 30.0, _fa_freqHigh[11 ].y );
						...
						finalPulse = saturate( dither.x * ( fresnel + pulse ) + edgeGlow ) * _fa_freqHigh[15 ].xyz * 2.0;
						inputs.diffuse_lighting.xyz.xyz += finalPulse;
					}
				};
				{ if ( _fa_freqHigh[19 ].x > 0.0 ) {
						vec4 colorWave = tex2DlodDeGamma( samp_transmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[27 ].xy + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[27 ].zw, 0.0, 0.0) );
						...
						vec3 close = crevices * crevices * vec3( 180.0 ) * _fa_freqHigh[28 ].xyz + finalOutline * _fa_freqHigh[28 ].xyz;
						vec3 far = _fa_freqHigh[28 ].xyz * vec3( 0.1 ) * colorWave.xyz * vec3( fresnel2 ) + vec3( fresnel2 ) * _fa_freqHigh[28 ].xyz * vec3( 20.0 );
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz, ( inputs.diffuse_lighting.xyz + mix( close, far, vec3( farFade ) ) * colorWave.xyz ) + _fa_freqHigh[28 ].xyz * vec3( 0.01 ) * vec3( fresnel2 ), vec3( _fa_freqHigh[19 ].x ) );
					}
				};
				{ if ( _fa_freqHigh[29 ].x > 0.0 ) {
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						inputs.diffuse_lighting.xyz.xyz += ( ( _fa_freqHigh[30 ].xyz * 0.001 ) * _fa_freqHigh[30 ].w * _fa_freqHigh[31 ].x ) * fresnel * fresnel * 0.8;
					}
				};
				{ if ( _fa_freqHigh[32 ].x == 1.0 ) { if ( _fa_freqHigh[33 ].x == 1.0 ) {
							vec4 shieldPattern = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[27 ].xy ) * 3.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[24 ].zw, 0.0, 0.0 ) );
							vec4 shieldPattern2 = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[27 ].xy ) * 2.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[27 ].zw, 0.0, 0.0 ) );
							...
							inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 1.0, 0.25 ) * vec3( 50.0 ), _fa_freqHigh[33 ].w );
						}
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.5, 1.0, 0.5 ) * vec3( 50.0 ), _fa_freqHigh[33 ].y );
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 0.0, 0.0 ) * vec3( 50.0 ), _fa_freqHigh[33 ].z );
					}
				};
				{ if ( _fa_freqHigh[34 ].x > 0.0 ) {
						...
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, holoColor.xyz, _fa_freqHigh[34 ].x );
						inputs.diffuse_lighting.xyz.xyz = mix (inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz * noise.xyz * 2.3, _fa_freqHigh[34 ].x);
					}
				};
			};
			{
