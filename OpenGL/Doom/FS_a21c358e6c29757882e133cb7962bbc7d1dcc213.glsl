...
uniform sampler2D samp_magnificationmask;
	...
	{
		{
			...
			} ;
			{
				...
				float teamfxID = _fa_freqHigh[24 ].x == 1.0 ? 0.0 : 1.0;
				...
				float distance = length( vofi_TexCoord4.xyz );
				{
					...
					float fadeStart = _fa_freqHigh[25 ].x;
					float fadeEnd = _fa_freqHigh[26 ].x;
					float fadeRange = max( 1.0, fadeEnd - fadeStart );
					float value = max( 0.0, distance - fadeStart );
					float alpha = saturate( value / fadeRange );
					fresnel = saturate( pow( fresnel, _fa_freqHigh[27 ].x ) * _fa_freqHigh[28 ].y );
					vec3 rimLightColor = ( 1.0 * _fa_freqHigh[28 ].x ) * _fa_freqHigh[29 ].xyz;
					vec3 finalRimlight = fresnel * rimLightColor;
					inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz + finalRimlight, alpha * teamfxID * _fa_freqHigh[30 ].x );
				};
				...
				vec2 noiseAnim = mix( vec2 ( 0.1, 0.6 ), vec2( 0.1, 1.5 ), _fa_freqHigh[31 ].xx );
				vec2 noiseTiling = vec2 (10.25,3.25) * _fa_freqHigh[32 ].xy;
				vec4 noise = tex2D( samp_magnificationmask, (vofi_TexCoord0.xy * noiseTiling) + vec2( _fa_freqLow[11 ].x ) * noiseAnim );
				noise += 0.03;
				vec4 glowMask = tex2D( samp_transmap, vofi_TexCoord0.xy);
				vec4 glowColor = ( _fa_freqHigh[33 ] * glowMask.x);
				vec4 glowColorC = ( _fa_freqHigh[34 ] * glowMask.y) ;
				glowColor *= _fa_freqHigh[35 ].x;
				inputs.diffuse_lighting.xyz.xyz += mix( inputs.diffuse_lighting.xyz.xyz + glowColor.xyz * (noise.x * _fa_freqHigh[35 ].y ), inputs.diffuse_lighting.xyz.xyz + ( glowColorC.xyz * (noise.x * _fa_freqHigh[35 ].x )), _fa_freqHigh[31 ].xxx );
			};
			{
