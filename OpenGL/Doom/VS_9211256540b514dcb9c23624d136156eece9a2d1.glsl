...
void main() {
	{
		...
		vec4 preAnimPosition = localPosition;
		{
		...
		float burnFalloff = 0.0;
		{ if ( _va_freqHigh[8 ].x < 1.0 ) {
				float burnTransition = 1.0 - _va_freqHigh[8 ].x;
				burnFalloff = saturate( length( preAnimPosition.xyz - vec3( 0.0, 0.0, 72.0 ) ) / 72.0 );
				burnFalloff = ( burnFalloff * 3.0 + 1.0 ) - ( burnTransition * 4.5 );
				float burnDistort = 1.0 - saturate( burnFalloff );
				burnDistort *= burnDistort * burnDistort;
				globalPosition.z -= burnDistort * 16.0;
				gl_Position.x = dot4( globalPosition, _va_freqLow[3 ] );
				gl_Position.y = dot4( globalPosition, _va_freqLow[4 ] );
				gl_Position.z = dot4( globalPosition, _va_freqLow[5 ] );
				gl_Position.w = dot4( globalPosition, _va_freqLow[6 ] );
			}
		};
		{
			vofi_TexCoord0 = ( in_TexCoord.xy * _va_freqLow[7 ].xy + _va_freqLow[7 ].zw ).xyxy;
		};
		vofi_TexCoord7.xy = vec2( 0.0 );
		vofi_TexCoord7.z = burnFalloff;
		vofi_TexCoord7.w = 0.0;
	}
}