	...
	{
		{
			...
			vec3 outputColor;
			{
				...
				float vlen_threshold = dot( _ca_freqHigh[0 ].zw, _ca_freqHigh[0 ].zw ); if ( dot( max_v.xy, max_v.xy ) - vlen_threshold > 0.0 && _ca_freqHigh[1 ].z == 1.0 ) {
					...
					vec2 tc_start = tc.xy - max_v.xy; if ( _ca_freqHigh[1 ].x == 1.0 ) {
						vec2 pixIdx = vec2( ivec2(gl_GlobalInvocationID.xy ).xy.xy & 1 ) * 2.0 - 1.0 ;
						float ditherK = 0.15;
						tc_start = tc_start + max_v * ( pixIdx.x * pixIdx.y * ditherK );
					}
