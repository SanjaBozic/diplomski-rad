...
void main() {
	{ if ( _fa_freqHigh[0 ].x != 3.0 ) {
		...
		vec2 mainSurfaceTC = tcMainSurface * _fa_freqHigh[6 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[7 ].zw;
		vec2 mainSurfaceTC2 = tcMainSurface * _fa_freqHigh[8 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[8 ].zw;
		...
		vec3 pulse = tex2D( samp_transmap, edgePulseTC ).xyz * _fa_freqHigh[6 ].w;
		vec3 mainSurface = tex2D( samp_transmap2, mainSurfaceTC ).xyz * _fa_freqHigh[7 ].y;
		vec3 mainSurface2 = tex2D( samp_transmap2, mainSurfaceTC2 ).xyz * _fa_freqHigh[7 ].x;
		...
		vec4 final = vec4( 0.0 );
		final.w = mix( mainSurface.x, edgeDiffuse1.x + edgeDiffuse2.x, intersect) ;
		final.w *= _fa_freqHigh[2 ].x;
		final.xyz = mix( boundaryColor.xyz * mainSurface.xyz * mainSurface2.xyz , boundaryEdgeColor.xyz ,intersect );
