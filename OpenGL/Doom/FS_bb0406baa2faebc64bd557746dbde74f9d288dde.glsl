...
void main() {
	{
		vec4 smoke = tex2D( samp_transmap1, vofi_TexCoord0.xy * _fa_freqHigh[0 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[0 ].zw ) * vofi_Color;
		vec4 scanlines = tex2D( samp_transmap2, vofi_TexCoord0.xy * _fa_freqHigh[1 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[1 ].zw ) * vofi_Color;
		vec4 final = tex2DDeGamma( samp_transmap, vofi_TexCoord0.xy ) * vofi_Color;
		final.xyz += smoke.xyz;
		final.xyz += scanlines.xyz;
