...
void main() {
	{
		...
		vofi_TexCoord1.z = mix( sin( _va_freqLow[4 ].x * 90.0 ), 0.0, 0.9 ) + 1.0;
		vofi_TexCoord1.w = saturate( dot( viewDir, worldNormal ) );
		vec3 dir = vec3( 0.0 );
		dir.x = sin( _va_freqLow[4 ].x * 1.7 + in_Position.y * 0.5 );
		dir.y = sin( _va_freqLow[4 ].x * 2.1 + in_Position.z * 0.63 );
		dir.z = sin( _va_freqLow[4 ].x * 2.3 + in_Position.x * 0.77 );
		dir = normalize( dir );
