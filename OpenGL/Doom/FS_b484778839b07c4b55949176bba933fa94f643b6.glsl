...
void main() {
	{
		vec3 worldNormal = normalize( vofi_TexCoord1.xyz );
		vec3 sunDir = normalize( _fa_freqHigh[0 ].xyz );
		float sunDot = saturate( dot( worldNormal, sunDir ) * 0.5 + 0.5 );
		sunDot = smoothstep( 0.999, 1.0, sunDot );
		vec2 tc1 = vofi_TexCoord0.xy * _fa_freqHigh[1 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[1 ].zw;
		vec4 stars = tex2DDeGamma( samp_transmap, tc1 );
		vec4 final = stars;
		final.xyz *= vec3( 1.0 - sunDot );
		float fog = saturate( -vofi_TexCoord1.z );
		final.xyz *= fog;
		final.xyz *= final.xyz;
		final.xyz *= 2.0;
		final.xyz *= _fa_freqHigh[2 ].x;
