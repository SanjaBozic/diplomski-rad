...
uniform sampler2D samp_viewcolorlastframemap;
	...
	{
		{
			...
			} ;
			{
				{ if ( _fa_freqHigh[9 ].x > 0 ) {
						...
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[11 ].x; if ( _fa_freqHigh[9 ].y >= _fa_freqHigh[11 ].z ) {
							pulsefrequency = _fa_freqHigh[11 ].y;
						}
						float width = mix( _fa_freqHigh[12 ].x, _fa_freqHigh[12 ].y, _fa_freqHigh[9 ].y );
						float zValue = ( vofi_TexCoord4.xyz.z ) * width;
						float pulse = sin( zValue + _fa_freqLow[11 ].x * pulsefrequency ) * magnitude + magnitude;
						float power = _fa_freqHigh[12 ].z;
						pulse = pow( pulse, power );
						pulse *= fresnel;
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[13 ].xyz * 2.0;
						finalPulse /= _fa_freqHigh[14 ].xyz;
				...
				{ if ( _fa_freqHigh[15 ].w > 0.0 ) {
						vec4 noise = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * vec2( 7.0 ) + vec2( 0.0, _fa_freqLow[11 ].x * 3.0 ), 0.0, 0.0 ) );
						noise = DeGamma( noise );
						float fade = 1.0 - _fa_freqHigh[15 ].w * _fa_freqHigh[15 ].w;
						inputs.diffuse_lighting.xyz.xyz.xyz += mix( inputs.diffuse_lighting.xyz.xyz.xyz, _fa_freqHigh[15 ].xyz * noise.z * noise.z * 20.0, _fa_freqHigh[15 ].w );
					}
				};
				inputs.diffuse_lighting.xyz.xyz += vec3( _fa_freqHigh[4 ].x * inputs.lightmap.x * inputs.lightmap.x ) * _fa_freqHigh[7 ].xyz;
				vec2 screenTC = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[10 ] );
				vec4 distort = tex2D( samp_magnificationclouds, screenTC + vec2( _fa_freqLow[11 ].x * 0.1 ) ) * 2.0 - 1.0;
				distort *= 0.05;
				vec2 localDistort = inputs.normal.xy * 2.0 - 1.0;
				vec2 perturb = mix( distort.xy, localDistort * 0.065, 0.35 ) * 0.333;
				vec4 lastFrame = tex2Dlod( samp_viewcolorlastframemap, vec4( screenTC + perturb, 0.0, 2.0 ) );
				vec4 final = lastFrame;
				final.w = 1.0;
				vec3 desaturated = vec3( GetLuma( inputs.diffuse_lighting.xyz.xyz ) );
				inputs.diffuse_lighting.xyz.xyz = inputs.diffuse_lighting.xyz + inputs.specular_lighting.xyz;
				inputs.diffuse_lighting.xyz.xyz = mix( final.xyz, desaturated * _fa_freqHigh[7 ].xyz * 15.0, 0.45 );
				{ if ( _fa_freqHigh[16 ].x < 1.0 ) {
						vec2 texcoord = gl_FragCoord.xy * ( 0.00390625 );
						float dither = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * _fa_freqHigh[17 ].xy, 0.0, 0.0 ) ).x;
						dither = saturate( abs( clamp( _fa_freqHigh[16 ].x - dither * 0.5 - 0.1, -1.0, 0.0 ) ) );
						dither *= 600.0;
