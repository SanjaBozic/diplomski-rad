...
void main() {
	{vec4 pos = in_Position;
		...
		vec2 tc = in_TexCoord.xy * _va_freqLow[0 ].x + _va_freqLow[0 ].y;
		vec2 noiseTC1 = ( tc * vec2( 5.2, 3.5 ) ) - vec2( _va_freqLow[1 ].x ) * vec2( 30.5, 40.3 );
		vec2 noiseTC2 = ( tc * vec2( 6.3, 4.2 ) ) - vec2( _va_freqLow[1 ].x ) * vec2( 25.3, 35.5 );
		vofi_TexCoord2.xy = noiseTC1;
		vofi_TexCoord2.zw = noiseTC2;
	}
}