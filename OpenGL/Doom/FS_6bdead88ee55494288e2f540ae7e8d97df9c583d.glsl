...
uniform sampler2D samp_radiositypreviewmap;
	...
	{
		{
			...
			{
				vec2 tc;
				tc.x = vofi_TexCoord10.x / vofi_TexCoord10.w;
				tc.y = vofi_TexCoord10.y / vofi_TexCoord10.w;
				tc = ( tc + 1.0 ) * 0.5;
				vec4 color = tex2D( samp_radiositypreviewmap, tc );
				inputs.lightmap.xyz = color.xyz;
			};
			{
				...
				{
					{
						...
						vec3 ambient = vec3( 0 );
						ambient = (inputs.lightmap.xyz * _fa_freqLow[7 ].xyz) * _fa_freqLow[8 ].x;
			...
			} ;
			{
				{ if ( _fa_freqHigh[9 ].w > 0 ) {
						...
						inputs.diffuse_lighting.xyz += dither.xxx * vec3( nDotSomething + 0.01 ) * _fa_freqHigh[9 ].xyz * vec3( _fa_freqHigh[9 ].w * 2.0 ) * 2.0;
					}
				};
				{ if ( _fa_freqHigh[11 ].x > 0 ) {
						...
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[12 ].x; if ( _fa_freqHigh[11 ].y >= _fa_freqHigh[12 ].z ) {
							pulsefrequency = _fa_freqHigh[12 ].y;
						}
						float width = mix( _fa_freqHigh[13 ].x, _fa_freqHigh[13 ].y, _fa_freqHigh[11 ].y );
						float zValue = ( vofi_TexCoord4.xyz.z ) * width;
						float pulse = sin( zValue + _fa_freqLow[12 ].x * pulsefrequency ) * magnitude + magnitude;
						float power = _fa_freqHigh[13 ].z;
						pulse = pow( pulse, power );
						pulse *= fresnel;
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[14 ].xyz * 2.0;
						finalPulse /= _fa_freqHigh[15 ].xyz;
						vec3 finalPulseColor = finalPulse * pulse;
						float saturation = ( _fa_freqLow[13 ].x == 0 ) ? 1.0 : 0.7;
						float blendScale = ( _fa_freqLow[13 ].x == 0 ) ? 1.0 : 0.5;
