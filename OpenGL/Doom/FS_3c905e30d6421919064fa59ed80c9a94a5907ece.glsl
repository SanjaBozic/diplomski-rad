...
void main() {
	{
		...
		vec3 c = tex2D( samp_tex0, tc ).xyz;
		out_FragColor0.rgb = linearSRGB( c ); if ( _fa_freqLow[1 ].x == 2.0 ) {
			float luma = GetLuma( out_FragColor0.rgb );
			out_FragColor0.rgb = vec3( luma );
			out_FragColor0.rgb = ( luma * _fa_freqHigh[1 ].w != 0.0 ) ? vec3( 1.0, 0.0, 0.0 ) : out_FragColor0.rgb;
			out_FragColor0.rgb = ( c.r < 0.0 || c.g < 0.0 || c.b < 0.0 ) ? vec3( 0.0, 0.0, 1.0 ) : out_FragColor0.rgb;
		} if ( _fa_freqLow[1 ].x == 3.0 ) {
			float luma = GetLuma( c );
			out_FragColor0.rgb = vec3( luma );
			out_FragColor0.rgb = ( luma < _fa_freqHigh[2 ].z ) ? vec3( 0.0, 0.0, 1.0 ) : out_FragColor0.rgb;
			out_FragColor0.rgb = ( luma > _fa_freqHigh[2 ].w ) ? vec3( 1.0, 0.0, 0.0 ) : out_FragColor0.rgb;
			out_FragColor0.rgb = linearSRGB( out_FragColor0.rgb );
		}
	}
}