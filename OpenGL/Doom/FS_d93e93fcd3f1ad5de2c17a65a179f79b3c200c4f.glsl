...
void main() {
	{
		...
		clip( opacity - 1e-6 );
		color.xyz = vec3( 1.0 ) - ( vec3( 1.0 ) - color.xyz ) * vec3( vofi_TexCoord2.z );
		color.xyz *= vec3( vofi_TexCoord2.x );
		color.w *= vofi_TexCoord2.y;
		color *= vofi_Color;
		color.xyz = mix( _fa_freqLow[0 ].xyz, color.xyz, fogVal ) * vec3( _fa_freqLow[1 ].x );
