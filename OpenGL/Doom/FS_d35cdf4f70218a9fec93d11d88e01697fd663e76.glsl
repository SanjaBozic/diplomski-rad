...
void main() {
	{
		...
		float albedoSaturation = ( _fa_freqLow[3 ].x == 0 ) ? 1.0 : 0.5;
		float albedoBlendScale = ( _fa_freqLow[3 ].x == 0 ) ? 1.0 : 0.9;
		sampleDiffuse.xyz = mix( GetLuma( sampleDiffuse.xyz ).xxx, sampleDiffuse.xyz, albedoSaturation.xxx ) * albedoBlendScale;
		{
		...
		lightingInput_t lightInput = lightingInput_t( vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, vec3(0,0,0), vec3(0,0,1), vec3(0,0,1), vec3(0,0,0), vec3(0,0,0), 1, 1, vec3(0,0,0), vec3(0,0,0), vec4(0,0,0,0), vec4(0,0,0,0), mat3x3( vec3(1,0,0), vec3(0,1,0), vec3(0,0,1) ), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, 0, 0, uvec4(0,0,0,0), uvec4(0,0,0,0), uvec4(0,0,0,0) );
		{
			...
			lightInput.position = _fa_freqLow[4 ].xyz + vofi_TexCoord4.xyz;
		...
		};
		{
			...
			lightInput.ambient_lighting.xyz += max ( indirectDiffuse.xyz * sampleDiffuse.xyz * saturate( _fa_freqLow[5 ].x ), vec3( 0 ) );
		};
		{
			...
			{
				{
					...
					clusterCoordinate.xy = screenPosToTexcoord( lightInput.fragCoord.xy, _fa_freqHigh[3 ] ) * _fa_freqLow[6 ].zw;
					clusterCoordinate.xy = floor( clusterCoordinate.xy * vec2( NUM_CLUSTERS_X, NUM_CLUSTERS_Y ) );
					float curr_z = GetLinearDepth( lightInput.fragCoord.z, _fa_freqLow[7 ], 1.0, false );
		...
		vec4 electric = tex2D( samp_transmap, screenTexCoord * _fa_freqHigh[6 ].xy + vec2( _fa_freqLow[12 ].x ) * _fa_freqHigh[6 ].zw );
		vec4 noise = tex2D( samp_transmap2, screenTexCoord * _fa_freqHigh[7 ].xy + vec2( fract( _fa_freqLow[12 ].x ) ) * _fa_freqHigh[7 ].zw );
		vec4 iconography = vec4( sampleLightmap.x );
		float fresnel = 1.0 - dot( globalNormal, normalize( -viewOffset ) );
		float pulse = 4.0;
		pulse *= sin( _fa_freqLow[12 ].x * 5.0 ) * 0.5 + 0.7;
		pulse *= pulse;
		pulse *= fresnel;
		float fresnel2 = pow( fresnel, 1.5 );
		fresnel = fresnel * fresnel;
		float edgeGlow = fresnel * fresnel * fresnel * fresnel;
		electric.xyz = pow( electric.xyz, vec3( 2.0 ) );
		noise.xyz = pow( noise.xyz, vec3( 3.0 ) );
		vec3 digitalNoise = electric.xyz + noise.yyy;
		vec3 finalPulseFresnel = saturate( digitalNoise.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[8 ].xyz;
		vec3 finalPulseFlat = saturate( digitalNoise.x ) * _fa_freqHigh[8 ].xyz;
		vec3 finalPulse = mix( finalPulseFresnel, finalPulseFlat, _fa_freqHigh[9 ].x );
		vec3 constantGlow = saturate( 0.5 * ( fresnel2 ) + edgeGlow ) * _fa_freqHigh[8 ].xyz;
		vec3 effect = constantGlow;
		effect += finalPulse * pulse;
		effect += iconography.xyz * _fa_freqHigh[10 ].xyz * 2.0;
		float saturation = ( _fa_freqLow[3 ].x == 0 ) ? 1.0 : 0.5;
		float blendScale = ( _fa_freqLow[3 ].x == 0 ) ? 1.0 : 0.2;
		lightInput.diffuse_lighting.xyz += mix( GetLuma( effect ).xxx, effect, saturation.xxx ) * blendScale;
		{
