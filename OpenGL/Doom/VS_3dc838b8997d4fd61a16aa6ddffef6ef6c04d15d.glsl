...
void main() {
	{
		...
		vec4 vertPosition = in_Position * _va_freqLow[0 ] + _va_freqLow[1 ];
		vec4 tempPosition;
		tempPosition.x = dot4( vertPosition, _va_freqHigh[5 ] );
		tempPosition.y = dot4( vertPosition, _va_freqHigh[6 ] );
		tempPosition.z = dot4( vertPosition, _va_freqHigh[7 ] );
		tempPosition.w = dot4( vertPosition, _va_freqHigh[10 ] );
		vofi_TexCoord6.xy = tempPosition.xy;
		vofi_TexCoord6.w = dot( ( in_Normal.xyz * 2.0 - 1.0 ), vec3( 0, 0, 1 ) );
		vofi_TexCoord6.w = 1.0 - vofi_TexCoord6.w;
		vofi_Color = in_Color;
	}
}