...
void main() {
	{
		...
		{
			vec4 far = tex2DBicubic( samp_tex0, texcoord.xy, _fa_freqHigh[1 ].xy );
			float dz = tex2Ddepth( samp_viewdepthmap, texcoord.xy * _fa_freqLow[0 ].zw );
			float lz = GetLinearDepth( dz, _fa_freqLow[1 ], 1.0, true );
			float cocFar = 0.0;
			if ( lz >= _fa_freqHigh[2 ].x ) {
				float farfield_bias = 1.0;
				cocFar = saturate( ( lz - _fa_freqHigh[2 ].x - farfield_bias ) / ( _fa_freqHigh[2 ].z - _fa_freqHigh[2 ].x - farfield_bias ) );
				float blendRatio = 10.0;
				out_FragColor0 = vec4( far.xyz * saturate( sqrt( cocFar * blendRatio) ), saturate( 1 - sqrt( cocFar * blendRatio ) ) );
			} else {
