...
void main() {
	{
		...
		vec2 tc2 = vofi_TexCoord0.xy * _fa_freqHigh[1 ].xy;
		vec4 diffuse = tex2DDeGammaApprox( samp_transmap1, tc );
		vec4 normal = tex2D( samp_transmap2, tc2 );
		vec4 specular = mix( _fa_freqHigh[2 ].xxxx, _fa_freqHigh[2 ].yyyy, tex2DDeGammaApprox( samp_transmap3, tc ) );
		vec4 power = mix( _fa_freqHigh[2 ].zzzz, _fa_freqHigh[2 ].wwww, tex2DDeGammaApprox( samp_transmap4, tc ) );
		vec3 globalNormal = vec3( 1.0 );
		vec4 color = vec4( 1.0 );
		vec3 viewDir = normalize( -vofi_TexCoord4.xyz );
		float fresnel = saturate( 1.0 - dot( globalNormal.xyz, viewDir.xyz ) );
		float fresnelMain = saturate( mix( _fa_freqHigh[3 ].x, _fa_freqHigh[3 ].y, pow( fresnel, _fa_freqHigh[3 ].z ) ) );
		{
			vec3 N = normal.wyz * 2.0 - 1.0;
		...
		};
		{
			...
			{
				{
					...
					vec2 texCoordsLOD = inputs.texCoords.xy; if ( _fa_freqHigh[4 ].x > 0 ) {
						texCoords.xy = fract( inputs.texCoords.xy ) * _fa_freqHigh[5 ].xy + _fa_freqHigh[5 ].zw;
						texCoordsLOD = inputs.texCoords.xy * _fa_freqHigh[5 ].xy;
					}
					float anisoLOD;
					float sampleLOD;
					{
						float widthInTexels = _fa_freqLow[1 ].z;
						float maxAnisoLog2 = _fa_freqHigh[6 ].z;
					...
					};
					{
						...
						float feedbackBias = _fa_freqHigh[6 ].y;
						uvec2 feedbackPageUV = uvec2( texCoords.xy * widthInPages );
						uint feedbackLOD = uint( min( _fa_freqHigh[6 ].w, max( 0.0, anisoLOD ) + feedbackBias ) );
			...
			inputs.emissive = vec3( _fa_freqHigh[7 ].x * sampleLightmap.x * sampleLightmap.x * _fa_freqHigh[8 ].x ) * _fa_freqHigh[9 ].xyz * _fa_freqHigh[10 ].xyz;
			...
			{
				inputs.albedo = diffuse.xyz;
				inputs.specular = specular.xyz;
				inputs.smoothness = power.y;
				inputs.view = normalize( vofi_TexCoord4.xyz );
				inputs.normal = globalNormal;
				inputs.alpha = color.w;
			};
			{
				...
				{
					{
						...
						clusterCoordinate.xy = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[12 ] ) * _fa_freqLow[4 ].zw;
			...
			{
				vec4 colBlue = vec4(0.05, 0.3, 1.0, 0.0);
				vec4 colRed = vec4(0.85, 0.025, 0.015, 0.0);
				vec4 coreColor = mix(colBlue, colRed, _fa_freqHigh[13 ].x) ;
				float rimFresnel = saturate( mix( _fa_freqHigh[14 ].x, _fa_freqHigh[14 ].y, pow( fresnel, _fa_freqHigh[14 ].z ) ) );
				vec4 finalRimlight = vec4( rimFresnel ) * coreColor;
				inputs.specular_lighting.xyz += finalRimlight.xyz;
				float iceOpacity = saturate( ( fresnelMain.x + diffuse.w) * _fa_freqHigh[15 ].x );
				vec3 regularColor = inputs.diffuse_lighting.xyz + ( inputs.specular_lighting.xyz * vec3( diffuse.x * _fa_freqHigh[0 ].w ) );
				vec3 specOnlyColor = inputs.specular_lighting.xyz * vec3( diffuse.x * _fa_freqHigh[0 ].z );
				inputs.output_lighting.xyz.xyz = mix( specOnlyColor, regularColor, iceOpacity );
				inputs.alpha = coreColor.w;
				inputs.alpha *= mix( saturate( fresnel * coreColor.w + 0.01 ), coreColor.w, coreColor.w );
				inputs.alpha += specular.w * 2.0;
				inputs.alpha *= ( ceil( ( 1.0 - _fa_freqHigh[16 ].x ) * 4.0 ) ) / 4.0 * _fa_freqHigh[15 ].x;
			};
