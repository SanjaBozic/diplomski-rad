	...
	{
		{
			...
			{
				vec2 panningUV = vofi_TexCoord0.xy * vec2( _fa_freqHigh[8 ].x, _fa_freqHigh[8 ].y ) + vec2( _fa_freqLow[4 ].x * _fa_freqHigh[8 ].z, _fa_freqLow[4 ].x * _fa_freqHigh[8 ].w );
				vec2 panningUV2 = vofi_TexCoord0.xy * vec2( _fa_freqHigh[9 ].x, _fa_freqHigh[9 ].y ) + vec2( _fa_freqLow[4 ].x * _fa_freqHigh[9 ].z, _fa_freqLow[4 ].x * _fa_freqHigh[9 ].w );
				vec4 normalRipples = tex2D( samp_sparebumpmap, panningUV );
				vec4 normalRipples2 = tex2D( samp_sparebumpmap, panningUV2 );
				vec4 flatNormal = tex2D( samp_transmap, vofi_TexCoord0.xy );
				vec3 finalRipple = vec3( 0.0 );
				finalRipple = mix( flatNormal.xyz, mix( normalRipples.xyz, normalRipples2.xyz, vec3( 0.5 ) ), vec3( vofi_Color.x ) ); if ( _fa_freqHigh[10 ].x > 0.0 ) {
					vec4 flowingNormal = mix( normalRipples, normalRipples2, 0.5 );
					finalRipple = normalize( vec3( flowingNormal.xy + flatNormal.xy, 1.0 ) );
				}
				vec4 texAlbedo = tex2DDeGamma( samp_sparediffusemap, panningUV );
				inputs.albedo = texAlbedo.xyz;
				inputs.specular = tex2DDeGamma( samp_sparespecularmap, panningUV ).xyz;
				inputs.smoothness = tex2DDeGamma( samp_sparepowermap, panningUV ).x;
				inputs.normal = finalRipple;
			};
			{
