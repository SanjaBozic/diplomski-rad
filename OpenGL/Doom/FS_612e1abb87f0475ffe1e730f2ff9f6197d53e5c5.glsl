...
void main() {
	{
		vec4 mask = vec4( 1.0 ); if ( _fa_freqHigh[0 ].x > 0 ) {
			mask = tex2D( samp_transmap, vofi_TexCoord1.xy );
		}
		float scale = 1.0; if ( _fa_freqHigh[0 ].y > 0 ) {
			...
			scale = saturate( ( dstDepth - srcDepth ) / _fa_freqHigh[0 ].y );
		}
		vec4 final = vofi_Color * mask;
		final.w *= scale;
