...
void main() {
	{
		...
		vec4 electric = tex2D( samp_transmap, screenTexCoord * _fa_freqHigh[6 ].xy + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[6 ].zw );
		vec4 noise = tex2D( samp_transmap2, screenTexCoord * _fa_freqHigh[7 ].xy + vec2( fract( _fa_freqLow[11 ].x ) ) * _fa_freqHigh[7 ].zw );
		...
		lightInput.diffuse_lighting.xyz += constantGlow;
		lightInput.diffuse_lighting.xyz += finalPulse * pulse;
		lightInput.diffuse_lighting.xyz += iconography.xyz * _fa_freqHigh[8 ].xyz * 2.0;
		{
