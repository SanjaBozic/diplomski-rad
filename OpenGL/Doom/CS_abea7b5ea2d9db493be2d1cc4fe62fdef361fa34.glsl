...
uint PackRG ( const uint r, const uint g ) {
	return ( r | ( g << 8 ) );
}
uint PackB ( const uint b ) {
	return ( b << 16 );
}
...
void DecodeRGPair ( const uint coeffsOffset, inout uint color1, inout uint color2 ) {
	const uint COEFF_CHANNEL_STRIDE = 8192;
	const int iShift = 3;
	const int iDiv = 1 << iShift;
	const int iBias = 1027;
...
void DecodeCover ( const uint coverOffset, const uint coverPackedOffset, out uint colors[ 16 ] ) {
	for ( int y = 0; y < 4; ++y ) {
		const uint packedCover = gputranscodecover[ coverOffset + y * 4 ];
		colors[ y * 4 + 0 ] = PackB( ( ( ( packedCover >> ( coverPackedOffset + 0 ) ) & 1 ) == 1 ) ? 255 : 0 );
		colors[ y * 4 + 1 ] = PackB( ( ( ( packedCover >> ( coverPackedOffset + 1 ) ) & 1 ) == 1 ) ? 255 : 0 );
		colors[ y * 4 + 2 ] = PackB( ( ( ( packedCover >> ( coverPackedOffset + 2 ) ) & 1 ) == 1 ) ? 255 : 0 );
		colors[ y * 4 + 3 ] = PackB( ( ( ( packedCover >> ( coverPackedOffset + 3 ) ) & 1 ) == 1 ) ? 255 : 0 );
	}
}
...
void DecodeRGB4x4WithCover ( const uint coeffsOffset, const uint coverOffset, const uint coverPackedOffset, out uint colors[ 16 ] ) {
	DecodeCover( coverOffset, coverPackedOffset, colors );
	DecodeRGPair( coeffsOffset + 0, colors[ 0 ], colors[ 1 ] );
	DecodeRGPair( coeffsOffset + 2, colors[ 3 ], colors[ 2 ] );
	DecodeRGPair( coeffsOffset + 1, colors[ 4 ], colors[ 5 ] );
	DecodeRGPair( coeffsOffset + 3, colors[ 7 ], colors[ 6 ] );
	DecodeRGPair( coeffsOffset + 5, colors[ 8 ], colors[ 9 ] );
	DecodeRGPair( coeffsOffset + 7, colors[ 11 ], colors[ 10 ] );
	DecodeRGPair( coeffsOffset + 4, colors[ 12 ], colors[ 13 ] );
	DecodeRGPair( coeffsOffset + 6, colors[ 15 ], colors[ 14 ] );
}
...
void main() {
	{
		...
		uint coverOffset = ( uint( transcodeParms.w ) * 512 ) + groupId.x + ( groupId.y * 128 ) + ( localId.y * 16 );
		uint coverPackedOffset = localId.x * 4;
		DecodeRGB4x4WithCover( coeffsOffset, coverOffset, coverPackedOffset, values );
