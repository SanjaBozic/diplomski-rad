...
void main() {
	{
		...
		vec4 mask = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy ).xxxy;
		float opacity_min = 0.5 / 255.0;
		clip ( mask.w - opacity_min );
		vec4 grime = tex2DDeGammaApprox( samp_transmap1, tc * vec2( 1.0, 0.6 ) );
		grime.xyz += _fa_freqHigh[1 ].x;
		out_FragColor0 = grime * mask * vofi_Color;
		out_FragColor0 *= vofi_TexCoord1.x;
	}
}