...
void main() {
	{
		...
		float wavesIntersect = tex2D( samp_transmap, vec2( 0.5, ( intersectGlow * 2.0 ) + _fa_freqLow[0 ].x ) ).x;
		intersectGlow *= intersectGlow * intersectGlow;
		intersectGlow *= vofi_TexCoord1.x;
		wavesIntersect *= intersectGlow;
		intersectGlow += wavesIntersect;
		intersectGlow *= 0.5;
		vec4 final = _fa_freqHigh[1 ];
		final.w = 0.0;
		final.w += intersectGlow;
		final.w *= viewFalloff;
		final.w *= _fa_freqHigh[2 ].w;
