...
void main() {
	{
		...
		float intersect = 1.0 - ( saturate ( ( dstDepth - srcDepth) / 65.0 ) );
		intersect = ( cos( intersect * 3.14 ) * -0.5 + 0.6 ) * intersect;
		float falloff = saturate( ( dstDepth - srcDepth ) * ( 1.0 / 20.0 ) );
		intersect *= falloff;
		vec3 viewOffset = vofi_TexCoord4.xyz;
		float fresnel = 1.0 - dot( globalNormal, normalize( -viewOffset ) );
		float fresnel2 = fresnel * fresnel;
		fresnel = saturate( pow( fresnel, 5.0) );
		vec4 edgeFire = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy * vec2( _fa_freqHigh[1 ].x, _fa_freqHigh[1 ].y ) + vec2( _fa_freqLow[0 ].x * _fa_freqHigh[1 ].z, _fa_freqLow[0 ].x * _fa_freqHigh[1 ].w ) );
		vec4 surfaceFire = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy * vec2( _fa_freqHigh[2 ].x, _fa_freqHigh[2 ].y ) + vec2( _fa_freqLow[0 ].x * _fa_freqHigh[2 ].z, _fa_freqLow[0 ].x * _fa_freqHigh[2 ].w ) );
		edgeFire *= edgeFire;
		surfaceFire *= surfaceFire;
		float blueToRed = _fa_freqHigh[3 ].w * _fa_freqHigh[3 ].w * _fa_freqHigh[3 ].w;
		vec3 mainColor = mix( _fa_freqHigh[4 ].xyz, _fa_freqHigh[5 ].xyz, blueToRed );
		vec3 edgeColor = mix( _fa_freqHigh[6 ].xyz, _fa_freqHigh[5 ].xyz, blueToRed );
		vec3 colorize = mix( edgeColor, mainColor, intersect );
		vec4 finalColor = ( edgeFire * intersect + surfaceFire * falloff * fresnel ) * vofi_Color * vofi_Color * _fa_freqHigh[7 ].x;
		finalColor.xyz += vec3( 1.0 ) * fresnel2 * 3.0;
		finalColor.xyz *= colorize * _fa_freqHigh[3 ].xyz;
		out_FragColor0 = finalColor;
		out_FragColor0.w *= _fa_freqHigh[3 ].w;
	}
}