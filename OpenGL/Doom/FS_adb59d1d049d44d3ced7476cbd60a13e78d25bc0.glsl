...
void main() {
	{
		vec4 dMap;
		{
			...
			dMap = ( result0 + result1 );
		};
		float interpolatedDepth = gl_FragCoord.z * _fa_freqHigh[0 ].x;
		...
		float scale;
		{
			...
			scale = saturate( ( r1 - r2 ) * vofi_TexCoord0.w / _fa_freqHigh[0 ].x );
			scale = saturate( mix( scale, scale * 500.0, 0.0 ) );
		};
		vec2 distortion = dMap.xy * 2.0 - 1.0;
		distortion *= vofi_Color.xy;
		distortion *= dMap.w * (scale * _fa_freqHigh[2 ].x) * vofi_Color.w;
		float blur = dMap.w * scale * vofi_Color.z * vofi_Color.w;
		out_FragColor0 = ComputeDistortionMV( distortion, blur );
	}
}