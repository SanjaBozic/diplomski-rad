	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				if ( _fa_freqLow[4 ].x == 8 || _fa_freqLow[4 ].x == 9 || _fa_freqLow[4 ].x == 12 ) {
					inputs.albedo.xyz = vec3( 1 );
				}
				if ( _fa_freqLow[4 ].x == 10 ) {
					inputs.albedo.xyz = vec3( 0 );
				}
				else
				if ( _fa_freqLow[4 ].x == 11 ) {
					inputs.albedo.xyz = vec3( 0 );
					inputs.specular.xyz = vec3( 1 );
					inputs.smoothness = 1;
				}
				{
					uint skipStaticModel = _fa_freqHigh[8 ].x > 0 ? ( 1 << 3 ) : 0;
					uint clusterOffset;
					{
						{
							...
							clusterCoordinate.xy = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[3 ] ) * _fa_freqLow[5 ].zw;
							clusterCoordinate.xy = floor( clusterCoordinate.xy * vec2( NUM_CLUSTERS_X, NUM_CLUSTERS_Y ) );
							float curr_z = GetLinearDepth( inputs.fragCoord.z, _fa_freqLow[6 ], 1.0, false );
					...
					{
						{
							...
							inputs.ticksDecals.xy = shaderClock();
							{
								...
								decalsMin = ( skipStaticModel == 0 && _fa_freqHigh[8 ].y == 0.0 ) ? decalsMax : decalsMin;
							...
							inputs.albedo = saturate( unpackR10G10B10( inputs.albedo_packed ) * _fa_freqLow[8 ].z );
							inputs.specular = saturate( unpackR10G10B10( inputs.specular_packed ) );
							inputs.albedo_packed = packR10G10B10( inputs.albedo.xyz );
							inputs.smoothness = saturate( pow( inputs.smoothness, _fa_freqLow[8 ].x ) * _fa_freqLow[8 ].y );
							vec3 ambient = vec3( 0 );
							{
								...
								ambient.xyz += max ( indirectDiffuse.xyz * vec3( 1 ) * saturate( _fa_freqLow[9 ].x ), vec3( 0 ) );
							};
									...
									float shadow = 1.0; if ( ( light_parms & ( 1 << 2 ) ) != 0 ) {
										{
											...
											uint idx = 0; if ( ( light_parms & ( ( 0 << 0 ) | ( 1 << 0 ) | ( 2 << 0 ) | ( 3 << 0 ) ) ) == ( 0 << 0 ) ) {
												...
												float fraction = ( 1.0 - max( shadowTC.x, shadowTC.y ) ) * _fa_freqLow[10 ].w;
												...
												{
													{
														if ( false ) {
															...
															vec2 uv = shadowTC.xy * _fa_freqLow[11 ].xy;
															vec2 shadowMapSizeInv = _fa_freqLow[11 ].zw;
				...
				if ( _fa_freqLow[4 ].x == 9 || _fa_freqLow[4 ].x == 12 ) {
				...
				float min_valid_albedo_non_metal_warning = _fa_freqLow[4 ].y;
				float max_valid_albedo_metal_warning = _fa_freqLow[4 ].z;
				if ( _fa_freqLow[4 ].x >= 1 && _fa_freqLow[4 ].x <= 6 ) {
					float albedo_avg = dot( orig_albedo.xyz, vec3( 0.333 ) );
					float spec_avg = dot( orig_specular.xyz, vec3( 0.333 ) );
					inputs.specular_lighting = vec3( 0 );
					if ( _fa_freqLow[4 ].x == 1 ) {
						inputs.diffuse_lighting.xyz = inputs.albedo.xyz;
					}
					else if ( _fa_freqLow[4 ].x == 2 ) {
						inputs.diffuse_lighting.xyz = inputs.specular.xyz;
					}
					else if ( _fa_freqLow[4 ].x == 3 ) {
						inputs.diffuse_lighting.xyz = inputs.smoothness.xxx;
					}
					else if ( _fa_freqLow[4 ].x == 4 ) {
						inputs.diffuse_lighting.xyz = inputs.normal.xyz * 0.5 + 0.5;
					}
					else if ( _fa_freqLow[4 ].x == 5 ) {
						if ( _fa_freqLow[4 ].w > 0 ) {
							min_valid_albedo = _fa_freqLow[4 ].y;
							max_valid_albedo = _fa_freqLow[4 ].z;
						}
				...
				if ( _fa_freqLow[4 ].x == 9 ) {
					inputs.diffuse_lighting.xyz += inputs.ambient_lighting.xyz;
				}
				else
				if ( _fa_freqLow[4 ].x == 12 ) {
					inputs.diffuse_lighting.xyz = inputs.ambient_lighting.xyz;
				}
				if ( _fa_freqLow[4 ].x == 7 || _fa_freqLow[4 ].x >= 17 && _fa_freqLow[4 ].x < 21 ) {
					float NUM_CLUSTERS_X = 16;
					float NUM_CLUSTERS_Y = 8;
					float NUM_CLUSTERS_Z = 24;
					int MAX_LIGHTS_PER_CLUSTER = 256;
					...
					vec2 norm_tc = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[3 ] ) * _fa_freqLow[5 ].zw;
			...
			out_FragColor0.xyz += _fa_freqLow[12 ].xyz;
		};
	}
}