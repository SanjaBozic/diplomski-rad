...
uniform sampler2D samp_spare3map;
...
void main() {
	{
		vec4 final = vec4(0, 0, 0, 0); if ( _fa_freqHigh[0 ].w > 0.66 ) {
			final = tex2DDeGamma( samp_spare1map, vofi_TexCoord0.xy );
		} else { if ( _fa_freqHigh[0 ].w > 0.33 ) {
				final = tex2DDeGamma( samp_spare2map, vofi_TexCoord0.xy );
			} else {
				final = tex2DDeGamma( samp_spare3map, vofi_TexCoord0.xy );
			}
		}
		out_FragColor0 = final * _fa_freqHigh[1 ];
