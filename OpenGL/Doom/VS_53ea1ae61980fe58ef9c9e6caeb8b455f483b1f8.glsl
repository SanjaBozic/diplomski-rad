...
void main() {
	{
		...
		vofi_TexCoord0.x = ( dot4( texcoord0.xy, _va_freqHigh[5 ] ) * _va_freqHigh[6 ].y ) - ( _va_freqLow[3 ].x * _va_freqHigh[6 ].x );
		vofi_TexCoord0.y = dot4( texcoord0.xy, _va_freqHigh[7 ] );
		vofi_TexCoord0.zw = vec2( 0.0 );
		vec4 vertColor = in_Color;
		vertColor.w = saturate( vertColor.w * 5.0 );
		vofi_Color = _va_freqHigh[8 ] * _va_freqHigh[9 ].x * _va_freqHigh[10 ] * DeGamma( vertColor ) * vec4( vec3( _va_freqHigh[11 ].x ), 1.0 );
	}
}