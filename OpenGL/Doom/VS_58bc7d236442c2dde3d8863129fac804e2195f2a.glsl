...
void main() {
	{
		...
		vec3 toViewer;
		toViewer = normalize( _va_freqLow[3 ].xyz - globalPosition.xyz );
		vofi_TexCoord5.w = localPosition.x;
		vofi_TexCoord6.w = localPosition.y;
		vofi_TexCoord7.w = localPosition.z;
		vofi_TexCoord0.w = saturate( exp2( -( clamp( gl_Position.w - _va_freqLow[4 ].x, 0.0, _va_freqLow[4 ].y ) * _va_freqLow[4 ].z ) ) );
