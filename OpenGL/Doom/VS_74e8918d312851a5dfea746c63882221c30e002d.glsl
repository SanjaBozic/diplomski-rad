...
void main() {
	{
		...
		vec3 headNormal = normalize( localPosition.xyz - _va_freqHigh[0 ].xyz );
		vofi_TexCoord5.x = dot3( headNormal, _va_freqHigh[1 ] );
		vofi_TexCoord5.y = dot3( headNormal, _va_freqHigh[2 ] );
		vofi_TexCoord5.z = dot3( headNormal, _va_freqHigh[3 ] );
		{
			vec4 localBitangent;
			vec4 globalPosition;
			float morphScale;
			{
				morphScale = 0.0; if ( _va_freqHigh[4 ].x != 0.0 ) {
			...
			{
				gl_Position.x = dot4( localPosition, _va_freqHigh[5 ] );
				gl_Position.y = dot4( localPosition, _va_freqHigh[6 ] );
				gl_Position.z = dot4( localPosition, _va_freqHigh[7 ] );
				gl_Position.w = dot4( localPosition, _va_freqHigh[8 ] );
				vofi_TexCoord1.x = dot3( localTangent, _va_freqHigh[1 ] );
				vofi_TexCoord1.y = dot3( localBitangent, _va_freqHigh[1 ] );
				vofi_TexCoord1.z = dot3( localNormal, _va_freqHigh[1 ] );
				vofi_TexCoord1.w = morphScale;
				vofi_TexCoord2.x = dot3( localTangent, _va_freqHigh[2 ] );
				vofi_TexCoord2.y = dot3( localBitangent, _va_freqHigh[2 ] );
				vofi_TexCoord2.z = dot3( localNormal, _va_freqHigh[2 ] );
				vofi_TexCoord2.w = 0.0;
				vofi_TexCoord3.x = dot3( localTangent, _va_freqHigh[3 ] );
				vofi_TexCoord3.y = dot3( localBitangent, _va_freqHigh[3 ] );
				vofi_TexCoord3.z = dot3( localNormal, _va_freqHigh[3 ] );
				vofi_TexCoord3.w = 0.0;
			};
			{
				globalPosition.x = dot4( localPosition, _va_freqHigh[1 ] );
				globalPosition.y = dot4( localPosition, _va_freqHigh[2 ] );
				globalPosition.z = dot4( localPosition, _va_freqHigh[3 ] );
		...
		{
			if ( _va_freqHigh[10 ].x > 0.0 && _va_freqHigh[10 ].y > 0.0 ) {
				...
				vec4 sampleSHR = tex3Dlod( samp_ambientshatlas, vec4( shTC.xy, shTC.z + ( atlasSHCubeSize.z * 0.0 ), 0.0 ) ) * _va_freqHigh[10 ].y;
				vec4 sampleSHG = tex3Dlod( samp_ambientshatlas, vec4( shTC.xy, shTC.z + ( atlasSHCubeSize.z * 1.0 ), 0.0 ) ) * _va_freqHigh[10 ].y;
				vec4 sampleSHB = tex3Dlod( samp_ambientshatlas, vec4( shTC.xy, shTC.z + ( atlasSHCubeSize.z * 2.0 ), 0.0 ) ) * _va_freqHigh[10 ].y;
		...
		};
		{
			...
			vec3 wound3A =( _va_freqHigh[11 ].xyz - vertexPos.xyz ) * _va_freqHigh[11 ].w;
			vec3 wound3B =( _va_freqHigh[12 ].xyz - vertexPos.xyz ) * _va_freqHigh[12 ].w;
			vec3 wound3C =( _va_freqHigh[13 ].xyz - vertexPos.xyz ) * _va_freqHigh[13 ].w;
			vec3 wound3D =( _va_freqHigh[14 ].xyz - vertexPos.xyz ) * _va_freqHigh[14 ].w;
			vec3 wound3E =( _va_freqHigh[15 ].xyz - vertexPos.xyz ) * _va_freqHigh[15 ].w;
			vec3 wound3F =( _va_freqHigh[16 ].xyz - vertexPos.xyz ) * _va_freqHigh[16 ].w;
			vec3 wound3G =( _va_freqHigh[17 ].xyz - vertexPos.xyz ) * _va_freqHigh[17 ].w;
			vec3 wound3H =( _va_freqHigh[18 ].xyz - vertexPos.xyz ) * _va_freqHigh[18 ].w;
			vec3 wound3I =( _va_freqHigh[19 ].xyz - vertexPos.xyz ) * _va_freqHigh[19 ].w;
			vec3 wound3J =( _va_freqHigh[20 ].xyz - vertexPos.xyz ) * _va_freqHigh[20 ].w;
			vec3 wound3K =( _va_freqHigh[21 ].xyz - vertexPos.xyz ) * _va_freqHigh[21 ].w;
			vec3 wound3L =( _va_freqHigh[22 ].xyz - vertexPos.xyz ) * _va_freqHigh[22 ].w;
			vec3 wound3M =( _va_freqHigh[23 ].xyz - vertexPos.xyz ) * _va_freqHigh[23 ].w;
			vec3 wound3N =( _va_freqHigh[24 ].xyz - vertexPos.xyz ) * _va_freqHigh[24 ].w;
			vec3 wound3O =( _va_freqHigh[25 ].xyz - vertexPos.xyz ) * _va_freqHigh[25 ].w;
			vec3 wound3P =( _va_freqHigh[26 ].xyz - vertexPos.xyz ) * _va_freqHigh[26 ].w;
			vec3 wound3Q =( _va_freqHigh[27 ].xyz - vertexPos.xyz ) * _va_freqHigh[27 ].w;
			vec3 wound3R =( _va_freqHigh[28 ].xyz - vertexPos.xyz ) * _va_freqHigh[28 ].w;
			vec3 wound3S =( _va_freqHigh[29 ].xyz - vertexPos.xyz ) * _va_freqHigh[29 ].w;
			vec3 wound3T =( _va_freqHigh[30 ].xyz - vertexPos.xyz ) * _va_freqHigh[30 ].w;
		...
		vofi_TexCoord7.xy = vofi_TexCoord0.xy * _va_freqHigh[31 ].xy;
