...
void main() {
	{
		...
		vec4 electric = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy * _fa_freqHigh[1 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[1 ].zw + mask.xy * 0.05 );
		vec4 electric2 = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy * _fa_freqHigh[2 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[2 ].zw + mask.xy * 0.05 );
		vec3 toViewer = normalize( vofi_TexCoord1.xyz );
		vec3 globalNormal = normalize( vofi_TexCoord2.xyz );
		float ndotv = dot3( toViewer, globalNormal );
		float fresnel = saturate( abs( ndotv ) );
		vec4 final = electric + electric2;
		final.xyz *= mask.w * _fa_freqHigh[3 ].x;
		final *= vofi_Color;
		final *= SRGBlinearApprox( _fa_freqHigh[4 ] );
		final.xyz *= 100.0;
		final.w *= fresnel * fresnel;
