...
void main() {
	{
		float veins = 1.0;
		{
			...
			{
				vec2 damageTiling = vec2( 8.0 );
				vec4 brainDamage = tex2D( samp_sparediffusemap, vofi_TexCoord0.xy * damageTiling.xy );
				vec3 brainDamageNormal = vec3( brainDamage.yz, 0.0 );
				brainDamage.xyz = DeGamma( brainDamage.xxx ) * vec3( 1.0, 0.0, 1.0 );
				brainDamage.w = DeGamma( brainDamage.w );
				vec4 brainDamageSpec = vec4( 0.38 );
				vec4 brainDamagePower = vec4( 0.94 );
				vec4 brainDamageMask = tex2DDeGamma( samp_transmap, vofi_TexCoord0.xy );
				vec4 woundMap = tex2D( samp_woundtexture, vofi_TexCoord7.xy );
				woundMap.xyz -= vec3( 0.5 );
				float bloody = ( 1.0 - vofi_TexCoord6.y ) * 2.0;
				inputs.albedo.xyz = mix( inputs.albedo.xyz, brainDamage.xyz, vec3( vofi_TexCoord6.y ) * brainDamageMask.xyz );
				inputs.normal.xyz = mix( inputs.normal.xyz, brainDamageNormal, vec3( vofi_TexCoord6.y ) * brainDamageMask.xyz );
				inputs.specular.xyz = mix( inputs.specular.xyz, brainDamageSpec.xyz, vec3( vofi_TexCoord6.y ) * brainDamageMask.xyz );
				inputs.smoothness.x = mix( inputs.smoothness.x, mix( brainDamagePower.x, 0.25, brainDamage.x ), vofi_TexCoord6.y );
				veins = brainDamage.w;
				vec3 veinColor = DeGamma( _fa_freqHigh[12 ].xyz );
				inputs.albedo.xyz += vec3( veins ) * veinColor * vec3( 10.0 ) * brainDamageMask.xyz * vec3( vofi_TexCoord6.y * vofi_TexCoord6.y );
				{
					float woundMask = saturate( ( ( 1.0 - saturate( min( mix( bloody, 1.0, 0.8 ), vofi_TexCoord6.x * vofi_TexCoord6.x ) * 0.5 ) ) - ( 1.0 - woundMap.w ) ) * 3.0 );
					vec3 diffuseBloodLerp = mix( inputs.albedo.xyz.xyz, _fa_freqHigh[13 ].xyz, vec3( woundMask ) );
					inputs.albedo.xyz.xyz *= mix( vec3( 1.0 ), _fa_freqHigh[14 ].xyz, vec3( woundMask ) );
						...
						for ( uint lightIdx = lightsMin; lightIdx < lightsMax; ) {
							...
							{ light_t lightParms;
								...
								light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _fa_freqLow[8 ].www );
								{
									...
									vec3 normalR = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[16 ].xxx );
									vec3 normalG = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[16 ].yyy );
									vec3 normalB = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[16 ].zzz );
									vec3 bssrdf = saturate( vec3( dot( normalR, light_vector ), dot( normalG, light_vector ), dot( normalB, light_vector ) ) );
									bssrdf += saturate( _fa_freqHigh[17 ].xyz * ( pow( saturate( dot( inputs.view.xyz, -light_vector.xyz ) ), _fa_freqHigh[17 ].w ) ) * inputs.thickness );
			...
			{
				{ if ( _fa_freqHigh[18 ].x > 0 ) {
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[19 ].zw );
						...
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[20 ].x; if ( _fa_freqHigh[18 ].y >= _fa_freqHigh[20 ].z ) {
							pulsefrequency = _fa_freqHigh[20 ].y;
						}
						float width = mix( _fa_freqHigh[21 ].x, _fa_freqHigh[21 ].y, _fa_freqHigh[18 ].y );
						...
						finalPulse /= _fa_freqHigh[23 ].xyz;
				...
				inputs.diffuse_lighting.xyz.xyz = inputs.diffuse_lighting.xyz;
			};
			{
