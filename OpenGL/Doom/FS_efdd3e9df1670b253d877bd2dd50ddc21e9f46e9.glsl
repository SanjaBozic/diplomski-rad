...
void main() {
	{
		...
		clip( maskDissolve - ( _fa_freqHigh[3 ].x * (1 - mask.w)));
		float lavaGlow = ( moltenMask + lavaEdge ) * mask.y * ( mask.z * 3.0 + 1.0 ) * ( 1.0 - lava.w );
		vec3 lavaGlowColor = ( _fa_freqHigh[4 ].xyz * lavaGlow * _fa_freqHigh[5 ].x ); vec4 lavaSpec = mix( lavaStopped, lavaFlow, vec4( lavaMask ) );
		...
		float gloss = pow( max( lavaSpec.w, saturate( lava.x ) ), 2.2 );
