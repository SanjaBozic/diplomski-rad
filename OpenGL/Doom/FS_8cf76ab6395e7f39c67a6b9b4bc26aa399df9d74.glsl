...
void main() {
	{
		lightingInput_t innerLayer = lightingInput_t( vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, vec3(0,0,0), vec3(0,0,1), vec3(0,0,1), vec3(0,0,0), vec3(0,0,0), 1, 1, vec3(0,0,0), vec3(0,0,0), vec4(0,0,0,0), vec4(0,0,0,0), mat3x3( vec3(1,0,0), vec3(0,1,0), vec3(0,0,1) ), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, 0, 0, uvec4(0,0,0,0), uvec4(0,0,0,0), uvec4(0,0,0,0) );
		{
			...
			{
				inputs.ssdoDiffuseMul = 0;
				mat3x3 invTS = transpose( inputs.invTS );
				vec3 viewTS = normalize( vec3( dot( inputs.view.xyz, invTS[0].xyz), dot( inputs.view.xyz, invTS[1].xyz), dot( inputs.view.xyz, invTS[2].xyz) ) );
				int num_taps = 8;
				vec3 step = vec3( - viewTS.xy, 1.0 ) / float( num_taps );
				vec3 curr = vec3( vofi_TexCoord0.xy, 0 );
				vec2 prev = curr.xy;
				float h = sqr( tex2Dlod( samp_sparediffusemap, vec4( curr.xy, 0, 0 ) ).w ) * _fa_freqHigh[8 ].x;
				float prev_h = h; for( int tap = 0; tap < num_taps && curr.z < h; ++tap ) {
					prev = curr.xy;
					prev_h = h;
					curr += step;
					h = sqr( tex2Dlod( samp_sparediffusemap, vec4( curr.xy, 0, 0 ) ).w ) * _fa_freqHigh[8 ].x;
				}
				float z_after = h - curr.z;
				float z_before = prev_h - curr.z + step.z;
				float w = saturate( z_after / ( z_after - z_before ) );
				vec2 tc_parallax = prev * w + curr.xy * ( 1 - w );
				vec2 tc = vofi_TexCoord0.xy;
				vec4 albedo = tex2DDeGammaApprox( samp_sparediffusemap, tc_parallax );
				inputs.albedo = innerLayer.albedo = albedo.xyz;
				inputs.albedo = mix( inputs.albedo, GetLuma( inputs.albedo ) * _fa_freqHigh[9 ].rgb, saturate( _fa_freqHigh[9 ].w * albedo.www * 8.0 ) );
				inputs.specular = _fa_freqHigh[10 ].xxx;
				inputs.smoothness = _fa_freqHigh[10 ].y;
				inputs.normal = tex2D( samp_sparebumpmap, tc ).xyz;
				innerLayer.specular = _fa_freqHigh[10 ].zzz * GetLuma( innerLayer.albedo );
				innerLayer.smoothness = _fa_freqHigh[10 ].w;
				innerLayer.normal = 1 - tex2Dbias( samp_sparebumpmap, vec4( tc_parallax, 0, 0 ) ).xyz;
				innerLayer.normal = GetWorldSpaceNormal( innerLayer.normal, inputs.invTS, true );
				{
					...
					woundMap.xyz -= vec3( 0.5 );
					{
						...
						vec3 diffuseBloodLerp = mix( inputs.albedo.xyz.xyz, _fa_freqHigh[11 ].xyz, vec3( woundMask ) );
						inputs.albedo.xyz.xyz *= mix( vec3( 1.0 ), _fa_freqHigh[12 ].xyz, vec3( woundMask ) );
						...
						inputs.normal.xy += mix( woundMap.xy * vec2( woundMask ), vec2( 0.0 ), vec2( _fa_freqHigh[12 ].w ) );
					};
				};
			};
			{
				uint skipStaticModel = _fa_freqHigh[13 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
								...
								{
									{
										...
										{
											float wrappedNdotL = saturate( ( dot( innerLayer.normal.xyz, light_vector.xyz ) + 0.25 ) / sqr( 1.0 + 0.25 ) );
											inputs.diffuse_lighting_packed = packRGBE( light_color_final * wrappedNdotL + unpackRGBE( inputs.diffuse_lighting_packed ) );
											vec3 spec = specBRDF( innerLayer.normal, inputs.view, light_vector, innerLayer.specular, innerLayer.smoothness );
											inputs.specular_lighting_packed = packRGBE( spec.xyz * light_spec_multiplier * light_color_final * NdotL + unpackRGBE( inputs.specular_lighting_packed ) );
										}
									};
								};
			...
			{
				float NdotL = saturate( dot( inputs.normal.xyz, inputs.view.xyz ) );
				inputs.specular_lighting += saturate( unpackRGBE( inputs.diffuse_lighting_packed ) ) * NdotL * specBRDF( inputs.normal.xyz, inputs.view, inputs.view, unpackR10G10B10( inputs.specular_packed ), inputs.smoothness );
			};
			{
			...
			out_FragColor0.xyz += _fa_freqLow[11 ].xyz;
		};
	}
}