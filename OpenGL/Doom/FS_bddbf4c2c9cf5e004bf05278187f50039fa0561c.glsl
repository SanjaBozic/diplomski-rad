...
void main() {
	{
		vec4 occlusion = tex2DDeGammaApprox( samp_sparediffusemap, vofi_TexCoord0.xy );
		out_FragColor0 = vec4( vec3( mix( 1.0, occlusion.x, min( _fa_freqHigh[0 ].x, 0.5 ) ) ), 1 );
		out_FragColor3 = vec4( vec3( mix( 1.0, occlusion.w, saturate( _fa_freqHigh[0 ].y ) ) ), 1 );
		out_FragColor1 = vec4( 1 );
		out_FragColor2 = vec4( 1 );
	}
}