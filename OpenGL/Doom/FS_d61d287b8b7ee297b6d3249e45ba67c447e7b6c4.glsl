...
void main() {
	{
		...
		vec3 normal = -normalize( cross( dFdx( vofi_TexCoord1.xyz ), dFdy( vofi_TexCoord1.xyz ) ) );
		float light = saturate( dot( normal, vec3( 0.0, 0.0, -1.0 ) ) ) * 0.7;
