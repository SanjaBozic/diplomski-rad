...
void main() {
	{
		vec4 dMap = tex2D( samp_transmap, vofi_TexCoord0.xy );
		dMap.w *= vofi_Color.w;
		clip( dMap.w - 0.01 );
		...
		clip( zReject - _fa_freqHigh[1 ].x );
		vec2 distortion = 2.0 * dMap.xy - 1.0;
		vec2 scale = ( _fa_freqHigh[2 ].xy * dMap.w * vofi_TexCoord0.z ) * vofi_Color.w;
		float blurScale = ( vofi_TexCoord2.x * vofi_Color.w ) * 5;
		out_FragColor0 = ComputeDistortion( distortion * scale, blurScale );
	}
}