	...
	{
		{
			...
			{
				vec4 degammaColor = DeGamma( _fa_freqHigh[6 ] );
				vec4 degammaColor2 = DeGamma( _fa_freqHigh[8 ] );
				vec2 tc = vofi_TexCoord0.xy;
				vec2 tc_scroll = tc + ( _fa_freqHigh[9 ].xy * vec2( _fa_freqLow[4 ].x ) );
				vec4 noise = tex2DDeGammaApprox( samp_transmap1, tc_scroll );
				float texNoise = ( noise.a * 2 - 1.0 );
				vec2 tc1 = tc + ( texNoise.xx * _fa_freqHigh[9 ].z );
				vec2 tc2 = tc + ( texNoise.xx * _fa_freqHigh[9 ].w );
				vec4 heightTex = tex2DDeGammaApprox( samp_transmap, tc1 );
				float vertMaskFloor = _fa_freqHigh[10 ].x;
				float vertMaskCeiling = _fa_freqHigh[10 ].y;
				float refValue = saturate( ( vofi_Color.r - vertMaskFloor ) / ( vertMaskCeiling - vertMaskFloor ) );
				float maskHeight = heightTex.a;
				float transitionSoftness = _fa_freqHigh[10 ].z;
				float layerMask = saturate( ( refValue - maskHeight ) * transitionSoftness );
				vec4 sampleDiffuse1 = tex2DDeGammaApprox( samp_sparediffusemap, tc1 ) * degammaColor;
				vec4 normal1 = tex2D( samp_sparebumpmap, tc1 );
				vec4 sampleSpecular1 = vec4( 0.24 );
				float samplePower1 = 0.85;
				vec4 normal2 = tex2D( samp_sparebumpmap2, tc2 );
				vec4 sampleSpecular2 = vec4( 0.18 );
				float samplePower2 = 0.19;
				vec4 sampleDiffuse = mix( sampleDiffuse1, degammaColor2, layerMask );
				vec4 normal = mix( saturate( ( normal1 + normal2 - 0.5 ) ), normal2, layerMask );
				vec4 sampleSpecular = mix( sampleSpecular1, sampleSpecular2, layerMask ) * sampleDiffuse.x;
				float samplePower = mix( samplePower1.x, samplePower2.x, layerMask );
				vec2 tc3 = tc + ( _fa_freqHigh[11 ].xy * _fa_freqLow[4 ].x ) + ( texNoise.xx * _fa_freqHigh[9 ].w );
				vec2 tc4 = tc + ( _fa_freqHigh[11 ].zw * _fa_freqLow[4 ].x ) + ( texNoise.xx * _fa_freqHigh[9 ].w );
				float slimeColor1 = tex2D( samp_sparebumpmap2, tc3 ).w;
				float slimeColor2 = tex2D( samp_sparebumpmap2, tc4 ).w;
				vec3 finalAlbedo = sampleDiffuse.xyz;
				vec3 finalSpecular = sampleSpecular.xyz;
				float finalSmoothness = samplePower;
				vec3 finalNormal = normal.xyz;
				inputs.albedo = finalAlbedo;
				inputs.specular = finalSpecular;
				inputs.smoothness = finalSmoothness;
				inputs.normal = finalNormal;
				inputs.emissive = vec3 (0,0,0);
				inputs.alpha = slimeColor1 * slimeColor2 * layerMask * _fa_freqHigh[10 ].w;
			};
			{
				uint skipStaticModel = _fa_freqHigh[12 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
				...
				{
					{
						...
						vec3 ambient = vec3( 0 );
						{
							...
							ambient.xyz += max ( indirectDiffuse.xyz * vec3( 1 ) * saturate( _fa_freqLow[8 ].x ), vec3( 0 ) );
						};
						ambient += inputs.emissive;
						inputs.smoothness = dot( inputs.emissive.xyz, vec3( 1 ) ) > 0.0 ? -inputs.smoothness : inputs.smoothness;
						ambient = mix( GetLuma( ambient.xyz ).xxx, ambient.xyz, _fa_freqLow[9 ].www );
						...
						for ( uint lightIdx = lightsMin; lightIdx < lightsMax; ) {
							...
							{ light_t lightParms;
								...
								light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _fa_freqLow[9 ].www );
								{
			...
			{
				vec4 degammaColor3 = DeGamma( _fa_freqHigh[13 ] );
				inputs.diffuse_lighting.xyz = inputs.diffuse_lighting.xyz + ( inputs.alpha * degammaColor3.xyz );
			};
			{
