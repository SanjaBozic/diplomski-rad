	...
	{
		{
			...
			float phase1 = sin( _va_freqLow[2 ].x * 4.0 ) * 0.5 + 0.5;
			float phase2 = sin( _va_freqLow[2 ].x * 4.0 + 2.0 ) * 0.5 + 0.5;
			float phase3 = sin( _va_freqLow[2 ].x * 4.0 + 4.0 ) * 0.5 + 0.5;
			localPosition.xyz += localNormal.xyz * vec3( phase1 * 4.0 ) * vertexColor.xxx * _va_freqHigh[0 ].x;
			localPosition.xyz += localNormal.xyz * vec3( phase2 * 2.0 ) * vertexColor.yyy * _va_freqHigh[0 ].x;
			localPosition.xyz += localNormal.xyz * vec3( phase3 * 4.0 ) * vertexColor.zzz * _va_freqHigh[0 ].x;
			{
		...
		vec3 worldPos = vofi_TexCoord4.xyz + _va_freqLow[3 ].xyz;
		{
