...
layout( binding = 0, r8 ) uniform image2D ib_uavssdo ;
	...
	{
		{
			...
			{
				{
					bool taaEnabled = _ca_freqHigh[1 ].x == 1.0 ? true : false;
					int numRays = 8;
					int numRaysOuter = 6;
					float accNormalization = 1.0 / float( numRays - numRaysOuter );
					float accOuterNormalization = 1.0 / float( numRaysOuter );
					float angle_bias = _ca_freqHigh[2 ].z;
					float position_bias = _ca_freqHigh[2 ].w;
					resultColor = vec4( 1 );
					float ndcZ = tex2Dlod( samp_viewdepthmap, vec4( tc.xy, 0, 0 ) ).x;
					float linearZ = GetLinearDepth( ndcZ, _ca_freqLow[0 ], 1.0, false );
					float distAtten = saturate( ( _ca_freqHigh[1 ].y - linearZ ) / 256.0 );
					vec4 n1 = vec4( NormalOctDecode( tex2Dlod( samp_tex0, vec4( tc.xy, 0, 0 ) ).xy, false ).xyz, SmoothnessDecode( tex2Dlod( samp_tex3, vec4( tc.xy, 0, 0 ) ).w ).y ); if ( distAtten * n1.w > 0.0 ) {
						float firstPersonArmsZ = GetNdcDepth( 3.5, _ca_freqLow[0 ] );
						float radii = ( ndcZ < firstPersonArmsZ ) ? _ca_freqHigh[2 ].w : _ca_freqHigh[2 ].y;
						float radii_inner = ( ndcZ < firstPersonArmsZ ) ? _ca_freqHigh[2 ].z : _ca_freqHigh[2 ].x;
						float max_radius_ws = 1.0 / radii;
						float normalBias = ( ndcZ < firstPersonArmsZ )? _ca_freqHigh[1 ].w * 0.5 : _ca_freqHigh[1 ].w;
						ndcZ = ( ndcZ < firstPersonArmsZ ) ? ndcZ * 10 : ndcZ;
						vec3 normalM;
						vec3 n2 = NormalOctDecode( tex2Dlod( samp_tex0, vec4( tc.xy + 0.5 * _ca_freqHigh[3 ].zw * vec2( 1.0, 0.0 ), 0, 0 ) ).xy, false );
						vec3 n3 = NormalOctDecode( tex2Dlod( samp_tex0, vec4( tc.xy + 0.5 * _ca_freqHigh[3 ].zw * vec2( 0.0, 1.0 ), 0, 0 ) ).xy, false );
						vec3 n4 = NormalOctDecode( tex2Dlod( samp_tex0, vec4( tc.xy + 0.5 * _ca_freqHigh[3 ].zw * vec2( 1.0, 1.0 ), 0, 0 ) ).xy, false );
						vec3 gbufN = ( n1.xyz + n2 + n3 + n4 ) / 4.0 ;
						normalM = gbufN.xyz;
						mat3x3 invProjT = mat3x3( _ca_freqLow[1 ].xyz, _ca_freqLow[2 ].xyz, _ca_freqLow[3 ].xyz );
						vec3 normalVS = normalize( MatrixMul( invProjT, normalM.xyz ) );
						mat3x3 basisN = BuildBasis( normalVS );
						ndcZ -= _ca_freqHigh[1 ].z;
						vec3 viewPosOrig = GetViewPos( vec3( tc.xy / _ca_freqLow[4 ].xy, ndcZ ), _ca_freqLow[5 ], _ca_freqLow[6 ] );
						viewPosOrig.xyz += normalVS.xyz * normalBias;
						ivec2 pix_idx = ivec2( ivec2(gl_GlobalInvocationID.xy ).xy.xy + ( 0*ivec2( _ca_freqLow[7 ].xy ) ) ) & 1;
						int idx_pix = pix_idx.x + pix_idx.y * 2;
						vec3 noiseVec = ( vec3( Hammersley2D_4bits( 4, ( idx_pix + int( taaEnabled ? _ca_freqHigh[4 ].x : 0 ) ) & 3 ) * 2.0 - 1.0, 0 ) );
						...
						float unoccl_ratio = 0; for (int t = 0; t < numRays; t++ ) {
							...
							vec3 winPosTap;
							winPosTap.xy = GetWindowPos( viewPosHemisphere, _ca_freqLow[8 ] ); if ( t < numRaysOuter ) {
								winPosTap.z = 1 - tex2Dlod( samp_tex2, vec4( winPosTap.xy * _ca_freqLow[4 ].xy, 0, 0 ) ).x;
							} else {
								winPosTap.z = 1 - tex2Dlod( samp_tex1, vec4( winPosTap.xy * _ca_freqLow[4 ].xy, 0, 0 ) ).x;
							}
							vec3 viewPosTap = GetViewPos( winPosTap, _ca_freqLow[5 ], _ca_freqLow[6 ] );
						...
						resultColor.x = saturate( 1 - unoccl_ratio * distAtten );
					}
				}
			};
			imageStore( ib_uavssdo, ivec2(gl_GlobalInvocationID.xy ).xy, resultColor );
		};
	}
}