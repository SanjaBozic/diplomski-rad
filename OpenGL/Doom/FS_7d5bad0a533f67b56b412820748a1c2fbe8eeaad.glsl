...
void main() {
	{
		...
		vec4 emissive = tex2DDeGammaApprox( samp_transmap2, distortedEmissiveTC);
		...
		vec4 final = _fa_freqHigh[1 ] * _fa_freqHigh[2 ] * emissive * mask;
		vec2 screenTC = screenPosToTexcoord(gl_FragCoord.xy, _fa_freqHigh[3 ]);
		float depth = tex2Ddepth( samp_viewdepthmap, screenTC); if ( _fa_freqHigh[4 ].x != 0.0)
		{
			float dstDepth = 1.0 / (1.0 - depth);
			float srcDepth = 1.0 / (1.0 - gl_FragCoord.z);
			float depthFade = saturate((dstDepth - srcDepth) * _fa_freqHigh[5 ].x);
			final *= depthFade;
		} if ( _fa_freqHigh[4 ].y != 0.0)
		{
			vec3 fromViewer = normalize(vofi_TexCoord4.xyz);
			vec3 globalNormal = normalize(vofi_TexCoord5.xyz);
			float edgeFade = saturate(abs(dot(globalNormal, fromViewer)));
			final *= pow(edgeFade, _fa_freqHigh[5 ].y);
		}
