...
void main() {
	{
		...
		vofi_TexCoord0.x = dot4( in_TexCoord, _va_freqHigh[5 ] );
		vofi_TexCoord0.y = dot4( in_TexCoord, _va_freqHigh[6 ] );
		vofi_TexCoord0.z = -( clamp( gl_Position.w - _va_freqLow[2 ].x, 0.0, _va_freqLow[2 ].y ) * _va_freqLow[2 ].z );
		vofi_TexCoord0.w = 0.0;
		vofi_TexCoord1.xy = in_TexCoord.xy;
