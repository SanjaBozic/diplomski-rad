...
void main() {
	{
		...
		vec3 degammaColor2 = SRGBlinearApprox( _fa_freqHigh[1 ].xyz );
		vec3 degammaColor3 = SRGBlinearApprox( _fa_freqHigh[2 ].xyz );
		vec3 fireMask = SRGBlinearApprox( albedo * 1.25 );
		float mask = dot3( fireMask.xyz, vec3( 1.0 ) );
		albedo = SRGBlinearApprox( albedo );
		vec3 unlit = albedo;
		albedo = mix( albedo, albedo * degammaColor3, albedo * albedo );
		float opacity = mix( curr_frame.y , next_frame.y , vofi_TexCoord1.z );
		...
		float opacitySoftZScale = 1.0;
		{
			...
			opacitySoftZScale = saturate( mix( opacitySoftZScale, opacitySoftZScale * 500.0, 0.0 ) );
		};
		out_FragColor0.a = opacity * opacitySoftZScale;
		vec3 emissive = albedo * vofi_TexCoord2.w;
		vec2 atlasTC = vec2( vofi_TexCoord1.x, vofi_TexCoord3.x );
		vec3 diffuse_lighting = tex2DBicubic( samp_particleslightatlasmap, atlasTC, _fa_freqLow[0 ].xy ).xyz;
		...
		float flameBits = saturate( ( atan( mask * 19.0 - 14 ) + 1.5 ) * 0.348 );
		vec3 fireTint = mix( degammaColor3, degammaColor2, flameBits );
		out_FragColor0.rgb = mix( color, mix( fogColor * vec3( _fa_freqLow[5 ].x ), unlit * fireTint * _fa_freqHigh[3 ].x, fogAmount ), vec3( flameBits ) );
		{
