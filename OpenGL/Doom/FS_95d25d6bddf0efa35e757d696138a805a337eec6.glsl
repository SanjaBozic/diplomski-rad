	...
	{
		{
			...
			} ;
			{
				...
				inputs.output_lighting.xyz = inputs.diffuse_lighting * inputs.alpha + inputs.specular_lighting;
				vec3 fogColor;
				float fogAmount;
				{
					float FLT_EPSILON = 0.0001;
					vec3 fogLightInscatterScaledColor = _fa_freqLow[8 ].xyz * _fa_freqLow[9 ].x;
					vec3 heightFogColor = mix( fogLightInscatterScaledColor, _fa_freqLow[10 ].xyz, vofi_TexCoord5.z );
				...
				inputs.output_lighting.xyz.xyz = mix( fogColor * vec3( _fa_freqLow[12 ].x ) * fogScale, inputs.output_lighting.xyz.xyz, fogAmount );
			};
