...
void main() {
	{
		vec4 texA = tex2Dlod( samp_transmap, vec4( fract( vofi_TexCoord1.xy ) * _fa_freqHigh[0 ].xy + _fa_freqHigh[0 ].zw, 0.0, 0.0 ) );
		vec4 texB = tex2Dlod( samp_transmap, vec4( fract( vofi_TexCoord1.zw ) * _fa_freqHigh[0 ].xy + _fa_freqHigh[0 ].zw, 0.0, 0.0 ) );
		vec4 maskTexture = tex2D( samp_transmap1, vec2( vofi_TexCoord2.y, 0.5 ) );
		vec4 final = mix( texA, texB, vec4( vofi_TexCoord2.x ) );
		final *= vofi_Color * _fa_freqHigh[1 ];
		final.xyz *= vec3( _fa_freqHigh[1 ].w ) * _fa_freqHigh[2 ].x;
		final *= maskTexture;
		...
		float srcDepth = 1.0 / ( 1.0 - gl_FragCoord.z );
		float edge = saturate( ( dstDepth - srcDepth ) / 6.0 );
		out_FragColor0.w *= edge;
		{
