...
void main() {
	{
		...
		vec4 hexagons = tex2D( samp_transmap1, screenTC * vec2( 15, 12 ) );
		vec4 final;
		{
		...
		float flicker = saturate( fract( hexagons.w + _fa_freqLow[0 ].x * 2.0 ) );
		flicker = pow( flicker, 5.0 );
		flicker *= hexagons.z;
		flicker += pow( final.w, 10.0 );
		flicker *= 5.0;
		out_FragColor0.xyz = final.xyz * vec3( flicker ) * vofi_Color.xyz;
