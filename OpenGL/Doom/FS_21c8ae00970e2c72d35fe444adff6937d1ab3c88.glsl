...
void main() {
	{
		...
		float maxDistanceSqr = 3000.0 * 3000.0;
		...
		float tex_02 = tex2D( samp_transmap, tc + vec2( -translate.x * 0.7, -translate.y ) ).x;
		...
		clip( zReject - 0.3 );
		float alphaScanFinal = step( tc.y - ( ( _fa_freqHigh[4 ].x * 0.4 ) + 0.3 ), 0.0 );
		out_FragColor0 = outline * _fa_freqHigh[5 ] * alphaScanFinal;
	}
}