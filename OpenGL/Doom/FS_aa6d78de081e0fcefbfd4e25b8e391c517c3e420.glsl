...
void main() {
	{
		...
		float diameter = ( vofi_TexCoord1.z / 6.28 ) * vofi_TexCoord1.x;
		float edgeScale = 1.0 - saturate( ( srcDepth - dstDepth ) / diameter );
		float viewScale = saturate( ( dstDepth - srcDepth ) / 32 );
		float viewFalloff = saturate( srcDepth * 0.05 );
		vec4 final = vofi_Color;
		final.w *= edgeScale * viewScale * viewFalloff * vofi_TexCoord1.w * vofi_TexCoord1.y;
		vec3 fogColor;
		float fogAmount;
		{
		...
		final.xyz = mix( fogColor, final.xyz, vec3( fogAmount ) );
		out_FragColor0 = final;
		out_FragColor0.w *= _fa_freqHigh[1 ].x;
		{
