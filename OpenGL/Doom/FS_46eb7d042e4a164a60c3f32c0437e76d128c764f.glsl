	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				{ if ( _fa_freqHigh[12 ].x > 0.0 ) {
						...
						float primaryColorMask = RangeExpand( inputs.colorMask.x, 130.0 / 255.0, 1.0 );
						float bloomMask = 1.0 - RangeExpand( inputs.colorMask.x, 0.0, 125.0 / 255.0 );
						float secondaryColorMask = RangeExpand( inputs.colorMask.z, 130.0 / 255.0, 1.0 );
						float patternColorMask = 1.0 - RangeExpand( inputs.colorMask.z, 0.0, 125.0 / 255.0 );
						float allRegionsCombinedMask = saturate( max( max( primaryColorMask, secondaryColorMask ), patternColorMask ) ); if ( _fa_freqHigh[13 ].z > 0.0 ) {
							vec3 normalMap = tex2Dlod( samp_tilingnormalmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[14 ].xy, 0, 0 ) ).xyz;
							vec3 tilingNormal;
							...
							customNormal = mix( customNormal, tilingNormal, vec3( ( 1.0 - patternColorMask ) * primaryColorMask ) * _fa_freqHigh[13 ].z );
						} if ( _fa_freqHigh[13 ].w > 0.0 ) {
							vec3 normalMap2 = tex2Dlod( samp_tilingnormalmap2, vec4( vofi_TexCoord0.xy * _fa_freqHigh[14 ].zw, 0, 0 ) ).xyz;
							vec3 tilingNormal2;
							...
							customNormal = mix( customNormal, tilingNormal2, vec3( ( 1.0 - patternColorMask ) * secondaryColorMask ) * _fa_freqHigh[13 ].w );
						}
						float tilingSmoothness = tex2Dlod( samp_tilingscratchdirtmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[13 ].xy, 0, 0 ) ).z;
						customAlbedo = mix( customAlbedo, _fa_freqHigh[15 ].xyz, primaryColorMask );
						customSpecular = mix( customSpecular, _fa_freqHigh[16 ].xyz, vec3( primaryColorMask ) );
						float primaryColorSmoothness = mix( _fa_freqHigh[15 ].w, ColorOverlay( _fa_freqHigh[15 ].w, tilingSmoothness ), _fa_freqHigh[16 ].w );
						customSmoothness = mix( customSmoothness, primaryColorSmoothness, primaryColorMask );
						customAlbedo = mix( customAlbedo, _fa_freqHigh[17 ].xyz, secondaryColorMask );
						customSpecular = mix( customSpecular, _fa_freqHigh[18 ].xyz, vec3( secondaryColorMask ) );
						float secondaryColorSmoothness = mix( _fa_freqHigh[17 ].w, ColorOverlay( _fa_freqHigh[17 ].w, tilingSmoothness ), _fa_freqHigh[18 ].w );
						customSmoothness = mix( customSmoothness, secondaryColorSmoothness, secondaryColorMask ); if ( _fa_freqHigh[19 ].x > 0.0 ) {
							float patternMap = tex2Dlod( samp_tilingpatternmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[19 ].xy, 0, 0 ) ).x;
							customAlbedo = mix( customAlbedo, mix( _fa_freqHigh[20 ].xyz, _fa_freqHigh[21 ].xyz, patternMap ), patternColorMask );
							vec3 pattern1Specular = mix( _fa_freqHigh[22 ].xyz, _fa_freqHigh[23 ].xyz, patternMap );
							customSpecular = mix( customSpecular, pattern1Specular, vec3( patternColorMask ) );
							float pattern1Smoothness = mix( _fa_freqHigh[20 ].w, _fa_freqHigh[21 ].w, patternMap );
							float pattern1SmoothnessScale = mix( _fa_freqHigh[22 ].w, _fa_freqHigh[23 ].w, patternMap );
							pattern1Smoothness = mix( pattern1Smoothness, ColorOverlay( pattern1Smoothness, tilingSmoothness ), pattern1SmoothnessScale );
							customSmoothness = mix( customSmoothness, pattern1Smoothness, patternColorMask );
						}
						float scratchMask = 0.0; if ( _fa_freqHigh[24 ].x > 0.0 && _fa_freqHigh[25 ].w > 0.0 ) {
							float scratchMap = tex2Dlod( samp_tilingscratchdirtmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[24 ].xy, 0, 0 ) ).x;
							scratchMask = clamp( curvatureMask - 0.5, 0.0, 0.5 ) * 2.0;
							scratchMask = pow( scratchMask, _fa_freqHigh[25 ].w );
							float finalScratchMask = ColorBurn( scratchMask, scratchMap );
							finalScratchMask *= allRegionsCombinedMask;
							customAlbedo = mix( customAlbedo, _fa_freqHigh[25 ].xyz, finalScratchMask );
							customSpecular = mix( customSpecular, _fa_freqHigh[26 ].xyz, vec3( finalScratchMask ) );
							customSmoothness = mix( customSmoothness, _fa_freqHigh[26 ].w, finalScratchMask );
						}
						float dirtMask = 0.0;
						float finalDirtMask = 0.0; if ( _fa_freqHigh[24 ].z > 0.0 && _fa_freqHigh[27 ].w > 0.0 ) {
							float dirtMap = tex2Dlod( samp_tilingscratchdirtmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[24 ].zw, 0, 0 ) ).y;
							dirtMask = 1.0 - ( clamp( curvatureMask, 0.0, 0.5 ) * 2.0 );
							dirtMask = mix( 0.0, dirtMask, pow( 0.8, _fa_freqHigh[27 ].w ) );
							finalDirtMask = ColorDodge( dirtMask, dirtMap );
							finalDirtMask = pow( finalDirtMask, _fa_freqHigh[27 ].w );
							customAlbedo = mix( customAlbedo, _fa_freqHigh[27 ].xyz, finalDirtMask ) ;
							customSpecular = mix( customSpecular, _fa_freqHigh[28 ].xyz, vec3( finalDirtMask ) );
							customSmoothness = mix( customSmoothness, _fa_freqHigh[28 ].w, finalDirtMask );
						}
						inputs.albedo = customAlbedo;
						inputs.smoothness = customSmoothness;
						inputs.specular = customSpecular;
						inputs.emissive = ( vec3( bloomMask ) * _fa_freqHigh[29 ].xyz ) * _fa_freqHigh[29 ].w;
				...
				};
				{
					...
					woundMap.xyz -= vec3( 0.5 );
					{
						...
						vec3 diffuseBloodLerp = mix( inputs.albedo.xyz.xyz, _fa_freqHigh[30 ].xyz, vec3( woundMask ) );
						inputs.albedo.xyz.xyz *= mix( vec3( 1.0 ), _fa_freqHigh[31 ].xyz, vec3( woundMask ) );
						...
						inputs.normal.xy += mix( woundMap.xy * vec2( woundMask ), vec2( 0.0 ), vec2( _fa_freqHigh[31 ].w ) );
					};
				};
			};
			{
				uint skipStaticModel = _fa_freqHigh[32 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
						...
						for ( uint lightIdx = lightsMin; lightIdx < lightsMax; ) {
							...
							{ light_t lightParms;
								...
								light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _fa_freqLow[8 ].www );
								{
									...
									vec3 normalR = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[33 ].xxx );
									vec3 normalG = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[33 ].yyy );
									vec3 normalB = mix( inputs.normal, inputs.normalSSS, _fa_freqHigh[33 ].zzz );
									vec3 bssrdf = saturate( vec3( dot( normalR, light_vector ), dot( normalG, light_vector ), dot( normalB, light_vector ) ) );
									bssrdf += saturate( _fa_freqHigh[34 ].xyz * ( pow( saturate( dot( inputs.view.xyz, -light_vector.xyz ) ), _fa_freqHigh[34 ].w ) ) * inputs.thickness );
			...
			{
				{ if ( _fa_freqHigh[35 ].x > 0 ) {
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[36 ].zw );
						...
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[37 ].x; if ( _fa_freqHigh[35 ].y >= _fa_freqHigh[37 ].z ) {
							pulsefrequency = _fa_freqHigh[37 ].y;
						}
						float width = mix( _fa_freqHigh[38 ].x, _fa_freqHigh[38 ].y, _fa_freqHigh[35 ].y );
						float zValue = ( vofi_TexCoord4.xyz.z ) * width;
						float pulse = sin( zValue + _fa_freqLow[11 ].x * pulsefrequency ) * magnitude + magnitude;
						float power = _fa_freqHigh[38 ].z;
						pulse = pow( pulse, power );
						pulse *= fresnel;
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[39 ].xyz * 2.0;
						finalPulse /= _fa_freqHigh[40 ].xyz;
				...
				{ if ( _fa_freqHigh[41 ].w > 0 ) {
						vec4 tiling = vec4( 0.003, 0.01, 0.05, 0.3 );
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[36 ].zw );
						vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy * 1.5, _fa_freqHigh[3 ] ) * ( resolution / 32.0 );
						vec4 electric = tex2Dlod( samp_highlightmap, vec4( screenTexCoord * tiling.xy + vec2( _fa_freqLow[11 ].x ) * tiling.zw, 0.0, 0.0 ) );
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						float pulse = 2.0;
						pulse *= sin( _fa_freqLow[11 ].x * 5.0 ) * 0.5 + 0.7;
						pulse *= pulse;
						pulse *= fresnel;
						fresnel *= fresnel;
						...
						vec3 finalPulseFresnel = saturate( digitalNoise.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[41 ].xyz;
						vec3 finalPulseFlat = saturate( digitalNoise.x ) * _fa_freqHigh[41 ].xyz;
						vec3 finalPulse = mix( finalPulseFresnel, finalPulseFlat, vec3( 0.25 ) );
						vec3 constantGlow = saturate( 0.5 * ( fresnel ) + edgeGlow ) * _fa_freqHigh[41 ].xyz;
						finalPulse /= _fa_freqHigh[40 ].xyz;
						vec3 finalPulseColor = finalPulse * _fa_freqHigh[41 ].w;
						float saturation = ( _fa_freqLow[12 ].x == 0 ) ? 1.0 : 0.9;
						float blendScale = ( _fa_freqLow[12 ].x == 0 ) ? 1.0 : 0.2;
						inputs.diffuse_lighting.xyz.xyz += mix( GetLuma( finalPulseColor ).xxx, finalPulseColor, saturation.xxx ) * blendScale;
					}
				}; if ( _fa_freqHigh[42 ].x * ( 1.0 - _fa_freqHigh[43 ].x ) > 0.0 ) {
					float teamfxID = _fa_freqHigh[44 ].x == 1.0 ? 0.0 : 1.0;
					vec3 viewDir = normalize( -vofi_TexCoord4.xyz );
					float distance = length( vofi_TexCoord4.xyz );
					{
						float fresnel = saturate( 1.0 - dot( inputs.normal.xyz.xyz, viewDir.xyz ) );
						float fadeStart = _fa_freqHigh[45 ].x;
						float fadeEnd = _fa_freqHigh[46 ].x;
						float fadeRange = max( 1.0, fadeEnd - fadeStart );
						float value = max( 0.0, distance - fadeStart );
						float alpha = saturate( value / fadeRange );
						fresnel = saturate( pow( fresnel, _fa_freqHigh[47 ].x ) * _fa_freqHigh[48 ].y );
						vec3 rimLightColor = ( 1.0 * _fa_freqHigh[48 ].x ) * _fa_freqHigh[49 ].xyz;
						vec3 finalRimlight = fresnel * rimLightColor;
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz + finalRimlight, alpha * teamfxID * _fa_freqHigh[50 ].x );
					};
				}
				{ if ( _fa_freqHigh[35 ].x > 0.0 ) {
						vec2 resolution = abs( vec2( 1.0 ) / _fa_freqHigh[36 ].zw );
						vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[3 ] ) * ( resolution / vec2( 32.0 ) );
						vec4 dither = tex2Dlod( samp_echodithermap, vec4( screenTexCoord, 0.0, 0.0 ) );
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						float pulseTime = mix( 10.0, 30.0, _fa_freqHigh[35 ].y );
						float pulse = sin( ( vofi_TexCoord4.xyz.z + _fa_freqLow[0 ].z ) * 0.1 - _fa_freqLow[11 ].x * pulseTime ) * 0.5 + 0.5;
						pulse = pow( pulse, 5.0 );
						pulse *= fresnel;
						fresnel = pow( fresnel, 3.0 );
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = vec3( 0.0 );
						finalPulse = saturate( dither.x * ( fresnel + pulse ) + edgeGlow ) * _fa_freqHigh[39 ].xyz * 2.0;
						inputs.diffuse_lighting.xyz.xyz += finalPulse;
					}
				};
				{ if ( _fa_freqHigh[43 ].x > 0.0 ) {
						vec4 colorWave = tex2DlodDeGamma( samp_transmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[51 ].xy + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[51 ].zw, 0.0, 0.0) );
						...
						vec3 close = crevices * crevices * vec3( 180.0 ) * _fa_freqHigh[52 ].xyz + finalOutline * _fa_freqHigh[52 ].xyz;
						vec3 far = _fa_freqHigh[52 ].xyz * vec3( 0.1 ) * colorWave.xyz * vec3( fresnel2 ) + vec3( fresnel2 ) * _fa_freqHigh[52 ].xyz * vec3( 20.0 );
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz, ( inputs.diffuse_lighting.xyz + mix( close, far, vec3( farFade ) ) * colorWave.xyz ) + _fa_freqHigh[52 ].xyz * vec3( 0.01 ) * vec3( fresnel2 ), vec3( _fa_freqHigh[43 ].x ) );
					}
				};
				{ if ( _fa_freqHigh[53 ].x > 0.0 ) {
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						inputs.diffuse_lighting.xyz.xyz += ( ( _fa_freqHigh[54 ].xyz * 0.001 ) * _fa_freqHigh[54 ].w * _fa_freqHigh[55 ].x ) * fresnel * fresnel * 0.8;
					}
				};
				{ if ( _fa_freqHigh[56 ].x == 1.0 ) { if ( _fa_freqHigh[57 ].x == 1.0 ) {
							vec4 shieldPattern = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[51 ].xy ) * 3.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[48 ].zw, 0.0, 0.0 ) );
							vec4 shieldPattern2 = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[51 ].xy ) * 2.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[51 ].zw, 0.0, 0.0 ) );
							shieldPattern.xyz = ( shieldPattern.xxx * shieldPattern2.yyy ) * 900.0;
							vec3 electricValue = mix( inputs.diffuse_lighting.xyz * vec3( 0.0, 3.0, 0.0 ) * vec3( 5.0 ), inputs.diffuse_lighting.xyz + shieldPattern.xyz * vec3( 0.0, 7.0, 0.0 ), 0.05 );
							inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, ( electricValue * inputs.diffuse_lighting.xyz.x ), 0.75 );
							inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 1.0, 0.25 ) * vec3( 50.0 ), _fa_freqHigh[57 ].w );
						}
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.5, 1.0, 0.5 ) * vec3( 50.0 ), _fa_freqHigh[57 ].y );
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 0.0, 0.0 ) * vec3( 50.0 ), _fa_freqHigh[57 ].z );
					}
				};
				{ if ( _fa_freqHigh[58 ].x > 0.0 ) {
						vec4 mainColor = vec4(38,177,255,255) * 0.01;
						vec2 noiseAnim = vec2 (1.0, 0.1);
						vec2 noiseTiling = vec2 (0.5,0.7);
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[36 ].zw );
						vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy * 1.5, _fa_freqHigh[3 ] ) * ( resolution / 32.0 );
						vec4 noise = tex2Dlod( samp_echodithermap, vec4( (screenTexCoord.xy * noiseTiling) + vec2( fract( _fa_freqLow[11 ].x ) ) * noiseAnim, 0.0, 0.0 ) );
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						fresnel = pow( fresnel, 1.5 );
						noise.xyz = pow( noise.xyz, vec3( 3.0 ) );
						float pulse = .25;
						pulse *= sin( _fa_freqLow[11 ].x * 90.0 ) * 0.5 + 0.7;
						pulse *= fresnel;
						vec3 holoColor = inputs.diffuse_lighting.xyz.xyz + mainColor.xyz * pulse;
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, holoColor.xyz, _fa_freqHigh[58 ].x );
						inputs.diffuse_lighting.xyz.xyz = mix (inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz * noise.xyz * 2.3, _fa_freqHigh[58 ].x);
					}
				};
			};
			{
