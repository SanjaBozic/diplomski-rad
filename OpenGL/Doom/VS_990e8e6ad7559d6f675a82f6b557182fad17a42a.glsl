...
void main() {
	{
		...
		vec4 v;
		v.x = dot( localPosition, _va_freqHigh[5 ] );
		v.y = dot( localPosition, _va_freqHigh[6 ] );
		v.z = dot( localPosition, _va_freqHigh[7 ] );
		v.w = dot( localPosition, _va_freqHigh[10 ] );
		vofi_TexCoord10.x = dot( v, _va_freqHigh[11 ] );
		vofi_TexCoord10.y = dot( v, _va_freqHigh[12 ] );
		vofi_TexCoord10.z = dot( v, _va_freqHigh[13 ] );
		vofi_TexCoord10.w = dot( v, _va_freqHigh[14 ] );
	}
}