...
void main() {
	{
		...
		vec2 step = vec2( 1.0, 0.0 ) * ( _fa_freqHigh[0 ].zw * _fa_freqLow[0 ].xy );
		float w[ 15 ] = { 0.06165, 0.063728, 0.065541, 0.067062, 0.06827, 0.069146, 0.069677, 0.069855, 0.069677, 0.069146, 0.06827, 0.067062, 0.065541, 0.063728, 0.06165 };
		vec3 sum = vec3(0,0,0); for ( int x = -7; x <= 7; x++ ) {
			int ndx = x + 7;
			sum += max( tex2Dlod( samp_rendercolorafterpost, vec4( tc + step * float(x), 0, 0 ) ).xyz, vec3( 0 ) ) * w[ ndx ];
		}
