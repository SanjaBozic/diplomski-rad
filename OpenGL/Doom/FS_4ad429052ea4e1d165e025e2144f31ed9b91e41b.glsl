...
void main() {
	{
		vec2 texcoord = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[0 ], _fa_freqLow[0 ] );
		{
			...
			vec4 accFar = vec4( 0 ); for ( int t = 0; t < num_taps; t++ ) {
				vec3 c = tex2Dlod( samp_tex0, vec4( texcoord.xy + _fa_freqHigh[1 ].zw * taps[ t ], 0, 0 ) ).xyz;
				float dz = tex2Dlod( samp_viewdepthmap, vec4( _fa_freqLow[0 ].zw * ( texcoord.xy + _fa_freqHigh[1 ].zw * taps[ t ] ), 0, 0 ) ).x;
				float lz = GetLinearDepth( dz, _fa_freqLow[1 ], 1.0, true );
				float cocFar = 0.0;
				cocFar = saturate( ( lz - _fa_freqHigh[2 ].x ) / ( _fa_freqHigh[2 ].z - _fa_freqHigh[2 ].x ) );
				accFar += vec4( c.xyz * dofTargetScale, 1 ) * cocFar;
			}
			out_FragColor0 = max( accFar / float( num_taps ), vec4( 0 ) );
		};
	}
}