...
void main() {
	{
		...
		vec4 sampleDiffuse = tex2DDeGammaApprox( samp_transmap1, tc );
		vec4 normal = tex2D( samp_transmap2, tc2 );
		vec4 sampleSpecular = mix( _fa_freqHigh[2 ].xxxx, _fa_freqHigh[2 ].yyyy, tex2DDeGammaApprox( samp_transmap3, tc ) );
		vec4 samplePower = mix( _fa_freqHigh[2 ].zzzz, _fa_freqHigh[2 ].wwww, tex2DDeGammaApprox( samp_transmap4, tc ) );
		vec3 globalNormal;
		...
		float fresnelMain = mix( _fa_freqHigh[5 ].x, _fa_freqHigh[5 ].y, pow( fresnel, _fa_freqHigh[5 ].z ) );
		vec4 colBlue = vec4(0.05, 0.3, 1.0, 0.0);
		vec4 colRed = vec4(0.85, 0.025, 0.015, 0.0);
		vec4 coreColor = mix(colBlue, colRed, _fa_freqHigh[6 ].x) * _fa_freqHigh[7 ].w;
		float rimFresnel = saturate( mix( _fa_freqHigh[7 ].x, _fa_freqHigh[7 ].y, pow( 1 - fresnel, _fa_freqHigh[7 ].z ) ) );
		float rimBlink = _fa_freqLow[8 ].x ;
		vec2 blinkTime = vec2( mix(1.0, 40.0, saturate(sin( rimBlink * 8.0 * _fa_freqHigh[8 ].x ))) );
		vec4 finalRimlight = rimFresnel * (coreColor * blinkTime.xyxy);
		float iceOpacity = saturate( fresnelMain.x * sampleDiffuse.x );
		out_FragColor0.xyz = (lightInput.diffuse_lighting.xyz * _fa_freqHigh[9 ].xyz + finalRimlight.xyz) + lightInput.specular_lighting.xyz;
		out_FragColor0.w = saturate(iceOpacity * sampleDiffuse.x * _fa_freqHigh[10 ].x) ;
	}
}