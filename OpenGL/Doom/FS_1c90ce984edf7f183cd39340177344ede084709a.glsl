...
void main() {
	{
		...
		vec2 maskDissolve_tc = tc * _fa_freqHigh[0 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[0 ].zw;
		float maskDissolve = tex2DDeGammaApprox( samp_transmap, maskDissolve_tc.xy).w;
		float maskAmount = mask.w;
		float dissolveEdgeWidth = _fa_freqHigh[1 ].y;
		vec2 tcN = (vofi_TexCoord0.xy * _fa_freqHigh[2 ].xy) + ( _fa_freqHigh[2 ].zw * _fa_freqLow[0 ].x );
		vec2 tcF = (vofi_TexCoord0.xy * _fa_freqHigh[3 ].xy) + ( _fa_freqHigh[3 ].zw * _fa_freqLow[0 ].x );
		vec4 foamDiffuse = tex2DDeGammaApprox( samp_transmap, tcF.xy);
		float foamColor = mask.y;
		float foamMask = mix( foamColor * foamDiffuse.x, foamColor, foamColor);
		vec4 normal1 = tex2D( samp_sparebumpmap, tcN);
		vec4 foamNorm = tex2D( samp_sparebumpmap2, tcF);
		float texNoise = ((normal1.x + foamNorm.x) );
		vec2 tcS = (tc * _fa_freqHigh[4 ].xy + ( _fa_freqHigh[5 ].zw * _fa_freqLow[0 ].x ))+ ( texNoise.xx * _fa_freqHigh[4 ].zw );
		vec4 sampleSpecular1 = tex2D( samp_sparespecularmap, tcS );
		vec4 normal = mix(saturate( normal1 + foamNorm - 0.5 ), normal1, foamMask);
		vec4 sampleSpecular = mix( vec4( 0.6 )* foamDiffuse.xxxx, mix ( vec4( 0.30 ),vec4( 0.05 ), sampleSpecular1.x) , foamMask) ;
		float samplePower = mix( 1.05, mix ( 0.9, 0.2, sampleSpecular1.x), 1 - foamColor) ;
		vec4 sampleDiffuse = mix((foamDiffuse.yyyy + 0.3) * _fa_freqHigh[6 ], _fa_freqHigh[7 ] * (foamDiffuse.xxxx + 0.1), 1 - foamMask) ;
		vec3 globalNormal;
		...
		lightingInput_t lightInput = lightingInput_t( vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, vec3(0,0,0), vec3(0,0,1), vec3(0,0,1), vec3(0,0,0), vec3(0,0,0), 1, 1, vec3(0,0,0), vec3(0,0,0), vec4(0,0,0,0), vec4(0,0,0,0), mat3x3( vec3(1,0,0), vec3(0,1,0), vec3(0,0,1) ), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, 0, 0, uvec4(0,0,0,0), uvec4(0,0,0,0), uvec4(0,0,0,0) );
		{
			...
			lightInput.specular.xyz = sampleSpecular.xyz * _fa_freqHigh[8 ].xyz.xyz;
			lightInput.smoothness = samplePower.x;
			lightInput.fragCoord = gl_FragCoord;
			lightInput.position = _fa_freqLow[1 ].xyz + vofi_TexCoord4.xyz;
		...
		};
		{
			...
			lightInput.ambient_lighting.xyz += max ( indirectDiffuse.xyz * sampleDiffuse.xyz * saturate( _fa_freqLow[2 ].x ), vec3( 0 ) );
		};
		{
			uint skipStaticModel = _fa_freqHigh[9 ].x > 0 ? ( 1 << 3 ) : 0;
			uint clusterOffset;
			{
				{
					...
					clusterCoordinate.xy = screenPosToTexcoord( lightInput.fragCoord.xy, _fa_freqHigh[10 ] ) * _fa_freqLow[3 ].zw;
					clusterCoordinate.xy = floor( clusterCoordinate.xy * vec2( NUM_CLUSTERS_X, NUM_CLUSTERS_Y ) );
					float curr_z = GetLinearDepth( lightInput.fragCoord.z, _fa_freqLow[4 ], 1.0, false );
					float slice = log2( max( 1.0, curr_z / _fa_freqLow[5 ].z ) ) * _fa_freqLow[5 ].w;
		...
		{
			out_FragColor0 = vec4( lightInput.diffuse_lighting.xyz + lightInput.specular_lighting.xyz, 1 );
		};
		vec3 fogColor;
		float fogAmount;
		{
			float FLT_EPSILON = 0.0001;
			vec3 fogLightInscatterScaledColor = _fa_freqLow[9 ].xyz * _fa_freqLow[10 ].x;
			vec3 heightFogColor = mix( fogLightInscatterScaledColor, _fa_freqLow[11 ].xyz, vofi_TexCoord6.z );
			...
			fogColor = mix( _fa_freqLow[12 ].xyz, heightFogColor, fogSelection );
			fogAmount = 1.0 - fogAmount;
		};
		out_FragColor0.xyz = mix( fogColor * vec3( _fa_freqLow[13 ].x ), out_FragColor0.xyz, fogAmount );
		out_FragColor0.w = mix(0.0, 1.0, mask.x);
