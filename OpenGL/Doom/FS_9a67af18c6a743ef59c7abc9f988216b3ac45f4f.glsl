...
void main() {
	{
		vec4 degammaColor = SRGBlinearApprox( _fa_freqHigh[0 ] );
		vec4 degammaColorOutline = SRGBlinearApprox( _fa_freqHigh[1 ] );
		float sinTime = sin( _fa_freqHigh[2 ].x * 2.0 ) * 0.3 + 0.7;
		vec4 normals = tex2D( samp_sparebumpmap, vofi_TexCoord0.xy );
		vec3 viewOffset = normalize( vofi_TexCoord5.xyz );
		vec4 globalNormal = vec4( 0.0 );
		{
			vec3 N = normals.wyz * 2.0 - 1.0;
		...
		float fresnel = pow( 1.0 - saturate ( dot( globalNormal.xyz, viewOffset.xyz ) ), _fa_freqHigh[3 ].x );
		out_FragColor0.xyz = degammaColor.xyz * _fa_freqHigh[4 ].x;
		vec3 bakedEdges = pow( vec3( normals.w ), vec3( _fa_freqHigh[3 ].x ) );
		bakedEdges *= bakedEdges * degammaColorOutline.xyz;
		vec3 fresnelOutlines = degammaColorOutline.xyz * fresnel * fresnel;
		out_FragColor0.xyz += bakedEdges + fresnelOutlines * _fa_freqHigh[4 ].x;
		float outlineAlpha = GetLuma( fresnelOutlines + bakedEdges );
		out_FragColor0.w = saturate( degammaColor.w + outlineAlpha );
	}
}