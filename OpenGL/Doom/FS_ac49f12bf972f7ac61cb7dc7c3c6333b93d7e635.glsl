...
void main() {
	{
		...
		vec2 edgeDiffuseTC2 = tcWorld * _fa_freqHigh[3 ].xy + vec2( _fa_freqLow[0 ].x ) * _fa_freqHigh[3 ].zw;
		vec3 mainSurface = vec3( tex2D( samp_transmap2, edgeDiffuseTC2 ) ) * clamp((srcDepth * 0.005), 0.0, 1.0);
		vec4 colBlue = vec4(0.1, 0.2, 1.0, 0.0);
		vec4 colRed = vec4(1.0, 0.05, 0.05, 0.0);
		vec4 colDefault = _fa_freqHigh[4 ];
		vec4 boundaryColor = mix(mix( colDefault, colBlue, clamp( _fa_freqHigh[5 ].x, 0.0, 1.0) ) , colRed, clamp(( _fa_freqHigh[5 ].x - 1), 0.0, 1.0) ) ;
		vec3 normalColor = boundaryColor.xyz * mainSurface.xyz;
		vec4 final = vec4( 0.0 );
		final.xyz = normalColor * mainSurface.xyz;
		final.w = mainSurface.x ;
		final.w *= gOpacity.x ;
