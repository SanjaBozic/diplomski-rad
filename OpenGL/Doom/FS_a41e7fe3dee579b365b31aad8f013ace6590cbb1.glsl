	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				...
				vec2 tc_scroll = tc + ( _fa_freqHigh[8 ].xy * vec2( _fa_freqLow[4 ].x ) );
				vec4 noise = tex2DDeGammaApprox( samp_transmap1, tc_scroll );
				float texNoise = ( noise.a * 2 - 1.0 );
				vec2 tc1 = tc + ( texNoise.xx * _fa_freqHigh[8 ].z );
				vec2 tc2 = tc + ( texNoise.xx * _fa_freqHigh[8 ].w );
				vec4 heightTex = tex2DDeGammaApprox( samp_transmap, tc1 );
				float vertMaskFloor = _fa_freqHigh[9 ].x;
				float vertMaskCeiling = _fa_freqHigh[9 ].y;
				float refValue = saturate( ( vofi_Color.r - vertMaskFloor ) / ( vertMaskCeiling - vertMaskFloor ) );
				float maskHeight = heightTex.a;
				float transitionSoftness = _fa_freqHigh[9 ].z;
				float layerMask = saturate( ( refValue - maskHeight ) * transitionSoftness );
				vec4 sampleDiffuse1 = tex2DDeGammaApprox( samp_sparediffusemap, tc1 ) * _fa_freqHigh[6 ];
				...
				vec4 sampleDiffuse = mix( sampleDiffuse1, _fa_freqHigh[10 ], layerMask );
				...
				vec2 tc3 = tc + ( _fa_freqHigh[11 ].xy * _fa_freqLow[4 ].x ) + ( texNoise.xx * _fa_freqHigh[8 ].w );
				vec2 tc4 = tc + ( _fa_freqHigh[11 ].zw * _fa_freqLow[4 ].x ) + ( texNoise.xx * _fa_freqHigh[8 ].w );
				...
				inputs.alpha = slimeColor1 * slimeColor2 * layerMask * _fa_freqHigh[9 ].w;
			};
			{
			...
			{
				inputs.diffuse_lighting.xyz = inputs.diffuse_lighting.xyz + ( inputs.alpha * _fa_freqHigh[13 ].xyz );
			};
			{
