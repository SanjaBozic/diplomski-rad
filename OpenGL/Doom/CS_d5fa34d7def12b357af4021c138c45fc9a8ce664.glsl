...
uniform vec4 _ca_freqHigh [8];
uniform vec4 _ca_freqLow [13];
...
layout( binding = 0, r11f_g11f_b10f ) uniform image2D ib_uavssrtrace0 ;
layout( binding = 1, r8 ) uniform image2D ib_uavssrtrace1 ;
	...
	{
		{
			...
			vec4 resultColor0 = vec4( 0 );
			vec4 resultColor1 = vec4( 0 );
			{
				{
					resultColor0 = vec4( 0 );
					resultColor1 = vec4( 0 );
					...
					{
						{
							vec3 normal = NormalOctDecode( tex2DFetch( samp_tex0, ivec2(gl_GlobalInvocationID.xy ).xy.xy, 0 ).xy, false );
							vec4 spec = tex2DFetch( samp_tex4, ivec2(gl_GlobalInvocationID.xy ).xy.xy, 0 );
							scene.normal = normal.xyz;
							mat3x3 invProjT = mat3x3( _ca_freqLow[0 ].xyz, _ca_freqLow[1 ].xyz, _ca_freqLow[2 ].xyz );
							...
							scene.specular = sqr( tex2DFetch( samp_tex4, ivec2(gl_GlobalInvocationID.xy ).xy.xy, 0 ).xyz );
							scene.depth = tex2DFetch( samp_tex1, ivec2(gl_GlobalInvocationID.xy ).xy.xy, 0 ).x;
							float zLinear = GetLinearDepth( scene.depth, _ca_freqLow[3 ], 1.0, true );
							scene.world_pos = GetViewPos( vec3( tc * _ca_freqLow[4 ].zw, scene.depth ), _ca_freqLow[5 ], _ca_freqLow[6 ] );
					...
					float smoothness_threshold = _ca_freqHigh[1 ].x;
					float VdovR_threshold = 0.0; if ( scene.smoothness > smoothness_threshold && dot( scene.view.xyz, scene.reflection.xyz ) <= VdovR_threshold && scene.depth < 1.0 ) {
						uvec2 idxPix = uvec2( ivec2(gl_GlobalInvocationID.xy ).xy.xy) & 3;
						float jitter = float( ( ( ( 2068378560 * ( 1 - ( idxPix.x >> 1 ) ) + 1500172770 * ( idxPix.x >> 1 ) ) >> ( ( idxPix.y + ( ( idxPix.x & 1 ) << 2 ) ) << 2 ) ) + int( _ca_freqHigh[2 ].w ) ) & 0xF ) / 15.0;
						jitter -= 1.0;
						int max_steps = 12;
						float stride = 1;
						vec3 ray_start_vs = scene.world_pos;
						vec3 ray_end_vs = ray_start_vs.xyz + scene.reflection * GetLinearDepth( scene.depth, _ca_freqLow[3 ], 1, true );
						vec4 hit_color;
						{
							{
								hit_color = vec4( 0 );
								vec3 ray_start = GetWindowPosZ( ray_start_vs, _ca_freqLow[7 ] ) * vec3( _ca_freqLow[4 ].xy, 1 );
								vec3 ray_end = GetWindowPosZ( ray_end_vs, _ca_freqLow[7 ] ) * vec3( _ca_freqLow[4 ].xy, 1 );
								...
								float prev_scene_z = ray_start.z; for ( int curr_step = 0; curr_step < max_steps ; curr_step += 4 ) {
									...
									vec4 worldPosM = MatrixMul( vec3( best_hit.xy * _ca_freqLow[4 ].zw, best_hit.z ), _ca_freqLow[8 ], _ca_freqLow[9 ], _ca_freqLow[10 ], _ca_freqLow[11 ] );
									worldPosM /= worldPosM.w;
									vec4 tc_reproj = MatrixMul( worldPosM.xyz, _ca_freqHigh[3 ], _ca_freqHigh[4 ], _ca_freqHigh[5 ], _ca_freqHigh[6 ] );
									tc_reproj.xy /= tc_reproj.w;
									tc_reproj.xy = tc_reproj.xy * 0.5 + 0.5;
									tc_reproj.xy *= _ca_freqLow[12 ].xy;
									vec4 finalRefl = vec4( tex2Dlod( samp_tex5, vec4( tc_reproj.xy, 0, 0 ) ).xyz, 1 );
									vec2 dist = ( tc_reproj.xy * _ca_freqLow[12 ].zw ) * 2 - 1;
						...
						hit_color.w = saturate( hit_color.w * smoothness_mask * _ca_freqHigh[7 ].x );
						resultColor0.rgb = max( vec3( 0 ), hit_color.w * hit_color.xyz * scene.phongEnvBRDF );
						resultColor1.r = saturate( hit_color.w );
					}
				}
			};
			imageStore( ib_uavssrtrace0, ivec2(gl_GlobalInvocationID.xy ).xy, resultColor0 );
			imageStore( ib_uavssrtrace1, ivec2(gl_GlobalInvocationID.xy ).xy, resultColor1 );
		};
	}
}