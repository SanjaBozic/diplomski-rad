...
void main() {
	{
		...
		vec4 cloud = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy * _fa_freqHigh[2 ].xy + vec2( _fa_freqLow[0 ].x * _fa_freqHigh[2 ].z, _fa_freqLow[0 ].y * _fa_freqHigh[2 ].w ) ) * vofi_Color;
		vec4 mask = tex2DDeGammaApprox( samp_transmap1, vofi_TexCoord0.xy );
		cloud *= _fa_freqHigh[3 ];
		cloud.xyz += vec3( _fa_freqHigh[4 ].x ) * cloud.xyz * mask.xyz;
		vec3 fogColor;
		float fogAmount;
		{
