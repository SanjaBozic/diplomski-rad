	...
	{
		{
			...
			{
				{
					vec3 desaturated = vec3( GetLuma( inputs.diffuse_lighting.xyz ) );
					inputs.diffuse_lighting.xyz = desaturated * _fa_freqHigh[11 ].xyz * vec3( 80.0 ) + vec3( 0.01 ) + vec3( fract( _fa_freqLow[11 ].x * 5.0 ) ) * vec3( 0.005 );
					vec2 tc = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[12 ] );
					vec2 grain_tc = tc.xy + ( ivec2( _fa_freqLow[12 ].xy ) & 1023 ) / 1023.0;
					float grain = fract( sin( grain_tc.x + grain_tc.y * 521.0 ) * 493013.0 ) * 2.0 - 1.0;
					inputs.diffuse_lighting.xyz.xyz = saturate( inputs.diffuse_lighting.xyz.xyz + ( saturate( 1 - GetLuma( inputs.diffuse_lighting.xyz.xyz ) ) * inputs.diffuse_lighting.xyz.xyz * grain.xxx * _fa_freqHigh[13 ].x ) );
				};
			};
			{
