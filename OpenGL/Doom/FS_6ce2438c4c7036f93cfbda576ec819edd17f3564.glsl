...
void main() {
	{
		...
		vec3 color = vofi_Color.xyz; if ( _fa_freqHigh[0 ].x > 0.0 ) {
			...
			float outlineThickness = _fa_freqHigh[0 ].x;
			...
			float outlineAlpha = smoothstep( outlineMinDistance, outlineMaxDistance, distance );
			vec4 fxOutlineColor = ( _fa_freqHigh[1 ].x > 0.0 ) ? DeGamma( _fa_freqHigh[2 ] ) : _fa_freqHigh[2 ];
			vec3 finalOutlineColor = mix( ( fxOutlineColor.xyz * outlineAlpha ), color, alpha );
			float finalOutlineAlpha = max( alpha, outlineAlpha * fxOutlineColor.a );
			...
			vec2 glowTC = vofi_TexCoord0.xy + ( _fa_freqHigh[3 ].xy / _fa_freqHigh[0 ].zw );
			vec4 glowTexel = tex2Dlod( samp_transmap, vec4( glowTC.xy, 0, 0 ) );
			float glowThickness = _fa_freqHigh[0 ].y;
			...
			vec4 fxGlowColor = ( _fa_freqHigh[1 ].x > 0.0 ) ? DeGamma( _fa_freqHigh[4 ] ) : _fa_freqHigh[4 ];
			color = mix( fxGlowColor.xyz, color, alpha );
			alpha = max( alpha, glowAlpha * fxGlowColor.a );
		}
