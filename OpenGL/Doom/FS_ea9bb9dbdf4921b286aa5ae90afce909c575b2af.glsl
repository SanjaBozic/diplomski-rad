...
void main() {
	{
		...
		float scale;
		{
			...
			scale = saturate( ( r1 - r2 ) * _fa_freqHigh[1 ].y );
			scale = saturate( mix( scale, scale * 500.0, 0.0 ) );
		};
		vec4 r0 = tex2DDeGamma( samp_transmap, vofi_TexCoord0.xy ) * _fa_freqHigh[2 ];
		out_FragColor0 = r0 * vofi_Color;
		out_FragColor0.xyz *= _fa_freqHigh[3 ].xyz * vec3( _fa_freqLow[0 ].x );
		out_FragColor0.w *= scale;
		{
