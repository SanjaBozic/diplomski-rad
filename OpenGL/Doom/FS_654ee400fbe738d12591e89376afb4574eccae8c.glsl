...
void main() {
	{
		float specShiftMask = 0;
		{
			...
			out_FragColor0.xyz = desaturated * _fa_freqHigh[6 ].xyz * vec3( 10.0 ) + vec3( 0.01 ) + vec3( fract( _fa_freqLow[9 ].x * 5.0 ) ) * vec3( 0.005 );
			...
			float glitch = mix( 2.0, 1.25, _fa_freqHigh[8 ].x );
			...
			ditherY = mix( ditherY, ditherYGlitch * ditherY * artifacts.x, _fa_freqHigh[8 ].x );
			ditherX *= ditherY;
			float clipValue = ditherX * mix( 0.0, mix( artifacts.x - 0.2, 1.0, _fa_freqHigh[9 ].w ), _fa_freqHigh[9 ].w );
			clip( clipValue - 0.1 );
			{
