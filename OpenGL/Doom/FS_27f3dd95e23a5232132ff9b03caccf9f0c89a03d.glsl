	...
	{
		{
			...
			{
				{ if ( _fa_freqHigh[9 ].w > 0 ) {
						vec4 tiling = vec4( 0.003, 0.01, 0.05, 0.3 );
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[10 ].zw );
						...
						vec3 finalPulseFresnel = saturate( digitalNoise.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[9 ].xyz;
						vec3 finalPulseFlat = saturate( digitalNoise.x ) * _fa_freqHigh[9 ].xyz;
						vec3 finalPulse = mix( finalPulseFresnel, finalPulseFlat, vec3( 0.25 ) );
						vec3 constantGlow = saturate( 0.5 * ( fresnel ) + edgeGlow ) * _fa_freqHigh[9 ].xyz;
						finalPulse /= _fa_freqHigh[11 ].xyz;
						vec3 finalPulseColor = finalPulse * _fa_freqHigh[9 ].w;
				...
				{ if ( _fa_freqHigh[12 ].w > 0 ) {
						...
						inputs.diffuse_lighting.xyz += dither.xxx * vec3( nDotSomething + 0.01 ) * _fa_freqHigh[12 ].xyz * vec3( _fa_freqHigh[12 ].w * 2.0 ) * 2.0;
					}
				};
				{ if ( _fa_freqHigh[13 ].x > 0 ) {
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[10 ].zw );
						vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy*1.5, _fa_freqHigh[3 ] ) * ( resolution / 32.0 );
						vec4 dither = tex2Dlod( samp_echodithermap, vec4( screenTexCoord * vec2( 0.7 ), 0.0, 0.0 ) );
						float fresnel = 1.0 - dot( inputs.normal, normalize( -vofi_TexCoord4.xyz ) );
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[14 ].x; if ( _fa_freqHigh[13 ].y >= _fa_freqHigh[14 ].z ) {
							pulsefrequency = _fa_freqHigh[14 ].y;
						}
						float width = mix( _fa_freqHigh[15 ].x, _fa_freqHigh[15 ].y, _fa_freqHigh[13 ].y );
						float zValue = ( vofi_TexCoord4.xyz.z ) * width;
						float pulse = sin( zValue + _fa_freqLow[14 ].x * pulsefrequency ) * magnitude + magnitude;
						float power = _fa_freqHigh[15 ].z;
						pulse = pow( pulse, power );
						pulse *= fresnel;
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[16 ].xyz * 2.0;
						finalPulse /= _fa_freqHigh[11 ].xyz;
				...
				{ if ( _fa_freqHigh[17 ].w > 0.0 ) {
						...
						inputs.diffuse_lighting.xyz += ( crevices + finalOutline * vec3( 15.0 ) ) * _fa_freqHigh[17 ].xyz;
					}
				};
				{ if ( _fa_freqHigh[18 ].w > 0.0 ) {
						vec2 resolution = abs( vec2( 1.0 ) / _fa_freqHigh[10 ].zw );
						vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[3 ] ) * ( resolution / vec2( 32.0 ) );
						vec4 colorWave = tex2DlodDeGamma( samp_transmap, vec4( screenTexCoord * _fa_freqHigh[19 ].xy + vec2( _fa_freqLow[14 ].x ) * _fa_freqHigh[19 ].zw, 0.0, 0.0) );
						...
						vec3 finalOutline = vec3( fresnel );
						vec3 close = crevices * crevices * vec3( 180.0 ) * _fa_freqHigh[18 ].xyz + finalOutline * _fa_freqHigh[18 ].xyz;
						vec3 far = _fa_freqHigh[18 ].xyz * vec3( 0.1 ) * colorWave.xyz * vec3( fresnel2 ) + vec3( fresnel2 ) * _fa_freqHigh[18 ].xyz * vec3( 20.0 );
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz, ( inputs.diffuse_lighting.xyz + mix( close, far, vec3( farFade ) ) * colorWave.xyz ) + _fa_freqHigh[18 ].xyz * vec3( 0.1 ) * vec3( fresnel2 ), vec3( _fa_freqHigh[18 ].w ) );
					}
				};
				{ if ( _fa_freqHigh[20 ].x > 0 ) {
						float clampedGunGlow = saturate( _fa_freqHigh[20 ].x * _fa_freqHigh[20 ].x );
						vec3 glowColor = mix( _fa_freqHigh[6 ].xyz, _fa_freqHigh[21 ].xyz, clampedGunGlow );
				...
				{ if ( _fa_freqHigh[22 ].x < 1.0 ) {
						vec2 texcoord = gl_FragCoord.xy * ( 0.00390625 );
						float dither = tex2Dlod( samp_magnificationclouds, vec4( vofi_TexCoord0.xy * _fa_freqHigh[23 ].xy, 0.0, 0.0 ) ).x;
						dither = saturate( abs( clamp( _fa_freqHigh[22 ].x - dither * 0.5 - 0.1, -1.0, 0.0 ) ) );
						dither *= 600.0;
