...
void main() {
	{
		...
		{
			{
				vec2 directional_occlusion = vec2( 1 ); if ( _fa_freqHigh[1 ].y > 0 ) {
					directional_occlusion = saturate( pow( tex2Dlod( samp_tex4, vec4( tc.xy, 0, 0 ) ).xx, _fa_freqHigh[1 ].zw ) );
				}
				vec3 specularBuffer = tex2Dlod( samp_tex0, vec4( tc.xy, 0, 0 ) ).xyz; if ( _fa_freqHigh[2 ].x > 0 ) {
					vec3 ssrBuffer = tex2Dlod( samp_tex2, vec4( tc.xy + _fa_freqHigh[0 ].zw*0.25, 0, 0 ) ).xyz;
					float ssrMask = tex2Dlod( samp_tex3, vec4( tc.xy, 0, 0 ) ).x;
					specularBuffer.xyz = mix( specularBuffer.xyz, ssrBuffer.xyz, ssrMask.xxx );
				}
				out_FragColor0 = vec4( specularBuffer.xyz * directional_occlusion.y, directional_occlusion.x );
			}
		};
	}
}