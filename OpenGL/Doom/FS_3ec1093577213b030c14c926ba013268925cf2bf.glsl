...
void main() {
	{
		...
		{
			{
				...
				vec2 texCoordsLOD = vofi_TexCoord0.xy; if ( _fa_freqHigh[0 ].x > 0 ) {
					texCoords.xy = fract( vofi_TexCoord0.xy ) * _fa_freqHigh[1 ].xy + _fa_freqHigh[1 ].zw;
		...
		vec3 diffuse = sampleDiffuse.xyz;
		{
		...
		lightingInput_t lightInput = lightingInput_t( vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, vec3(0,0,0), vec3(0,0,1), vec3(0,0,1), vec3(0,0,0), vec3(0,0,0), 1, 1, vec3(0,0,0), vec3(0,0,0), vec4(0,0,0,0), vec4(0,0,0,0), mat3x3( vec3(1,0,0), vec3(0,1,0), vec3(0,0,1) ), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), vec3(0,0,0), 0, 0, 0, 0, 0, uvec4(0,0,0,0), uvec4(0,0,0,0), uvec4(0,0,0,0) );
		{
			...
			lightInput.colorMask = sampleMask.xyz;
		};
		{
			...
			{
				{
					...
					clusterCoordinate.xy = screenPosToTexcoord( lightInput.fragCoord.xy, _fa_freqHigh[4 ] ) * _fa_freqLow[4 ].zw;
					clusterCoordinate.xy = floor( clusterCoordinate.xy * vec2( NUM_CLUSTERS_X, NUM_CLUSTERS_Y ) );
					float curr_z = GetLinearDepth( lightInput.fragCoord.z, _fa_freqLow[5 ], 1.0, false );
			...
			{
				{
					...
					ambient = mix( GetLuma( ambient.xyz ).xxx, ambient.xyz, _fa_freqLow[7 ].www );
					...
					for ( uint lightIdx = lightsMin; lightIdx < lightsMax; ) {
						...
						{ light_t lightParms;
							...
							float shadow = 1.0; if ( ( light_parms & ( 1 << 2 ) ) != 0 ) {
								{
									...
									uint idx = 0; if ( ( light_parms & ( ( 0 << 0 ) | ( 1 << 0 ) | ( 2 << 0 ) | ( 3 << 0 ) ) ) == ( 0 << 0 ) ) {
										...
										float fraction = ( 1.0 - max( shadowTC.x, shadowTC.y ) ) * _fa_freqLow[8 ].w;
										...
										{
											{
												if ( false ) {
													...
													vec2 uv = shadowTC.xy * _fa_freqLow[9 ].xy;
													vec2 shadowMapSizeInv = _fa_freqLow[9 ].zw;
							...
							light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _fa_freqLow[7 ].www );
							{
		...
		{ if ( _fa_freqHigh[5 ].x > 0 ) {
				vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[6 ].zw );
				vec2 screenTexCoord = screenPosToTexcoord( gl_FragCoord.xy*1.5, _fa_freqHigh[4 ] ) * ( resolution / 32.0 );
				vec4 dither = tex2Dlod( samp_echodithermap, vec4( screenTexCoord * vec2( 0.7 ), 0.0, 0.0 ) );
				float fresnel = 1.0 - dot( globalNormal, normalize( -vofi_TexCoord4.xyz ) );
				float magnitude = 1.0;
				float pulsefrequency = _fa_freqHigh[7 ].x; if ( _fa_freqHigh[5 ].y >= _fa_freqHigh[7 ].z ) {
					pulsefrequency = _fa_freqHigh[7 ].y;
				}
				float width = mix( _fa_freqHigh[8 ].x, _fa_freqHigh[8 ].y, _fa_freqHigh[5 ].y );
				float zValue = ( vofi_TexCoord4.xyz.z ) * width;
				float pulse = sin( zValue + _fa_freqLow[10 ].x * pulsefrequency ) * magnitude + magnitude;
				float power = _fa_freqHigh[8 ].z;
				pulse = pow( pulse, power );
				pulse *= fresnel;
				float edgeGlow = pow( fresnel, 4.0 );
				vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[9 ].xyz * 2.0;
				finalPulse /= _fa_freqHigh[10 ].xyz;
				vec3 finalPulseColor = finalPulse * pulse;
				float saturation = ( _fa_freqLow[11 ].x == 0 ) ? 1.0 : 0.7;
				float blendScale = ( _fa_freqLow[11 ].x == 0 ) ? 1.0 : 0.5;
				lightInput.diffuse_lighting.xyz += max( vec3( 0 ) , mix( GetLuma( finalPulseColor ).xxx, finalPulseColor, saturation.xxx ) ) * blendScale;
			}
		};
		{
