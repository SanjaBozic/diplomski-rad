...
void main() {
	{
		...
		vofi_TexCoord5.x = dot3( localPosition.xyz, _va_freqHigh[0 ] );
		vofi_TexCoord5.y = dot3( localPosition.xyz, _va_freqHigh[1 ] );
		vofi_TexCoord5.z = dot3( localPosition.xyz, _va_freqHigh[2 ] );
		{
			vec4 localPosition;
			vec4 localNormal;
			vec4 localTangent;
			...
			{
				morphScale = 0.0; if ( _va_freqHigh[3 ].x != 0.0 ) {
			...
			{
				gl_Position.x = dot4( localPosition, _va_freqHigh[4 ] );
				gl_Position.y = dot4( localPosition, _va_freqHigh[5 ] );
				gl_Position.z = dot4( localPosition, _va_freqHigh[6 ] );
				gl_Position.w = dot4( localPosition, _va_freqHigh[7 ] );
		...
		vec3 worldNormal;
		worldNormal.x = dot3( normal, _va_freqHigh[0 ] );
		worldNormal.y = dot3( normal, _va_freqHigh[1 ] );
		worldNormal.z = dot3( normal, _va_freqHigh[2 ] );
		vofi_TexCoord2.xyz = normalize( worldNormal );
		vofi_TexCoord2.w = 0.0;
		{
			globalPosition.x = dot4( localPosition, _va_freqHigh[0 ] );
			globalPosition.y = dot4( localPosition, _va_freqHigh[1 ] );
		...
		vec3 toViewer;
		toViewer = normalize( _va_freqLow[2 ].xyz - globalPosition.xyz );
		vofi_TexCoord6.xyz = toViewer;
