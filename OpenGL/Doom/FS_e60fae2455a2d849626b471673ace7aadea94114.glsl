...
uvec2 clockDelta (uvec4 beforeAfterTicks) {
	return beforeAfterTicks.zw - beforeAfterTicks.xy;
}
float clockDeltaToFloat (uvec2 delta) {
	return float(delta.y) * 0.00005;
}
	...
	{
		{
			...
			{
				vec3 orig_albedo = inputs.albedo.xyz;
				vec3 orig_specular = inputs.specular.xyz;
				if ( _fa_freqLow[6 ].x == 8 || _fa_freqLow[6 ].x == 9 || _fa_freqLow[6 ].x == 12 ) {
					inputs.albedo.xyz = vec3( 1 );
				}
				if ( _fa_freqLow[6 ].x == 10 ) {
					inputs.albedo.xyz = vec3( 0 );
				}
				else
				if ( _fa_freqLow[6 ].x == 11 ) {
					inputs.albedo.xyz = vec3( 0 );
					inputs.specular.xyz = vec3( 1 );
					inputs.smoothness = 1;
				}
				{
					...
					{
						{
							...
							clusterCoordinate.xy = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[2 ] ) * _fa_freqLow[7 ].zw;
							clusterCoordinate.xy = floor( clusterCoordinate.xy * vec2( NUM_CLUSTERS_X, NUM_CLUSTERS_Y ) );
							float curr_z = GetLinearDepth( inputs.fragCoord.z, _fa_freqLow[8 ], 1.0, false );
					...
					{
						{
							...
							inputs.ticksDecals.xy = shaderClock();
							{
							...
							inputs.ticksDecals.zw = shaderClock();
							inputs.albedo = saturate( unpackR10G10B10( inputs.albedo_packed ) * _fa_freqLow[10 ].z );
							inputs.specular = saturate( unpackR10G10B10( inputs.specular_packed ) );
							inputs.albedo_packed = packR10G10B10( inputs.albedo.xyz );
							inputs.smoothness = saturate( pow( inputs.smoothness, _fa_freqLow[10 ].x ) * _fa_freqLow[10 ].y );
							vec3 ambient = vec3( 0 );
							ambient = (inputs.lightmap.xyz * _fa_freqLow[11 ].xyz) * _fa_freqLow[12 ].x;
							ambient += inputs.emissive;
							inputs.smoothness = dot( inputs.emissive.xyz, vec3( 1 ) ) > 0.0 ? -inputs.smoothness : inputs.smoothness;
							ambient = mix( GetLuma( ambient.xyz ).xxx, ambient.xyz, _fa_freqLow[10 ].www );
							inputs.ambient_lighting = ambient;
							inputs.diffuse_lighting_packed = packRGBE( ambient );
							inputs.ticksLights.xy = shaderClock();
							for ( uint lightIdx = lightsMin; lightIdx < lightsMax; ) {
								...
								{ light_t lightParms;
									...
									float shadow = 1.0; if ( ( light_parms & ( 1 << 2 ) ) != 0 ) {
										{
											...
											uint idx = 0; if ( ( light_parms & ( ( 0 << 0 ) | ( 1 << 0 ) | ( 2 << 0 ) | ( 3 << 0 ) ) ) == ( 0 << 0 ) ) {
												...
												float fraction = ( 1.0 - max( shadowTC.x, shadowTC.y ) ) * _fa_freqLow[13 ].w;
												...
												{
													{
														if ( false ) {
															...
															vec2 uv = shadowTC.xy * _fa_freqLow[14 ].xy;
															vec2 shadowMapSizeInv = _fa_freqLow[14 ].zw;
									...
									light_color_final = mix( GetLuma( light_color_final.xyz ).xxx, light_color_final.xyz, _fa_freqLow[10 ].www );
									{
							...
							inputs.ticksLights.zw = shaderClock();
				...
				if ( _fa_freqLow[6 ].x == 9 || _fa_freqLow[6 ].x == 12 ) {
					inputs.specular_lighting = vec3( 0 );
				}
				float min_valid_spec = 0.02;
				float max_valid_spec_dielectric = 0.05;
				float metal_check_error_threshold = 0.03;
				float min_valid_spec_metal = dot( vec3( 0.541931, 0.496791, 0.449419 ), vec3( 0.3333 ) );
				float min_valid_albedo = 0.02;
				float max_valid_albedo = 0.9;
				float min_valid_albedo_non_metal_warning = _fa_freqLow[6 ].y;
				float max_valid_albedo_metal_warning = _fa_freqLow[6 ].z;
				if ( _fa_freqLow[6 ].x >= 1 && _fa_freqLow[6 ].x <= 6 ) {
					float albedo_avg = dot( orig_albedo.xyz, vec3( 0.333 ) );
					float spec_avg = dot( orig_specular.xyz, vec3( 0.333 ) );
					inputs.specular_lighting = vec3( 0 );
					if ( _fa_freqLow[6 ].x == 1 ) {
						inputs.diffuse_lighting.xyz = inputs.albedo.xyz;
					}
					else if ( _fa_freqLow[6 ].x == 2 ) {
						inputs.diffuse_lighting.xyz = inputs.specular.xyz;
					}
					else if ( _fa_freqLow[6 ].x == 3 ) {
						inputs.diffuse_lighting.xyz = inputs.smoothness.xxx;
					}
					else if ( _fa_freqLow[6 ].x == 4 ) {
						inputs.diffuse_lighting.xyz = inputs.normal.xyz * 0.5 + 0.5;
					}
					else if ( _fa_freqLow[6 ].x == 5 ) {
						if ( _fa_freqLow[6 ].w > 0 ) {
							min_valid_albedo = _fa_freqLow[6 ].y;
							max_valid_albedo = _fa_freqLow[6 ].z;
						}
						if ( albedo_avg > max_valid_albedo )
						inputs.diffuse_lighting.xyz = vec3( 1, 1, 1);
						else
						if ( albedo_avg < min_valid_albedo )
						inputs.diffuse_lighting.xyz = vec3( 1, 0, 0);
						else
						inputs.diffuse_lighting.xyz = vec3( 0, 1, 0);
						if ( _fa_freqLow[6 ].w == 0 ) {
							if ( spec_avg > min_valid_spec && spec_avg <= max_valid_spec_dielectric ) {
								if ( albedo_avg > min_valid_albedo && albedo_avg < min_valid_albedo_non_metal_warning ) {
									inputs.diffuse_lighting.xyz = vec3(1, 1, 0);
								}
							}
							if ( spec_avg > min_valid_spec_metal ) {
								if ( albedo_avg > max_valid_albedo_metal_warning && albedo_avg < max_valid_albedo ) {
									inputs.diffuse_lighting.xyz = vec3(1, 0.5, 0);
								}
							}
						}
					}
					else if ( _fa_freqLow[6 ].x == 6 ) {
						if ( spec_avg < min_valid_spec )
						inputs.diffuse_lighting.xyz = vec3(1,0,0);
						else
						if ( spec_avg >= min_valid_spec && spec_avg <= max_valid_spec_dielectric )
						inputs.diffuse_lighting.xyz = vec3(0,1,0);
						else
						if ( spec_avg > max_valid_spec_dielectric && spec_avg < min_valid_spec_metal - metal_check_error_threshold )
						inputs.diffuse_lighting.xyz = vec3(1,0,0);
						else
						if ( spec_avg >= min_valid_spec_metal - metal_check_error_threshold )
						inputs.diffuse_lighting.xyz = vec3(0,0,1);
					}
				}
				if ( _fa_freqLow[6 ].x == 9 ) {
					inputs.diffuse_lighting.xyz += inputs.ambient_lighting.xyz;
				}
				else
				if ( _fa_freqLow[6 ].x == 12 ) {
					inputs.diffuse_lighting.xyz = inputs.ambient_lighting.xyz;
				}
				if ( _fa_freqLow[6 ].x == 7 || _fa_freqLow[6 ].x >= 17 && _fa_freqLow[6 ].x < 21 ) {
					float NUM_CLUSTERS_X = 16;
					float NUM_CLUSTERS_Y = 8;
					float NUM_CLUSTERS_Z = 24;
					int MAX_LIGHTS_PER_CLUSTER = 256;
					...
					float overdraw = 0;
					if ( _fa_freqLow[6 ].x == 7 ) {
						overdraw = saturate( float( ( lightsMax - lightsMin ) + ( probesMax - probesMin ) ) / 10.0 );
					}
					else
					if ( _fa_freqLow[6 ].x == 19 ) {
						overdraw = saturate( float( decalsMax - decalsMin ) / 10.0 );
					}
					else
					if ( _fa_freqLow[6 ].x == 18 ) {
						overdraw = saturate( float( probesMax - probesMin ) / 10.0 );
					}
					else
					if ( _fa_freqLow[6 ].x == 17 ) {
						overdraw = saturate( float( lightsMax - lightsMin ) / 10.0 );
					}
					else {
						overdraw = saturate( float( ( lightsMax - lightsMin ) + ( probesMax - probesMin ) + ( decalsMax - decalsMin ) ) / 10.0 );
					}
					vec2 norm_tc = screenPosToTexcoord( inputs.fragCoord.xy, _fa_freqHigh[2 ] ) * _fa_freqLow[7 ].zw;
					float xmin = 0.025, xmax = 0.325, ymin = 0.97, ymax = 0.99;
					if( norm_tc.x > xmin && norm_tc.x < xmax && norm_tc.y > ymin && norm_tc.y < ymax ) {
						overdraw = 1.0 - ( ( xmax - norm_tc.x ) / ( xmax - xmin ) );
						overdraw = saturate( float( floor( overdraw * 11 ) ) / ( 10.0 ) );
					}
					inputs.diffuse_lighting.xyz = mix( mix( vec3( 0, 0, 1 ), vec3( 0, 1, 0 ), vec3( overdraw * 2 ) ), vec3( 1, 0, 0 ), vec3( overdraw * 2 - 1 ) );
					inputs.specular_lighting.xyz = vec3( 0 );
				}
				if ( _fa_freqLow[6 ].x == 14 ) {
					vec3 vertexN = normalize( vec3( inputs.invTS[0].z, inputs.invTS[1].z, inputs.invTS[2].z ) );
					inputs.diffuse_lighting = vec3( saturate( dot( vertexN.xyz, inputs.view.xyz ) ) );
					inputs.specular_lighting = vec3( 0 );
				}
				if ( _fa_freqLow[6 ].x == 22 ) {
					float deltaDecals = clockDeltaToFloat( clockDelta( inputs.ticksDecals ) );
					float deltaProbes = clockDeltaToFloat( clockDelta( inputs.ticksProbes ) );
					float deltaLights = clockDeltaToFloat( clockDelta( inputs.ticksLights ) );
					inputs.diffuse_lighting.xyz *= 0.001;
					if ( deltaDecals > deltaProbes && deltaDecals > deltaLights ) {
						inputs.diffuse_lighting.x = 1.0;
					} else if ( deltaProbes > deltaLights ) {
						inputs.diffuse_lighting.y = 1.0;
					} else {
						inputs.diffuse_lighting.z = 1.0;
					}
					inputs.specular_lighting.xyz *= vec3( 0.001 );
				}
				vec4 virtualMapping = vofi_TexCoord5;
				if ( _fa_freqLow[6 ].x == 15 ) {
					vec2 virtCoords = vofi_TexCoord0.zw;
					vec2 virtualMappingScale = virtualMapping.xy;
					vec2 texCoords = virtCoords.xy * virtualMappingScale;
					float posRateOfChangeX = length( dFdx( vofi_TexCoord7.xyz ) );
					float texcoordRateOfChangeX = length( dFdx( texCoords ) );
					float posRateOfChangeY = length( dFdy( vofi_TexCoord7.xyz ) );
					float texcoordRateOfChangeY = length( dFdy( texCoords ) );
					float density = ( texcoordRateOfChangeX / posRateOfChangeX + texcoordRateOfChangeY / posRateOfChangeY ) / 2.0 * _fa_freqLow[1 ].z;
					if ( density > 12.0 ) {
						inputs.diffuse_lighting.xyz = vec3( 1.0, 0.0, 0.0 );
					} else if ( density > 11.0 ) {
						inputs.diffuse_lighting.xyz = mix( vec3( 1.0, 1.0, 0.0 ), vec3( 1.0, 0.0, 0.0 ), density - 11.0 );
					} else if ( density > 8.0 ) {
						inputs.diffuse_lighting.xyz = vec3( 1.0, 1.0, 0.0 );
					} else if ( density > 7.0 ) {
						inputs.diffuse_lighting.xyz = mix( vec3( 0.0, 1.0, 0.0 ), vec3( 1.0, 1.0, 0.0 ), density - 7.0 );
					} else {
						inputs.diffuse_lighting.xyz = vec3( 0.0, 1.0, 0.0 );
					}
					inputs.specular_lighting = vec3( 0.0 );
				}
				if ( _fa_freqLow[6 ].x == 16 ) {
					vec2 textureSizeXY = virtualMapping.xy * _fa_freqLow[1 ].z;
					float textureSize = max( textureSizeXY.x, textureSizeXY.y );
					if ( textureSize > 7500.0 ) {
						inputs.diffuse_lighting.xyz = vec3( 1.0, 0.0, 0.0 );
					} else if ( textureSize > 3750.0 ) {
						inputs.diffuse_lighting.xyz = vec3( 1.0, 1.0, 0.0 );
					} else if ( textureSize > 1875.0 ) {
						inputs.diffuse_lighting.xyz = vec3( 0.0, 1.0, 0.0 );
					} else {
						inputs.diffuse_lighting.xyz = vec3( 0.0, 0.0, 1.0 );
					}
				}
			};
			{
			...
			out_FragColor0.xyz += _fa_freqLow[15 ].xyz;
		};
	}
}