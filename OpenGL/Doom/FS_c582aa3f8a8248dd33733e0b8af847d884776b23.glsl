	...
	{
		{
			...
			luma_curr = ( luma_curr * _fa_freqHigh[0 ].w != 0.0 ) ? 1.0 : max( 0.0, luma_curr );
			float autoExposureAdjust = mix( _fa_freqHigh[1 ].x, luma_curr * _fa_freqHigh[2 ].w, _fa_freqHigh[1 ].y );
			float rcp_log2_10 = 1.0 / log2( 10 );
			float sceneKey = 1.03 - 2.0 / ( 2.0 + ( log2( ( autoExposureAdjust ) + 1.0 ) * rcp_log2_10 ) );
			float currAutoExposure = clamp( sceneKey / autoExposureAdjust, _fa_freqHigh[2 ].x, _fa_freqHigh[2 ].y );
			float prevAutoExposure = clamp( tex2Dlod( samp_tex1, vec4( 0.5, 0.5, 0.0, 0.0 ) ).x, 0.0, 64.0 );
			float autoExposureFinal = mix( prevAutoExposure, currAutoExposure, _fa_freqHigh[1 ].w );
			out_FragColor0.xyz = vec3( max( autoExposureFinal, 0.0 ) );
		};
	}
}