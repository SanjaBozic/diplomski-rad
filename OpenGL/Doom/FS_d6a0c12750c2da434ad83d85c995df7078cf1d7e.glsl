...
void main() {
	{
		...
		vec4 final = vec4( texColor.w );
		final += texMask.x;
		final.xyz *= _fa_freqHigh[2 ].x;
		final *= _fa_freqHigh[3 ] * mix( _fa_freqHigh[4 ], _fa_freqHigh[5 ], texMask.x );
		final.w *= texMask.y * scale;
		final.xyz *= ( texMask.y ) * 0.5 + 0.5;
		float falloff = smoothstep( -32.0, 1.0, vofi_TexCoord1.x );
		final.w *= falloff;
		vec3 fogColor;
		float fogAmount;
		{
		...
		final.xyz = mix( _fa_freqLow[4 ].xyz, final.xyz, fogAmount );
		final.xyz *= final.w;
		final.w = 1.0;
