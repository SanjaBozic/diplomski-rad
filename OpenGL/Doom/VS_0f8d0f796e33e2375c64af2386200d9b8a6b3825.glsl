...
void main() {
	{
		...
		float globMask = sin( _va_freqLow[2 ].x * 5.0 - in_TexCoord.y * 2.0 + localPosition.x * 0.003 + localPosition.y * 0.03 + vertexColor.z * 3.0 );
		globMask += sin( _va_freqLow[2 ].x * 2.0 - in_TexCoord.y * 3.0 + localPosition.x * 0.01 + localPosition.y * 0.1 + vertexColor.z * 3.0 );
		localPosition.xyz += localNormal.xyz * vec3( globMask * 0.5 * vertexColor.y );
		{
		...
		{
			if ( _va_freqHigh[8 ].x > 0.0 && _va_freqHigh[8 ].y > 0.0 ) {
				...
				vec4 sampleSHR = tex3Dlod( samp_ambientshatlas, vec4( shTC.xy, shTC.z + ( atlasSHCubeSize.z * 0.0 ), 0.0 ) ) * _va_freqHigh[8 ].y;
				vec4 sampleSHG = tex3Dlod( samp_ambientshatlas, vec4( shTC.xy, shTC.z + ( atlasSHCubeSize.z * 1.0 ), 0.0 ) ) * _va_freqHigh[8 ].y;
				vec4 sampleSHB = tex3Dlod( samp_ambientshatlas, vec4( shTC.xy, shTC.z + ( atlasSHCubeSize.z * 2.0 ), 0.0 ) ) * _va_freqHigh[8 ].y;
