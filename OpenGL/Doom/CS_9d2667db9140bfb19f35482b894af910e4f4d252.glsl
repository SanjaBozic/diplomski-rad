...
void main() {
	{
		float autoExposure = tex2Dlod( samp_tex2, vec4( vec2( 0.5 ), 0.0, 0.0 ) ).x;
		ivec3 pixelLocation = ivec3(gl_GlobalInvocationID );
		vec2 texcoord = ( pixelLocation.xy + 0.5 ) * _ca_freqHigh[0 ].zw;
		vec3 outputColor;
		{
			vec2 unscaledTexcoord = texcoord.xy / _ca_freqLow[0 ].xy;
			{
			...
			texcoordD *= _ca_freqHigh[1 ].x * pow( directionLen, _ca_freqHigh[1 ].y ) / directionLen * _ca_freqLow[0 ].xy;
			...
			vec4 resScalingInfo = vec4( _ca_freqLow[0 ].xy, _ca_freqHigh[0 ].zw );
			vec3 linearHDR =
			...
			vec3 bloom = tex2Dlod( samp_tex1, vec4( unscaledTexcoord, 0.0, 0.0 ) ).xyz * _ca_freqHigh[2 ].xyz;
			vec3 flares = tex2Dlod( samp_tex3, vec4( texcoord.xy, 0, 0 ) ).xyz * _ca_freqHigh[3 ].z;
			float vignetting = 1.0 - pow( directionLen, _ca_freqHigh[1 ].w ) / directionLen * _ca_freqHigh[1 ].z;
			vec3 lensDirt = tex2Dlod( samp_bloomdustmap, vec4( 1.4 * ( texcoord.xy / _ca_freqLow[0 ].xy ) * vec2( 1.0, _ca_freqHigh[4 ].z ), 0.0, 0 ) ).rgb; if ( _ca_freqLow[1 ].x == 2 ) {
				...
				bloom += lensRefl * _ca_freqHigh[2 ].xyz * _ca_freqHigh[3 ].z;
			}
			linearHDR.xyz += flares + ( ( flares + bloom * lensDirt ) ) * _ca_freqHigh[4 ].w;
			linearHDR.xyz = autoExposure * ( linearHDR.xyz * _ca_freqHigh[4 ].x + bloom.xyz * _ca_freqHigh[4 ].y );
			linearHDR.xyz = _ca_freqLow[1 ].x == 0.0 ? mix( linearHDR.xyz, _ca_freqHigh[5 ].xyz, 1.0 - vignetting ) : linearHDR.xyz * vignetting;
			uvec2 idxPix = uvec2( pixelLocation.xy ) & 3;
			...
			{
				vec2 unscaledTC = texcoord.xy / _ca_freqLow[0 ].xy; if ( _ca_freqHigh[6 ].w != 0.0 ) {
					float pulse = sin( _ca_freqLow[2 ].x * 5.0 ) * 0.1 + 0.9;
					float mask = unscaledTC.x * ( 1.0 - unscaledTC.x ) * unscaledTC.y * ( 1.0 - unscaledTC.y );
					mask = saturate( 1.0 - mask );
					mask = pow(mask, 20.0);
					mask = 1.0 - mask;
					vec4 flowMap = tex2Dlod( samp_flowvariation, vec4( unscaledTexcoord.xy * vec2( _ca_freqHigh[7 ].x, _ca_freqHigh[7 ].y ) + vec2( _ca_freqLow[2 ].x * _ca_freqHigh[7 ].z, _ca_freqLow[2 ].x * _ca_freqHigh[7 ].w ), 0, 0 ) );
					...
					powerUpPulse = mix( powerUpPulse, pow( powerUpPulse, vec4( 5.0 ) ), _ca_freqHigh[8 ].x );
					powerUpPulse.xyz = powerUpPulse.xyz * powerUpPulseStatic.xyz;
					vec3 inColor = linearHDR.xyz;
					linearHDR.xyz = mix( linearHDR.xyz, linearHDR.xyz + powerUpPulse.xyz * _ca_freqHigh[6 ].xyz * pulse, powerUpPulse.w );
					linearHDR.xyz = mix( inColor, linearHDR.xyz, _ca_freqHigh[6 ].w );
				}
				{
					float overlayWeight = _ca_freqHigh[9 ].z; if ( overlayWeight > 0.0 ) {
						bool bottomOnly = _ca_freqHigh[9 ].w > 0.0;
						vec2 dimensions = vec2( _ca_freqHigh[9 ].x, _ca_freqHigh[9 ].y );
				...
				{
					float overlayWeight = _ca_freqHigh[11 ].z; if ( overlayWeight > 0.0 ) {
						bool bottomOnly = _ca_freqHigh[11 ].w > 0.0;
						vec2 dimensions = vec2( _ca_freqHigh[11 ].x, _ca_freqHigh[11 ].y );
						bvec2 mirrorTiles = bvec2( dimensions.x >= 0.0, dimensions.y >= 0.0 );
						dimensions = abs( dimensions );
						bvec2 tiled = bvec2( dimensions.x <= 0.5, dimensions.y <= 0.5 );
						dimensions = min( dimensions, vec2( 0.5 ) );
						if ( abs( 0.5 - unscaledTC.x ) >= 0.5 - dimensions.x ) {
							if ( abs( 0.5 - unscaledTC.y ) >= 0.5 - dimensions.y && ( !bottomOnly || unscaledTC.y <= 0.5 ) ) {
								...
								overlayColor *= _ca_freqHigh[12 ];
								linearHDR.xyz = mix( linearHDR.xyz, overlayColor.xyz, overlayColor.w * overlayWeight );
							}
						}
					}
				}; if ( _ca_freqHigh[13 ].x > 0.0 ) {
					vec4 scanlineTex = tex2Dlod( samp_emdistortionmap, vec4( texcoord.xy * vec2( 0.5, 100.0 ) + floor( vec2( _ca_freqLow[2 ].x * 15.0, _ca_freqLow[2 ].x * 20.0 ) ) * 0.3, 0, 0 ) );
					float colorBlend = linearHDR.x + ( 0.3333 * linearHDR.y ) + ( 0.6666 * linearHDR.z );
					linearHDR.xyz = mix( linearHDR.xyz, mix( vec3( colorBlend ) , vec3( colorBlend ) * vec3( 0.3, 0.6, 0.0 ) , scanlineTex.x ), saturate( _ca_freqHigh[13 ].y * 0.5 * ( 1.0 - colorBlend ) ) ); if ( _ca_freqHigh[13 ].x >= 2.0 ) {
				...
				} if ( _ca_freqHigh[14 ].x == 1.0 && _ca_freqHigh[13 ].x == 0.0) {
					...
					vec3 outlines = mix( vec3( 0.0 ), initialOutline.xyz * (linearHDR.x * 60.5), _ca_freqHigh[15 ].x );
					float pingScalar;
					vec3 gridColor;
					{
						...
						vec4 globalPos;
						{
							vec2 texcoordGrid = screenPosToTexcoord( pixelLocation.xy, _ca_freqHigh[0 ] );
							vec2 scaledTCGrid = vec2( texcoordGrid.x / _ca_freqLow[0 ].x, texcoordGrid.y / _ca_freqLow[0 ].y );
							...
							globalPos.x = dot( device, _ca_freqLow[3 ] );
							globalPos.y = dot( device, _ca_freqLow[4 ] );
							globalPos.z = dot( device, _ca_freqLow[5 ] );
							globalPos.w = dot( device, _ca_freqLow[6 ] );
							globalPos.xyzw /= vec4( globalPos.w );
						}
						float distance = length( globalPos.xyz - _ca_freqHigh[16 ].xyz );
						float maxPingRange = _ca_freqHigh[17 ].x;
						float minPingRange = max( 0.0, _ca_freqHigh[17 ].x - _ca_freqHigh[17 ].y );
						float maxVisionDistance = _ca_freqHigh[17 ].z;
						float pingPowerExponent = _ca_freqHigh[17 ].w;
						float gridVal;
						{
					...
					vec3 outlinedColor = mix( linearHDR.xyz, mix( infraRedWorld, outlines.xyz, saturate( outlines.x * 2.0 ) ), _ca_freqHigh[18 ].x );
					linearHDR.xyz = mix( outlinedColor.xyz, gridColor, pingScalar );
				} if ( _ca_freqHigh[14 ].x == 1.0 && _ca_freqHigh[13 ].x == 1.0 ) {
					vec4 outlineTex = tex2Dlod( samp_outlinesmap, vec4(unscaledTC.xy, 0, 0) );
					...
					float waveVal;
					{
						...
						float distance = length( globalPos.xyz - _ca_freqHigh[16 ].xyz );
						float maxPingRange = _ca_freqHigh[17 ].x;
						float minPingRange = _ca_freqHigh[17 ].x - _ca_freqHigh[17 ].y;
						float maxVisionDistance = _ca_freqHigh[17 ].z;
						...
						vec2 direction = globalPos.xy - _ca_freqHigh[16 ].xy;
						float radial = atan( direction.x, direction.y ) / 3.1415926;
						float depthNoiseLookup = mod( ( distance - ( maxPingRange / 4.0 ) ) / _ca_freqHigh[19 ].x, 1.0 );
						float heightNoiseLookup = mod( globalPos.z / _ca_freqHigh[19 ].y, 1.0 );
				...
				} if ( ( _ca_freqHigh[20 ].x > 0.0 ) && ( _ca_freqHigh[14 ].x == 0.0 ) && ( _ca_freqHigh[13 ].x == 0.0 ) ) {
					vec4 outlineTex = tex2Dlod( samp_outlinesmap, vec4( unscaledTC.xy, 0, 0 ) );
					linearHDR.xyz += outlineTex.xyz;
				} if ( _ca_freqHigh[21 ].x > 0.0 ) {
					vec4 emiTex = tex2Dlod( samp_emdistortionmap, vec4( texcoord.xy * vec2( 1.3, 1.3 ) + floor( vec2( _ca_freqLow[2 ].x * 15.0, _ca_freqLow[2 ].x * 20.0 ) ) * 0.3, 0, 0 ) );
					linearHDR.xyz = mix( linearHDR.xyz, emiTex.xyz, saturate( _ca_freqHigh[21 ].x ) ) * vec3( 1.0, 0.2, 0.1 );
				} if ( _ca_freqHigh[22 ].x > 0.0 ) {
					float eyeCrossX = mix( 1.0, 0.9, _ca_freqHigh[22 ].x );
					float eyeCrossOffset = mix( 0.0, 0.1, _ca_freqHigh[22 ].x );
					...
					linearHDR.xyz = mix( linearHDR.xyz, doubleVision, vec3( _ca_freqHigh[22 ].x ) );
				} if ( _ca_freqHigh[23 ].x > 0.0 ) {
					float desaturated = GetLuma( linearHDR.xyz );
					linearHDR.xyz = vec3( desaturated );
				} if ( _ca_freqHigh[24 ].w > 0.0 ) {
					linearHDR.xyz = mix( linearHDR.xyz, _ca_freqHigh[24 ].xyz, _ca_freqHigh[24 ].w );
				}
			};
			float lum = GetLuma( linearHDR.xyz );
			float shadow2Mid = saturate( lum * _ca_freqHigh[25 ].x + _ca_freqHigh[25 ].y );
			float mid2High = saturate( lum * _ca_freqHigh[25 ].z + _ca_freqHigh[25 ].w );
			float saturation = _ca_freqHigh[26 ].z * mid2High + _ca_freqHigh[26 ].y * shadow2Mid + _ca_freqHigh[26 ].x;
			float gamma = _ca_freqHigh[27 ].z * mid2High + _ca_freqHigh[27 ].y * shadow2Mid + _ca_freqHigh[27 ].x;
			vec4 color = _ca_freqHigh[28 ] * mid2High + _ca_freqHigh[29 ] * shadow2Mid + _ca_freqHigh[30 ];
			linearHDR.xyz = mix( vec3( lum ), linearHDR.xyz, saturation );
			linearHDR.xyz = linearHDR.xyz * color.xyz + vec3( color.w );
			float ShoStren = _ca_freqHigh[31 ].x, LinStren = _ca_freqHigh[31 ].y, LinAngle = 0.1, ToeStren = 0.2, ToeNum = _ca_freqHigh[31 ].z, ToeDenom = 0.3;
			vec3 c = max( linearHDR.xyz, vec3( 0.0 ) );
			vec3 toneMapped = ( ( c * ( ShoStren * c + _ca_freqHigh[32 ].x ) + _ca_freqHigh[32 ].y ) / ( c * ( ShoStren * c + LinStren ) + ToeStren * ToeDenom ) ) - _ca_freqHigh[31 ].w;
			outputColor = saturate( linearSRGB( pow( toneMapped, vec3( gamma ) ) * _ca_freqHigh[32 ].z ) + dither );
			outputColor = saturate( mix( vec3( 0.5 ), outputColor.xyz, _ca_freqHigh[3 ].www ) );
			if ( _ca_freqHigh[3 ].x > 2.0 ) {
