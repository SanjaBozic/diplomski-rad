...
void main() {
	{
		vec2 invResolutionScale = vec2( 1 / _fa_freqLow[0 ].x, 1 / _fa_freqLow[0 ].y );
		vec2 texcoord = screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[0 ] ) * invResolutionScale;
		{
			int NUM_TAPS_SIDE = 8;
			...
			out_FragColor0.xyz = accFar.xyz / float( accFar.w + 1e-6 );
			out_FragColor0.w = accFar.w / float( NUM_TAPS );
		};
	}
}