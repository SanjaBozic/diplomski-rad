...
void main() {
	{
		...
		{
			vec2 step = _fa_freqHigh[1 ].xy * _fa_freqHigh[0 ].zw;
			...
			for ( int s = 0; s < 8; s++ ) {
				sum += tex2Dlod( samp_tex0, vec4( texcoord + step * b_off[ s ], 0, 0 ) ).xyz * w[ s ];
			}
			out_FragColor0.xyz = sum.xyz * _fa_freqHigh[1 ].z + tex2Dlod( samp_tex1, vec4( texcoord, 0, 0 ) ).xyz;
		};
	}
}