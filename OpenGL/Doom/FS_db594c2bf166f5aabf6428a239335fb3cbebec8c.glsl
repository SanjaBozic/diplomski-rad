...
void main() {
	{
		float maskDissolve = tex2DDeGammaApprox( samp_transmap1, vofi_TexCoord0.xy ).x;
		float maskAmount = 1.0 - _fa_freqHigh[0 ].w;
		clip( maskDissolve - maskAmount );
		vec4 diffuse = tex2DDeGammaApprox( samp_transmap, vofi_TexCoord0.xy );
		float emissveMask = diffuse.w;
		float dissolveEdgeWidth = _fa_freqHigh[1 ].x;
		float dissolveEdge = smoothstep( maskAmount, maskAmount + dissolveEdgeWidth, maskDissolve );
		vec3 edgeColor = vofi_TexCoord1.xyz * vec3( dissolveEdge );
		vec3 edgeEmissive = mix( vec3( 0.0 ), edgeColor * ( 1.0 - dissolveEdge ), vec3( maskAmount ) );
		vec4 final = vec4( 0.0, 0.0, 0.0, 1.0 );
		final.xyz += diffuse.xyz * vec3( 1 - emissveMask );
		final.xyz += vofi_Color.xyz * vec3( emissveMask );
		final.xyz += edgeEmissive.xyz;
