	...
	{
		{
			...
			inputs.normal = sampleNormal.wyz;
			{
				{ if ( _fa_freqHigh[8 ].x > 0.0 ) {
						...
						float allRegionsCombinedMask = saturate( max( max( primaryColorMask, secondaryColorMask ), patternColorMask ) ); if ( _fa_freqHigh[9 ].z > 0.0 ) {
							vec3 normalMap = tex2Dlod( samp_tilingnormalmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[10 ].xy, 0, 0 ) ).xyz;
							vec3 tilingNormal;
							...
							customNormal = mix( customNormal, tilingNormal, vec3( ( 1.0 - patternColorMask ) * primaryColorMask ) * _fa_freqHigh[9 ].z );
						} if ( _fa_freqHigh[9 ].w > 0.0 ) {
							vec3 normalMap2 = tex2Dlod( samp_tilingnormalmap2, vec4( vofi_TexCoord0.xy * _fa_freqHigh[10 ].zw, 0, 0 ) ).xyz;
							vec3 tilingNormal2;
							...
							customNormal = mix( customNormal, tilingNormal2, vec3( ( 1.0 - patternColorMask ) * secondaryColorMask ) * _fa_freqHigh[9 ].w );
						}
						float tilingSmoothness = tex2Dlod( samp_tilingscratchdirtmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[9 ].xy, 0, 0 ) ).z;
						customAlbedo = mix( customAlbedo, _fa_freqHigh[11 ].xyz, primaryColorMask );
						customSpecular = mix( customSpecular, _fa_freqHigh[12 ].xyz, vec3( primaryColorMask ) );
						float primaryColorSmoothness = mix( _fa_freqHigh[11 ].w, ColorOverlay( _fa_freqHigh[11 ].w, tilingSmoothness ), _fa_freqHigh[12 ].w );
						customSmoothness = mix( customSmoothness, primaryColorSmoothness, primaryColorMask );
						customAlbedo = mix( customAlbedo, _fa_freqHigh[13 ].xyz, secondaryColorMask );
						customSpecular = mix( customSpecular, _fa_freqHigh[14 ].xyz, vec3( secondaryColorMask ) );
						float secondaryColorSmoothness = mix( _fa_freqHigh[13 ].w, ColorOverlay( _fa_freqHigh[13 ].w, tilingSmoothness ), _fa_freqHigh[14 ].w );
						customSmoothness = mix( customSmoothness, secondaryColorSmoothness, secondaryColorMask ); if ( _fa_freqHigh[15 ].x > 0.0 ) {
							float patternMap = tex2Dlod( samp_tilingpatternmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[15 ].xy, 0, 0 ) ).x;
							customAlbedo = mix( customAlbedo, mix( _fa_freqHigh[16 ].xyz, _fa_freqHigh[17 ].xyz, patternMap ), patternColorMask );
							vec3 pattern1Specular = mix( _fa_freqHigh[18 ].xyz, _fa_freqHigh[19 ].xyz, patternMap );
							customSpecular = mix( customSpecular, pattern1Specular, vec3( patternColorMask ) );
							float pattern1Smoothness = mix( _fa_freqHigh[16 ].w, _fa_freqHigh[17 ].w, patternMap );
							float pattern1SmoothnessScale = mix( _fa_freqHigh[18 ].w, _fa_freqHigh[19 ].w, patternMap );
						...
						float scratchMask = 0.0; if ( _fa_freqHigh[20 ].x > 0.0 && _fa_freqHigh[21 ].w > 0.0 ) {
							float scratchMap = tex2Dlod( samp_tilingscratchdirtmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[20 ].xy, 0, 0 ) ).x;
							scratchMask = clamp( curvatureMask - 0.5, 0.0, 0.5 ) * 2.0;
							scratchMask = pow( scratchMask, _fa_freqHigh[21 ].w );
							float finalScratchMask = ColorBurn( scratchMask, scratchMap );
							finalScratchMask *= allRegionsCombinedMask;
							customAlbedo = mix( customAlbedo, _fa_freqHigh[21 ].xyz, finalScratchMask );
							customSpecular = mix( customSpecular, _fa_freqHigh[22 ].xyz, vec3( finalScratchMask ) );
							customSmoothness = mix( customSmoothness, _fa_freqHigh[22 ].w, finalScratchMask );
						}
						float dirtMask = 0.0;
						float finalDirtMask = 0.0; if ( _fa_freqHigh[20 ].z > 0.0 && _fa_freqHigh[23 ].w > 0.0 ) {
							float dirtMap = tex2Dlod( samp_tilingscratchdirtmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[20 ].zw, 0, 0 ) ).y;
							dirtMask = 1.0 - ( clamp( curvatureMask, 0.0, 0.5 ) * 2.0 );
							dirtMask = mix( 0.0, dirtMask, pow( 0.8, _fa_freqHigh[23 ].w ) );
							finalDirtMask = ColorDodge( dirtMask, dirtMap );
							finalDirtMask = pow( finalDirtMask, _fa_freqHigh[23 ].w );
							customAlbedo = mix( customAlbedo, _fa_freqHigh[23 ].xyz, finalDirtMask ) ;
							customSpecular = mix( customSpecular, _fa_freqHigh[24 ].xyz, vec3( finalDirtMask ) );
							customSmoothness = mix( customSmoothness, _fa_freqHigh[24 ].w, finalDirtMask );
						}
						inputs.albedo = customAlbedo;
						inputs.smoothness = customSmoothness;
						inputs.specular = customSpecular;
						inputs.emissive = ( vec3( bloomMask ) * _fa_freqHigh[25 ].xyz ) * _fa_freqHigh[25 ].w;
				...
				};
				{
					...
					woundMap.xyz -= vec3( 0.5 );
					{
						...
						vec3 diffuseBloodLerp = mix( inputs.albedo.xyz.xyz, _fa_freqHigh[26 ].xyz, vec3( woundMask ) );
						inputs.albedo.xyz.xyz *= mix( vec3( 1.0 ), _fa_freqHigh[27 ].xyz, vec3( woundMask ) );
						...
						inputs.normal.xy += mix( woundMap.xy * vec2( woundMask ), vec2( 0.0 ), vec2( _fa_freqHigh[27 ].w ) );
					};
				};
			};
			{
				uint skipStaticModel = _fa_freqHigh[28 ].x > 0 ? ( 1 << 3 ) : 0;
				uint clusterOffset;
				{
					{
			...
			{
				{ if ( _fa_freqHigh[29 ].x > 0 ) {
						vec2 resolution = abs( vec2( 1.0, 1.0 ) / _fa_freqHigh[30 ].zw );
						...
						float magnitude = 1.0;
						float pulsefrequency = _fa_freqHigh[31 ].x; if ( _fa_freqHigh[29 ].y >= _fa_freqHigh[31 ].z ) {
							pulsefrequency = _fa_freqHigh[31 ].y;
						}
						float width = mix( _fa_freqHigh[32 ].x, _fa_freqHigh[32 ].y, _fa_freqHigh[29 ].y );
						float zValue = ( vofi_TexCoord4.xyz.z ) * width;
						float pulse = sin( zValue + _fa_freqLow[11 ].x * pulsefrequency ) * magnitude + magnitude;
						float power = _fa_freqHigh[32 ].z;
						pulse = pow( pulse, power );
						pulse *= fresnel;
						float edgeGlow = pow( fresnel, 4.0 );
						vec3 finalPulse = saturate( dither.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[33 ].xyz * 2.0;
						finalPulse /= _fa_freqHigh[34 ].xyz;
				...
				{ if ( _fa_freqHigh[35 ].w > 0 ) {
						...
						vec3 finalPulseFresnel = saturate( digitalNoise.x * ( fresnel ) + edgeGlow ) * _fa_freqHigh[35 ].xyz;
						vec3 finalPulseFlat = saturate( digitalNoise.x ) * _fa_freqHigh[35 ].xyz;
						vec3 finalPulse = mix( finalPulseFresnel, finalPulseFlat, vec3( 0.25 ) );
						vec3 constantGlow = saturate( 0.5 * ( fresnel ) + edgeGlow ) * _fa_freqHigh[35 ].xyz;
						finalPulse /= _fa_freqHigh[34 ].xyz;
						vec3 finalPulseColor = finalPulse * _fa_freqHigh[35 ].w;
				...
				}; if ( _fa_freqHigh[36 ].x * ( 1.0 - _fa_freqHigh[37 ].x ) > 0.0 ) {
					float teamfxID = ( _fa_freqHigh[38 ].x * -1.0 ) + 1.0;
					...
					float distance = length( vofi_TexCoord4.xyz );
					{
						...
						float fadeStart = _fa_freqHigh[39 ].x;
						float fadeEnd = _fa_freqHigh[40 ].x;
						float fadeRange = max( 1.0, fadeEnd - fadeStart );
						float value = max( 0.0, distance - fadeStart );
						float alpha = saturate( value / fadeRange );
						fresnel = saturate( pow( fresnel, _fa_freqHigh[41 ].x ) * _fa_freqHigh[42 ].y );
						vec3 rimLightColor = ( 1.0 * _fa_freqHigh[42 ].x ) * _fa_freqHigh[43 ].xyz;
						vec3 finalRimlight = fresnel * rimLightColor;
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz + finalRimlight, alpha * teamfxID * _fa_freqHigh[44 ].x );
					};
				}
				{ if ( _fa_freqHigh[29 ].x > 0.0 ) {
						vec2 resolution = abs( vec2( 1.0 ) / _fa_freqHigh[30 ].zw );
						...
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						float pulseTime = mix( 10.0, 30.0, _fa_freqHigh[29 ].y );
						...
						finalPulse = saturate( dither.x * ( fresnel + pulse ) + edgeGlow ) * _fa_freqHigh[33 ].xyz * 2.0;
						inputs.diffuse_lighting.xyz.xyz += finalPulse;
					}
				};
				{ if ( _fa_freqHigh[37 ].x > 0.0 ) {
						vec4 colorWave = tex2DlodDeGamma( samp_transmap, vec4( vofi_TexCoord0.xy * _fa_freqHigh[45 ].xy + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[45 ].zw, 0.0, 0.0) );
						...
						vec3 close = crevices * crevices * vec3( 180.0 ) * _fa_freqHigh[46 ].xyz + finalOutline * _fa_freqHigh[46 ].xyz;
						vec3 far = _fa_freqHigh[46 ].xyz * vec3( 0.1 ) * colorWave.xyz * vec3( fresnel2 ) + vec3( fresnel2 ) * _fa_freqHigh[46 ].xyz * vec3( 20.0 );
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz, ( inputs.diffuse_lighting.xyz + mix( close, far, vec3( farFade ) ) * colorWave.xyz ) + _fa_freqHigh[46 ].xyz * vec3( 0.01 ) * vec3( fresnel2 ), vec3( _fa_freqHigh[37 ].x ) );
					}
				};
				{ if ( _fa_freqHigh[47 ].x > 0.0 ) {
						float fresnel = 1.0 - dot( inputs.normal.xyz, normalize( -vofi_TexCoord4.xyz ) );
						inputs.diffuse_lighting.xyz.xyz += ( ( _fa_freqHigh[48 ].xyz * 0.001 ) * _fa_freqHigh[48 ].w * _fa_freqHigh[49 ].x ) * fresnel * fresnel * 0.8;
					}
				};
				{ if ( _fa_freqHigh[50 ].x == 1.0 ) { if ( _fa_freqHigh[51 ].x == 1.0 ) {
							vec4 shieldPattern = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[45 ].xy ) * 3.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[42 ].zw, 0.0, 0.0 ) );
							vec4 shieldPattern2 = tex2DlodDeGamma( samp_transmap2, vec4( ( vofi_TexCoord0.xy * _fa_freqHigh[45 ].xy ) * 2.0 + vec2( _fa_freqLow[11 ].x ) * _fa_freqHigh[45 ].zw, 0.0, 0.0 ) );
							...
							inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 1.0, 0.25 ) * vec3( 50.0 ), _fa_freqHigh[51 ].w );
						}
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.5, 1.0, 0.5 ) * vec3( 50.0 ), _fa_freqHigh[51 ].y );
						inputs.diffuse_lighting.xyz = mix( inputs.diffuse_lighting.xyz, inputs.diffuse_lighting.xyz * vec3( 0.25, 0.0, 0.0 ) * vec3( 50.0 ), _fa_freqHigh[51 ].z );
					}
				};
				{ if ( _fa_freqHigh[52 ].x > 0.0 ) {
						...
						inputs.diffuse_lighting.xyz.xyz = mix( inputs.diffuse_lighting.xyz.xyz, holoColor.xyz, _fa_freqHigh[52 ].x );
						inputs.diffuse_lighting.xyz.xyz = mix (inputs.diffuse_lighting.xyz.xyz, inputs.diffuse_lighting.xyz.xyz * noise.xyz * 2.3, _fa_freqHigh[52 ].x);
					}
				};
			};
			{
