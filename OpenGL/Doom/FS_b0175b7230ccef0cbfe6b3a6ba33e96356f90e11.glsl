...
void main() {
	{
		vec2 texcoord = screenPosToTexcoord( gl_FragCoord.xy - 0.5, _fa_freqHigh[0 ] );
		vec2 translate = _fa_freqHigh[0 ].zw;
		vec4 total = tex2D( samp_viewcolormap, texcoord.xy );
		texcoord.x += translate.x;
		total += tex2D( samp_viewcolormap, texcoord.xy );
		texcoord.y += translate.y;
		total += tex2D( samp_viewcolormap, texcoord.xy );
		texcoord.x -= translate.x;
		total += tex2D( samp_viewcolormap, texcoord.xy );
		out_FragColor0 = total * 0.25;
	}
}